...
class TritonDecoder(Decoder):
    ...
    def _get_multimodal_enc_tensors(self, request: Request, preproc: PreprocResponse):
        name_map_request = {
            "image_input": "IMAGE",
        }
        name_map_preproc = {
    ...
    def _get_multimodal_enc_response(self, triton_output):
        ...
        return self.convert_triton_response(
            triton_output, MultimodalEncResponse, name_map
        )
    ...
    def _get_llm_tensors_from_request(
        ...
        if num_output_tokens is not None:
            ...
            out_len = (
                len(draft_request.draft_input_ids[0]) + 1
        ...
        if out_len_tensor is None:
            ...
            tensors.append(
                pb_utils.Tensor(
                    "request_output_len", np.array(out_len_tensor, dtype=np.int32)
                )
            )
        ...
        return_context_logits = np.array(
            [return_context_logits_data] * batch_size, dtype=bool
        )
        return_generation_logits = np.array(
            [return_generation_logits_data] * batch_size, dtype=bool
        )

        assert len(return_context_logits.shape) == 2
        assert len(return_generation_logits.shape) == 2

        tensors.append(pb_utils.Tensor("return_context_logits", return_context_logits))
        tensors.append(
            pb_utils.Tensor("return_generation_logits", return_generation_logits)
        )
        return tensors
