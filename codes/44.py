...
@triton.jit
def swizzle_tiled_m_with_padding(
    pid_m, num_pid_m_per_rank, node_id, rank, LOCAL_WORLD_SIZE, NNODES
):
...
def threadblock_swizzle_gemm_reduce_scatter_triton(
    tiled_m, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M
):
    import torch
    ...
    def _threadblock_swizzle_run(
        ...
        thread_idx = tid(0)
        tiled_m_new = threadblock_swizzle_gemm_reduce_scatter_kernel(
            tiled_m, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M, DEBUG
        )
        if thread_idx == 0:
            st(output, tiled_m_new)

    output = torch.empty((1,), dtype=torch.int32, device="cuda")
    _threadblock_swizzle_run[(1,)](
        output, tiled_m, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M, num_warps=1
    )
    return int(output.item())


def threadblock_swizzle_gemm_reduce_scatter(
    tiled_m, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M
):
    LOCAL_WORLD_SIZE = WORLD_SIZE // NNODES
    node_id = rank // LOCAL_WORLD_SIZE
    ...
    tiled_m_sizes = np.concatenate(
        (swizzled_tiled_m_sizes[-node_start:], swizzled_tiled_m_sizes[:-node_start])
    )
    ...
    tiled_m_intra_node_new = (
        tiled_m_intra_node + rank_offset
    ) % swizzled_tiled_m_sizes[n]
    logging.debug(
        f"{swizzled_tiled_m_sizes} => {swizzled_tiled_m_sizes_accum} => {tiled_m_sizes_accum}"
    )
    ...
    def _check_threadblock_swizzle_with(M):
        ...
        for rank in range(WORLD_SIZE):
            ...
            for n in range(cdiv(M, BLOCK_SIZE_M)):
                x = threadblock_swizzle_gemm_reduce_scatter_triton(
                    n, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M
                )
                y = threadblock_swizzle_gemm_reduce_scatter(
                    n, M, rank, WORLD_SIZE, NNODES, BLOCK_SIZE_M
                )
                assert x == y, f"{n} => {x} {y}"
