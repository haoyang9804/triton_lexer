...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "token_softmax_llama", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(4, 14):
            ...
            Logics = torch.rand(
                (head_num, batch_size, max_input_len), dtype=torch.float32
            )
            ...
            self.input_tensors.append(
                (Logics, B_Start_Loc, B_Seqlen, Prob_Out, max_input_len)
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        total_bytes = 2 * Logics.numel() * Logics.element_size()
