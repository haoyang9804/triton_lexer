...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "layernorm_fwd_triton", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(4, 18):
            ...
            X = torch.rand((2048, 8, size), dtype=torch.float32)
    ...
    def get_gbps(self, input_tensor, runtime):
        X, W = input_tensor
        total_bytes = (X.numel() + W.numel() + X.numel()) * X.element_size()
