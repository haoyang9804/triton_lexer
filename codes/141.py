...
@pytest.fixture
def create_chunk_scan_tensors(
    batch, seqlen, nheads, ngroups, chunk_size, headdim, dstate, has_z, has_d, dtype
):
    ...
    dA_cumsum_b = (
        (0.5 * torch.randn(nheads, nheads, nchunks, chunk_size))
        .to(device=device, dtype=torch.float32)
        .requires_grad_()
    )
    C = (
        (0.5 * torch.randn(batch, seqlen, ngroups, dstate))
        .to(device=device, dtype=torch.float32)
        .requires_grad_()
    )
    states_f = (
        (0.5 * torch.randn(batch, nchunks, nheads, headdim, dstate))
        .to(device=device, dtype=torch.float32)
        .requires_grad_()
    )
    states_b = (
    ...
    D = (
        (0.5 * torch.randn(nheads, headdim))
        .to(device=device, dtype=torch.float32)
        .requires_grad_()
        if has_d
        else None
...
def cumsum_compare(create_chunk_scan_tensors):
    CB, x, delta, dA_cumsum_f, dA_cumsum_b, C, states_f, states_b, D, z = (
        create_chunk_scan_tensors
    )
    (
        CB_ref,
        x_ref,
        ...
        D_ref,
        z_ref,
    ) = create_ref_tensors(*create_chunk_scan_tensors)

    out, out_x = _chunk_scan_fwd(CB, x, delta, dA_cumsum_f, C, states_f, D, z)
    out2, out_x2 = _chunk_scan_fwd(
        CB.flip([1, 3, 4]),
        x.flip([1]),
        delta.flip([2, 3]),
        dA_cumsum_b.flip([2]),
        ...
        states_b.flip([1, 3, 4]),
        D.flip([0]),
        z.flip([1]) if z is not None else None,
    )
