...
maxauto_baseline = torch.compile(
    baseline_torch, fullgraph=True, mode="max-autotune-no-cudagraphs"
)


from variants.highmem import linear_cross_entropy as linear_cross_entropy_highmem
from variants.double_recomp2 import (
    linear_cross_entropy as linear_cross_entropy_double_recomp,
)
from variants.double_recomp3 import (
    linear_cross_entropy as linear_cross_entropy_parallel_recomp,
)
...
from variants.malek_like_with_my_fwd import (
    linear_cross_entropy as linear_cross_entropy_z_chunks,
)
from variants.malek_like_with_my_fwdNV import (
    linear_cross_entropy as linear_cross_entropy_z_chunksNV,
)


from variants.malek_like_with_my_fwdNV5 import (
    linear_cross_entropy as linear_cross_entropy_z_chunksNV_v2,
)
from variants.final_fusion import (
    linear_cross_entropy as linear_cross_entropy_z_chunks_full_fused,
)
from variants.final_fusion_parallel import (
    linear_cross_entropy as linear_cross_entropy_z_chunks_full_fused_parallel,
)
...
from variants.final_fusion_parallel_splitN_V_usable import (
    linear_cross_entropy as linear_cross_entropy_final,
)
from variants.final_fusion_usable_variant import (
    linear_cross_entropy as linear_cross_entropy_final_var,
)
from variants.final_fusion_VII import (
    linear_cross_entropy as linear_cross_entropy_final_with_all_tricks,
)
...
def provider_lookup(provider, x, y, At, A):
    ...
    if provider == "triton-z-chunks-in-sram-fused":
        fn = lambda: linear_cross_entropy_z_chunks_full_fused(
            x, y, At, N_chunk_size=4096
        )
    if provider == "triton-z-chunks-in-sram-fusedP":
        fn = lambda: linear_cross_entropy_z_chunks_full_fused_parallel(
            x, y, At, N_chunk_size=4096
        )
    if provider == "triton-z-chunks-in-sram-fusedP-2k":
        fn = lambda: linear_cross_entropy_z_chunks_full_fused_parallel(
            x, y, At, N_chunk_size=2048
        )
    if provider == "triton-z-chunks-in-sram-fusedP-05k":
        fn = lambda: linear_cross_entropy_z_chunks_full_fused_parallel(
            x, y, At, N_chunk_size=512
        )
    if provider == "triton-z-chunks-in-sram-fusedP-split":
        fn = lambda: linear_cross_entropy_z_chunks_full_fused_parallel_split(
            x, y, At, N_chunk_size=4096
        )
    ...
    if provider == "triton-z-chunks-in-sram-vii":
        fn = lambda: linear_cross_entropy_final_with_all_tricks(
            x, y, At, N_chunk_size=4096
        )
        ...
        for acc in ["fp16", "bf16", "fp16-mixed"]:
            configs.append(
                triton.testing.Benchmark(
                    x_names=[variable],
                    x_vals=[(2**i) // f for i in range_dict[variable]],
                    x_log=True,
                    line_arg="provider",
...
def benchmark(H=2048, V=131072, N=(4096 * 4), provider="torch", mode="fwd", acc="fp16"):
    ...
    if acc == "fp16":
        ...
        dtype = torch.float32
    x = torch.randn(N, H, requires_grad=True, device=device, dtype=dtype)
    ...
    try:
        ...
        if mode == "fwd":
            ...
            def test_fn():
                with torch.autocast(
                    "cuda",
                    dtype=torch.bfloat16 if "bf16" in acc else torch.float16,
                    enabled="mixed" in acc,
                ):
                    loss = fn()
                loss.backward()
        ...
        if ("base" in provider) and ((N * V) >= (32768 * 131072)):
            ...
            ms, min_ms, max_ms = triton.testing.do_bench(
                test_fn, quantiles=quantiles, warmup=1000, rep=5000
            )
            ms, min_ms, max_ms = triton.testing.do_bench(
                test_fn, quantiles=quantiles, warmup=1000, rep=5000
            )
...
def benchmark_with_memory_reporting(func, quantiles, *args, **kwargs):
    ...
    ms, min_ms, max_ms = triton.testing.do_bench(
        lambda: func(*args, **kwargs), quantiles=quantiles, warmup=50, rep=200
    )
    ...
    for acc in ["fp16", "bf16", "fp16-mixed"]:
        for variable in ["H", "N", "V"]:
            configs.append(
                triton.testing.Benchmark(
                    x_names=[variable],
                    x_vals=[2**i for i in range_dict[variable]],
                    x_log=True,
                    line_arg="provider",
                    ylabel="Peak Memory in GB (excluding inputs)",
                    line_vals=method_list,
                    line_names=method_list,
                    args={"mode": mode, "acc": acc},
...
def benchmark(H=2048, V=131072, N=(4096 * 4), provider="torch", mode="fwd", acc="fp16"):
    ...
    try:
        ...
        if ("base" in provider) and ((N * V) >= (32768 * 131072)):
            ...
            ms, min_ms, max_ms, max_memory_allocated = benchmark_with_memory_reporting(
                test_fn, quantiles=quantiles
            )
            ms, min_ms, max_ms, max_memory_allocated = benchmark_with_memory_reporting(
                test_fn, quantiles=quantiles
            )
...
def get_reference_vals(x, y, A):
    ...
    reference_A_grad = A.grad.float().detach().clone()
    reference_x_grad = x.grad.float().detach().clone()
...
def simple_bench(fn, x, At, A, reference_loss, reference_x_grad, reference_A_grad):
    ...
    with torch.autocast(device_type="cuda", enabled=False):
        ...
        x_error = torch.dist(reference_x_grad, x.grad).item()
        ...
        for variable in ["H", "N", "V"]:
            configs.append(
                triton.testing.Benchmark(
                    ...
                    line_vals=method_list,
                    line_names=method_list,
                    ylabel="Average Abs. Error (torch.dist to reference)",
...
def benchmark(H=2048, V=131072, N=(4096 * 4), provider="torch", mode="fwd", acc="fp16"):
    ...
    x = 0.1 * A[y].clone().detach() + torch.randn(N, H, device=device, dtype=dtype)
    x.requires_grad_()
