...
def test_sync_client_init_raises_error_when_invalid_url_provided():
    with pytest.raises(PyTritonClientInvalidUrlError, match="Invalid url"):
        ModelClient(["localhost:8001"], "dummy")
...
def test_sync_grpc_client_init_raises_error_when_use_non_lazy_init_on_non_responding_server():
    with pytest.raises(
        PyTritonClientTimeoutError, match="Waiting for (.*) to be ready timed out."
    ):
        ModelClient("dummy:43299", "dummy", lazy_init=False, init_timeout_s=1)


def test_sync_grpc_client_init_raises_error_when_requested_unavailable_model_and_non_lazy_init_called(
    mocker,
):
    ...
    mock_get_repo_index.return_value = service_pb2.RepositoryIndexResponse(
        models=[
            service_pb2.RepositoryIndexResponse.ModelIndex(
                name="OtherName", version="1", state="READY", reason=""
            ),
        ]
    )
    mocker.patch.object(
    ...
    with pytest.raises(
        PyTritonClientTimeoutError,
        match="Waiting for model (.*) to be ready timed out.",
    ):
        ModelClient(
            _GRPC_LOCALHOST_URL, "NotExistentModel", lazy_init=False, init_timeout_s=1.5
        )
    ...
    with pytest.raises(
        ...
        ModelClient(
            _GRPC_LOCALHOST_URL, "OtherName", "2", lazy_init=False, init_timeout_s=1.5
        )


def test_sync_grpc_client_init_obtain_expected_model_config_when_lazy_init_is_disabled(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    ...
    spy_get_model_config = mocker.spy(
...
def test_sync_grpc_client_model_config_raises_error_when_requested_unavailable_model(
    ...
    with ModelClient(
        _GRPC_LOCALHOST_URL, "NonExistentModel", init_timeout_s=1.5
    ) as client:
        with pytest.raises(
            PyTritonClientTimeoutError,
            match="Waiting for model (.*) to be ready timed out.",
    ...
    with ModelClient(
        _GRPC_LOCALHOST_URL, "OtherName", "2", init_timeout_s=1.5
    ) as client:
        with pytest.raises(
            PyTritonClientTimeoutError,
            match="Waiting for model (.*) to be ready timed out.",
...
def test_sync_grpc_client_infer_sample_returns_expected_result_when_positional_args_are_used(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    ...
    with ModelClient(
        _GRPC_LOCALHOST_URL, ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG.model_name
    ) as client:
        mock_infer = mocker.patch.object(client._infer_client, "infer")
        mock_infer.return_value = wrap_to_grpc_infer_result(
        ...
        for arg_name, arg_value in expected_kwargs.items():
            if arg_name not in ["inputs", "outputs"]:
        ...
        assert [output.name() for output in called_kwargs.get("outputs")] == list(
            expected_kwargs["outputs"]
        )
        inputs_called_arg = {
            i.name(): extract_array_from_grpc_infer_input(i)
            for i in called_kwargs.get("inputs")
        }
        verify_equalness_of_dicts_with_ndarray(
            inputs_called_arg, expected_kwargs["inputs"]
        )
        verify_equalness_of_dicts_with_ndarray(expected_result, result)


def test_sync_grpc_client_infer_sample_returns_expected_result_when_infer_on_model_with_batching(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    patch_grpc_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
    ...
    server_result = {
        name: data[np.newaxis, ...] for name, data in expected_result.items()
    }

    with ModelClient(
        _GRPC_LOCALHOST_URL, ADD_SUB_WITH_BATCHING_MODEL_CONFIG.model_name
    ) as client:
        mock_infer = mocker.patch.object(client._infer_client, "infer")
        mock_infer.return_value = wrap_to_grpc_infer_result(
        ...
        expected_kwargs.update(
            {
                "model_name": ADD_SUB_WITH_BATCHING_MODEL_CONFIG.model_name,
                "inputs": {
                    name: data[np.newaxis, ...] for name, data in inputs_dict.items()
                },
                "outputs": list(expected_result),
            }
        )
...
def test_sync_grpc_client_infer_sample_returns_expected_result_when_named_args_are_used(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    patch_grpc_client__model_up_and_ready(mocker, ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG)
...
def test_sync_grpc_client_infer_batch_returns_expected_result_when_positional_args_are_used(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    patch_grpc_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
...
def test_sync_grpc_client_infer_batch_returns_expected_result_when_named_args_are_used(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    patch_grpc_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
...
def test_sync_grpc_client_infer_batch_raises_error_when_model_doesnt_support_batching(
    mocker,
):
    patch_grpc_client__server_up_and_ready(mocker)
    patch_grpc_client__model_up_and_ready(mocker, ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG)
...
def test_sync_http_client_init_obtain_expected_model_config_when_lazy_init_is_disabled(
    mocker,
):
...
def test_sync_http_client_init_raises_error_when_requested_unavailable_model_and_non_lazy_init_called(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    mock_get_repo_index = mocker.patch.object(
    ...
    with pytest.raises(
        ...
        ModelClient(
            _HTTP_LOCALHOST_URL, "NotExistentModel", lazy_init=False, init_timeout_s=1.5
        )
    ...
    with pytest.raises(
        ...
        ModelClient(
            _HTTP_LOCALHOST_URL, "OtherName", "2", lazy_init=False, init_timeout_s=1.5
        )


def test_sync_http_client_model_config_raises_error_when_requested_unavailable_model(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    mock_get_repo_index = mocker.patch.object(
    ...
    with ModelClient(
        _HTTP_LOCALHOST_URL, "NonExistentModel", init_timeout_s=1.5
    ) as client:
        with pytest.raises(
            PyTritonClientTimeoutError,
            match="Waiting for model (.*) to be ready timed out.",
    ...
    with ModelClient(
        _HTTP_LOCALHOST_URL, "OtherName", "2", init_timeout_s=1.5
    ) as client:
        with pytest.raises(
            PyTritonClientTimeoutError,
            match="Waiting for model (.*) to be ready timed out.",
...
def test_sync_http_client_infer_sample_returns_expected_result_when_infer_on_model_with_batching(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    patch_http_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
    ...
    with ModelClient(
        _HTTP_LOCALHOST_URL, ADD_SUB_WITH_BATCHING_MODEL_CONFIG.model_name
    ) as client:
        mock_infer = mocker.patch.object(client._infer_client, "infer")
        mock_infer.return_value = wrap_to_http_infer_result(
            ADD_SUB_WITH_BATCHING_MODEL_CONFIG, "0", server_result
        )
        result = client.infer_sample(a, b)
        ...
        inputs_called_arg = {
            i.name(): extract_array_from_http_infer_input(i)
            for i in called_kwargs.get("inputs")
        }
        verify_equalness_of_dicts_with_ndarray(
            inputs_called_arg, expected_kwargs["inputs"]
...
def test_sync_http_client_infer_sample_returns_expected_result_when_positional_args_are_used(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    ...
    with ModelClient(
        _HTTP_LOCALHOST_URL, ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG.model_name
    ) as client:
        mock_infer = mocker.patch.object(client._infer_client, "infer")
        mock_infer.return_value = wrap_to_http_infer_result(
            ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG, "0", server_result
...
def test_sync_http_client_infer_sample_from_existing_client(mocker, infer_state):
    ...
    with ModelClient(
        ...
        spy_is_server_ready = mocker.spy(
            tritonclient.http.InferenceServerClient, "is_server_ready"
        )
        spy_is_server_live = mocker.spy(
        ...
        with ModelClient.from_existing_client(client) as client_from_existing:
            mock_infer_from_existing = mocker.patch.object(
                client_from_existing._infer_client, "infer"
            )
            ...
            verify_equalness_of_dicts_with_ndarray(
                expected_result, result_from_existing
            )
...
def test_sync_http_client_infer_batch_returns_expected_result_when_positional_args_are_used(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    patch_http_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
...
def test_sync_http_client_infer_batch_returns_expected_result_when_named_args_are_used(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    patch_http_client__model_up_and_ready(mocker, ADD_SUB_WITH_BATCHING_MODEL_CONFIG)
...
def test_sync_http_client_infer_batch_raises_error_when_model_doesnt_support_batching(
    mocker,
):
    patch_http_client__server_up_and_ready(mocker)
    patch_http_client__model_up_and_ready(mocker, ADD_SUB_WITHOUT_BATCHING_MODEL_CONFIG)
