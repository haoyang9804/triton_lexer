...
def _inner_function(x_block, y_block, A, num_blocks, ignore_index=5):
    return (
        F.cross_entropy(F.linear(x_block, A), y_block, ignore_index=ignore_index)
        / num_blocks
    )
...
def early_config_prune(configs, named_args, **kwargs):
    ...
    if dtype not in [torch.float16, torch.float32]:
        configs = [
            ...
            or (config.kwargs.get("SPLIT_V", 1) == 1)
        ]
        ...
        with torch.cuda.device(x.device.index):
            ...
            with torch.inference_mode():
                ...
                for idx_N_group in range(math.ceil(N / N_group)):
                    ...
                    V_BLOCK_SIZE = linear_xent_fwd_kernel_matmul_t.best_config.kwargs[
                        "V_BLOCK_SIZE"
                    ]
                    ...
                    lse_sum += (
                        lse_global.sum() + z_regularization * lse_global.pow(2).sum()
                    ) / reduction
...
def linear_cross_entropy(
    x, y, At, ignore_index=-100, z_regularization=0.0, N_chunk_size: int = 4096
):
    return LinearCrossEntropyLoss.apply(
        x, y, At, ignore_index, z_regularization, N_chunk_size
    )
...
if __name__ == "__main__":
    ...
    loss = baseline_torch(
        x.float(), y, A.float(), ignore_index=5, z_regularization=z_reg
    )
    loss.backward()
