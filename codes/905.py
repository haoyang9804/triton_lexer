...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "apply_penalty", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 10):
            ...
            p_token_ids = torch.randint(
                0, vocab_size, (batch_size * seq_len,), dtype=torch.int32
            )
            p_token_counts = torch.randint(
                1, 10, (batch_size * seq_len,), dtype=torch.int32
            )
            p_cumsum_seq_len = torch.cumsum(
                torch.randint(1, seq_len, (batch_size + 1,), dtype=torch.int32), dim=0
            )
    ...
    def to_cuda(self, input_tensor):
        ...
        return (
            Logits.cuda(),
            presence_penalty.cuda(),
            freqency_penalty.cuda(),
            repetition_penalty.cuda(),
            p_token_ids.cuda(),
            p_token_counts.cuda(),
            p_cumsum_seq_len.cuda(),
            p_max_len_in_batch,
        )
