...
def chunk_rwkv6_fwd_cumsum(
    ...
    gi, ge = torch.empty_like(g, dtype=torch.float), torch.empty_like(
        g, dtype=torch.float
    )
...
def chunk_rwkv6_bwd(
    ...
    dA = chunk_gla_bwd_dA(
        v=v, do=do, scale=scale, cu_seqlens=cu_seqlens, chunk_size=chunk_size
    )
    dv = chunk_gla_bwd_dv(
        k=k, g=gi, A=A, do=do, dh=dh, cu_seqlens=cu_seqlens, chunk_size=chunk_size
    )
    dq, dk = chunk_rwkv6_bwd_dqk_intra(
        q=q, k=k, gi=gi, ge=ge, dA=dA, cu_seqlens=cu_seqlens, chunk_size=chunk_size
    )
