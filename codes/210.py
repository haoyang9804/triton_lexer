...
def preprocess_input_cf(imgs, w, h, to_tensor=True):
    ...
    imgs = [TF.resize(img, [w, h], interpolation=3) for img in imgs]
...
class Frame(object):
    ...
    def __init__(self, image_path, data_format, dtype, target_shape):
        ...
        assert self.c in [1, 3], "Number of channels should be 1 or 3. Got {}".format(
            self.c
        )
    ...
    def _load_img_re_identification(self):
        ...
        inference_input = preprocess_input(
            np.array(image).astype(np.float32).transpose(2, 0, 1), mode="torch"
        )
    ...
    def _load_img_foundationpose(self, depth_folder, depth_ext=".png"):
        ...
        if not os.path.exists(depth_path):
            raise NotImplementedError(
                f"Expecting the correct path of the depth file: {depth_path}"
            )
