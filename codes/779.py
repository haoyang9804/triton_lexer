...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "fp4_to_bf16_conversion", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(12, 28):
            size = 2**i
            x = torch.randint(0, 16, (size,), dtype=torch.uint8)
            s_e8m0 = torch.randint(0, 256, (size // 32,), dtype=torch.uint8)
