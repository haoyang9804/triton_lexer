...
def batched_vecmat(M, N, K, block_m, block_n, block_k, num_warps=4, num_stages=1):

    A = torch.randn(M, K, device="cuda", dtype=torch.float32)
    B = torch.randn(M, N, K, device="cuda", dtype=torch.float32)
...
def test_vecmat():
    ...
    output2 = batched_vecmat(
        M, N, K, block_m, block_n, block_k, num_warps=2, num_stages=2
    )
    results["test_case_2"] = output2.clone()
