...
def check_memory(new_cnt=-1):
    ...
    print(
        f"[{cnt}] mem_alloc: {mem_alloc:.4f}, mem_reserve: {mem_reserve:.4f}, mem_peak: {mem_peak:.4f}"
    )
    cnt += 1
    return
...
class MyTestCase(unittest.TestCase):
    def test_forward_attention(self):
        ...
        query = torch.randn(
            (batch_size, head_size, seq_len, dim), device="cuda", dtype=dtype
        )
        key = torch.randn(
            (batch_size, head_size, seq_len, dim), device="cuda", dtype=dtype
        )
        ...
        q_buckets_idx = torch.randint(
            0, 64, (batch_size, head_size, seq_len), device="cuda"
        )
        k_buckets_idx = torch.randint(
            0, 64, (batch_size, head_size, seq_len), device="cuda"
        )
        ...
        attn_block, lse_block = flash_attn_func(
            query_split_per_block.transpose(1, 2),
            key_split_per_block.transpose(1, 2),
            value_split_per_block.transpose(1, 2),
        )

        attn_sample, lse_sample = flash_attn_func(
            query.transpose(1, 2),
        ...
        attn, lse = add_self_attentions(
            flash_attn_block, lse_block, attn_sample, lse_sample.unsqueeze(-1)
        )
        lse = lse.squeeze(-1)
        t1 = time.time()
        check_memory()
        print(
            "the runtime of flash attention with permutation and indexing of queries:",
            t1 - t0,
        )
        ...
        lse_block_torch = lse_block_torch.view(
        ...
        lse_torch = (
            (lse_sample_torch.exp() + lse_block_torch.exp())
            .log()
            .to(dtype=lse_block_torch.dtype)
        )
        print(
            "diff between lse with sample and without: ",
            (lse_block_torch - lse_torch).norm(),
            lse_torch.norm(),
        )
        ...
        print(
            "diff lse hyper_attention and flash with indexing and permutation: ",
            (lse - lse_hyper).norm(),
            lse.norm(),
        )

        print(
            "error hyper attention lse: ",
            (lse_hyper - lse_torch).norm(),
            lse_torch.norm(),
        )
    ...
    def test_gradient(self):
        ...
        attn_torch, lse_torch = add_self_attentions(
            attn_torch_block, lse_torch_block, attn_torch_sample, lse_torch_sample
        )
        ...
        print(
            "difference of torch attention and flash attn: ",
            (attn_torch - attn).norm(),
            attn_torch.norm(),
        )
        print(
            "difference of torch lse and hyper_attention lse: ",
            (lse_torch - hyper_lse).norm(),
            lse_torch.norm(),
        )
        ...
        print(
            (q_grad_torch - q_grad_hyper).norm(),
            q_grad_torch.norm(),
            q_grad_hyper.norm(),
        )
        ...
        print(
            (k_grad_torch - k_grad_hyper).norm(),
            k_grad_torch.norm(),
            k_grad_hyper.norm(),
        )
        ...
        print(
            (v_grad_torch - v_grad_hyper).norm(),
            v_grad_torch.norm(),
            v_grad_hyper.norm(),
        )
