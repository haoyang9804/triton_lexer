...
def parallel_path_bwd_dq_fn(
    q, k, v, g_cumsum, do, dg_cumsum, hc_whole, scale, L, D, cu_seqlens, S, BT, BS
):
    B, T, HQ, K = q.shape
    V = v.shape[-1]
    H = k.shape[-2]
    ...
    split_offsets = (
        prepare_chunk_offsets(cu_seqlens, S) if cu_seqlens is not None else None
    )
    ...
    dhc_whole = torch.zeros(
        hc_whole.shape[0], HQ, K, K, dtype=torch.float32, device=q.device
    )
