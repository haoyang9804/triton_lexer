...
class FoundationposeModel(TritonModel):
    ...
    def parse_model(model_metadata, model_config):
        ...
        if (
            model_metadata["name"] == "foundationpose_refiner_tao"
            and len(model_metadata.outputs) != 2
        ):
            raise Exception(
                "expecting 2 output, got {}".format(len(model_metadata.outputs))
            )
        if (
            model_metadata["name"] == "foundationpose_scorer_tao"
            and len(model_metadata.outputs) != 1
        ):
            raise Exception(
    ...
    @classmethod
    def from_metadata(
        cls, modelA_metadata, modelA_config, modelB_metadata, modelB_config
    ):
    ...
    def refiner_predict(
        ...
        for _ in range(iteration):
            ...
            for i in range(len(input_names)):
                inputs = [
                    client.InferInput(
                        input_names[i], batched_image_data[i].shape, triton_dtype[i]
                    )
                ]
    ...
    def find_best_among_pairs(self, pose_data: BatchPoseData, model_metadata):
        ...
        outputs = [client.InferRequestedOutput(out_name) for out_name in output_names]

        response = self.triton_client.infer(
            "foundationpose_scorer_tao", inputs, request_id=str(1), outputs=outputs
        )
    ...
    def scorer_predict(
        ...
        pose_data = make_crop_data_batch_score(
            (160, 160),
            ob_in_cams,
            mesh,
            rgb,
    ...
    def register(
        ...
        poses = self.refiner_predict(
            ...
            mesh_diameter=mesh_diameter,
            iteration=iteration,
            model_metadata=modelA_metadata,
        )
        ...
        scores = self.scorer_predict(
            ...
            mesh_tensors=mesh_tensor,
            mesh_diameter=mesh_diameter,
            model_metadata=modelB_metadata,
        )
    ...
    def track_one(
        ...
        return (
            pose @ get_tf_to_centered_mesh(mesh_model_center)
        ).data.cpu().numpy().reshape(4, 4), pose

    def draw_image(self, color, pose, to_origin, extent_bbox):
        center_pose = np.array(pose) @ np.linalg.inv(np.array(to_origin))
        vis = draw_posed_3d_box(
            self.K, img=color, ob_in_cam=center_pose, bbox=extent_bbox
        )
        vis = draw_xyz_axis(
            color,
