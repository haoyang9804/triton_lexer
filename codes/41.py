...
if "USE_TRITON_DISTRIBUTED_AOT" in os.environ and os.environ[
    "USE_TRITON_DISTRIBUTED_AOT"
] in ["1", "true", "on", "ON", "On", True]:
    use_aot = True
else:
    use_aot = False
...
split_kv_signature = (
    (
        "*{input_dtype}:16, *{cache_dtype}:16, *{cache_dtype}:16, *{output_dtype}:16, "
        "fp32, "
        "*i32:16, *i32, "
        "i32, "
    )
...
def get_triton_split_kv_algo_info(
    ...
    return {
        ...
        "BLOCK_DPE": q_head_dim - 2 ** int(math.log2(q_head_dim)),
...
@aot_compile_spaces(
    {
        "gqa_fwd_batch_decode_split_kv_fp16_fp16_fp32": {
            "signature": split_kv_signature.format(
                input_dtype="fp16", cache_dtype="fp16", output_dtype="fp32"
            ),
            "grid": _split_kv_grid,
            "triton_algo_infos": [
                get_triton_split_kv_algo_info(
                    96, 12, 128, 128, 1, split_kv=32, soft_cap=0
                ),
                get_triton_split_kv_algo_info(
                    96 // 4, 12 // 4, 128, 128, 1, split_kv=32, soft_cap=0
                ),
            ],
        },
        "gqa_fwd_batch_decode_split_kv_fp16_fp16_fp16": {
            "signature": split_kv_signature.format(
                input_dtype="fp16", cache_dtype="fp16", output_dtype="fp16"
            ),
            "grid": _split_kv_grid,
            "triton_algo_infos": [
                get_triton_split_kv_algo_info(
...
def kernel_gqa_fwd_batch_decode_split_kv(
    ...
    if BLOCK_DPE > 0:
        ...
        offs_qpe = (
            bid * stride_q_bs + cur_head[:, None] * stride_q_h + offs_dpe[:, None]
        )
        qpe = tl.load(
            q_ptr + offs_qpe, mask=mask_h[:, None] & mask_dpe[None, :], other=0.0
        )
    ...
    for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
        ...
        kv_page_number = tl.load(
            block_table_ptr + bid * stride_table_bs + offs_n // PAGE_SIZE,
            mask=offs_n < split_kv_end,
            other=0,
        )
        kv_loc = kv_page_number * PAGE_SIZE + offs_n % PAGE_SIZE
        offs_cache_k = (
            kv_loc[None, :] * stride_k_cache_bs
            + kv_hid * stride_k_cache_h
            + offs_d[:, None]
        )
        k = tl.load(
            k_cache_ptr + offs_cache_k,
        ...
        offs_cache_v = (
            kv_loc[:, None] * stride_v_cache_bs
            + kv_hid * stride_v_cache_h
            + offs_dv[None, :]
        )
        v = tl.load(
            v_cache_ptr + offs_cache_v,
    ...
    offs_log = (
        bid * stride_o_bs + cur_head * stride_o_h + split_kv_id * stride_o_split + V_DIM
    )
    tl.store(output_ptr + offs_log, e_max + tl.log(e_sum), mask=mask_h)


combine_kv_signature = (
    ("*{input_dtype}:16, *{output_dtype}:16, " "*i32, ")
    + (
        ", ".join(
            [
                "i32",
...
def get_triton_combine_kv_algo_info(split_kv, v_head_dim, block_dv=None):
    return {
        "NUM_KV_SPLITS": split_kv,
        "BLOCK_DV": (
            triton.next_power_of_2(v_head_dim) if block_dv is None else block_dv
        ),
        "Lv": v_head_dim,
        "num_warps": 4,
        "num_stages": 2,
...
@aot_compile_spaces(
    {
        "gqa_fwd_batch_decode_combine_kv_fp32_fp16": {
            "signature": combine_kv_signature.format(
                input_dtype="fp32", output_dtype="fp16"
            ),
            "grid": _combine_kv_grid,
            "triton_algo_infos": [
...
@aot_compile_spaces(
    {
        "intra_rank_gqa_fwd_batch_decode_combine_kv_fp32_fp16": {
            "signature": combine_kv_signature_intra_rank.format(
                input_dtype="fp32", output_dtype="fp16"
            ),
            "grid": _combine_kv_grid,
            "triton_algo_infos": [
                ...
                get_triton_combine_kv_algo_info(
                    split_kv=8, v_head_dim=128, block_dv=1024
                ),
                get_triton_combine_kv_algo_info(
                    split_kv=16, v_head_dim=128, block_dv=1024
                ),
                get_triton_combine_kv_algo_info(
                    split_kv=32, v_head_dim=128, block_dv=1024
                ),
                get_triton_combine_kv_algo_info(
                    split_kv=64, v_head_dim=128, block_dv=1024
                ),
            ],
        },
...
@aot_compile_spaces(
    {
        "inter_rank_gqa_fwd_batch_decode_combine_kv_fp32_fp16": {
            "signature": combine_kv_signature_inter_rank.format(
                input_dtype="fp32", output_dtype="fp16"
            ),
            "grid": _combine_kv_grid,
            "triton_algo_infos": [
...
persistent_signature = (
    (
        "*{input_dtype}:16, *{cache_dtype}:16, *{cache_dtype}:16, *{output_dtype}:16, *{output_dtype}:16, "
        "fp32, "
        "*i32:16, *i32, *i32:16, "
        "i32, "
    )
...
def get_triton_persistent_algo_info(
    q_heads,
    kv_heads,
    q_head_dim,
    v_head_dim,
...
@aot_compile_spaces(
    {
        "gqa_fwd_batch_decode_split_kv_persistent_fp16_fp16_fp16": {
            "signature": persistent_signature.format(
                input_dtype="fp16", cache_dtype="fp16", output_dtype="fp16"
            ),
            "grid": _persistent_grid,
            "triton_algo_infos": [
                get_triton_persistent_algo_info(
                    96, 12, 128, 128, 1, split_kv=32, soft_cap=0, max_kv_len=8192
                ),
            ],
        }
    }
...
def kernel_gqa_fwd_batch_decode_split_kv_persistent(
    ...
    for tile_id in range(sm_id, num_tiles, num_sms):
        ...
        offs_q = (
            bid * stride_q_bs + cur_head[:, None] * stride_q_h + offs_d[None, :] * 1
        )
        q = tl.load(
            q_ptr + offs_q, mask=(mask_h[:, None]) & (mask_d[None, :]), other=0.0
        )
        ...
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            offs_n = start_n + tl.arange(0, BLOCK_N)
            kv_page_number = tl.load(
                block_table_ptr + bid * stride_table_bs + offs_n // PAGE_SIZE * 1,
                mask=offs_n < split_kv_end,
                other=0,
            )
            kv_loc = kv_page_number * PAGE_SIZE + offs_n % PAGE_SIZE
    ...
    if sm_id < batch * q_head_num:
        ...
        for split_kv_id in range(0, NUM_KV_SPLITS):
            ...
            if split_kv_end > split_kv_start:
                tv = tl.load(
                    output_ptr + offs_v + split_kv_id * stride_o_split,
                    mask=mask_d,
                    other=0.0,
                )
        ...
        tl.store(
            final_output_ptr
            + cur_batch * stride_final_o_bs
            + cur_head * stride_final_o_h
            + offs_d,
            acc / e_sum,
            mask=mask_d,
        )
...
def gqa_fwd_batch_decode(
    ...
    assert (
        ...
        and kv_heads == v_cache.shape[2]
        and k_head_dim == q_head_dim
    )
    ...
    grid_split_kv = (
        batch,
        triton.cdiv(q_heads, min(BLOCK_H, kv_group_num)),
        NUM_KV_SPLITS,
    )

    output_split = (
        torch.empty(
            [batch, q_heads, NUM_KV_SPLITS, v_head_dim + 1],
            dtype=torch.float16,
            device=q.device,
        )
    ...
    output_combine = (
        torch.empty([batch, q_heads, v_head_dim], dtype=torch.float16, device=q.device)
...
def gqa_fwd_batch_decode_intra_rank(
    q,
    k_cache,
    v_cache,
    workspace,
...
def gqa_fwd_batch_decode_persistent(
    q,
    k_cache,
    v_cache,
    workspace,
...
def gqa_fwd_batch_decode_aot(
    ...
    if use_aot:
        ...
        output_combine = (
            torch.empty(
                [batch, q_heads, v_head_dim], dtype=torch.float16, device=q.device
            )
            if output_combine is None
            else output_combine
        )
        ...
        if output_split.dtype == torch.float32:
            ...
            split_algo_info = (
                distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp32__triton_algo_info_t()
            )
            combine_algo_info = (
                distributed.gqa_fwd_batch_decode_combine_kv_fp32_fp16__triton_algo_info_t()
            )
            ...
            split_algo_info = (
                distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp16__triton_algo_info_t()
            )
            combine_algo_info = (
                distributed.gqa_fwd_batch_decode_combine_kv_fp16_fp16__triton_algo_info_t()
            )
        else:
            raise RuntimeError(
                "Unsupported data type of intermediate output:", output_split.dtype
            )

        py_split_algo_info = get_triton_split_kv_algo_info(
            q_heads,
            kv_heads,
            q_head_dim,
            v_head_dim,
        ...
        py_combine_algo_info = get_triton_combine_kv_algo_info(
            split_kv=NUM_KV_SPLITS, v_head_dim=v_head_dim
        )
...
def gqa_fwd_batch_decode_intra_rank_aot(
    stream,
    q,
    k_cache,
    v_cache,
    ...
    if use_aot:
        ...
        output_combine = (
            torch.empty(
                [batch, q_heads, v_head_dim + 1], dtype=torch.float16, device=q.device
            )
            if output_combine is None
            else output_combine
        )
        ...
        if output_split.dtype == torch.float32:
            kernel_split = distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp32
            kernel_combine = (
                distributed.intra_rank_gqa_fwd_batch_decode_combine_kv_fp32_fp16
            )
            split_algo_info = (
                distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp32__triton_algo_info_t()
            )
            combine_algo_info = (
                distributed.intra_rank_gqa_fwd_batch_decode_combine_kv_fp32_fp16__triton_algo_info_t()
            )
        elif output_split.dtype == torch.float16:
            kernel_split = distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp16
            kernel_combine = (
                distributed.intra_rank_gqa_fwd_batch_decode_combine_kv_fp16_fp16
            )
            split_algo_info = (
                distributed.gqa_fwd_batch_decode_split_kv_fp16_fp16_fp16__triton_algo_info_t()
            )
            combine_algo_info = (
                distributed.intra_rank_gqa_fwd_batch_decode_combine_kv_fp16_fp16__triton_algo_info_t()
            )
        else:
            raise RuntimeError(
                "Unsupported data type of intermediate output:", output_split.dtype
...
def gqa_fwd_batch_decode_persistent_aot(
    stream,
    q,
    k_cache,
    v_cache,
    ...
    if use_aot:
        ...
        algo_info = (
            distributed.gqa_fwd_batch_decode_split_kv_persistent_fp16_fp16_fp16__triton_algo_info_t()
        )
        py_algo_info = get_triton_persistent_algo_info(
            q_heads,
            kv_heads,
            q_head_dim,
            v_head_dim,
