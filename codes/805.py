...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "kv_cache_filling", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 18):
            ...
            k_states = torch.rand(
                (batch_size, seq_len, num_heads, head_dim), dtype=torch.float16
            )
            v_states = torch.rand(
                (batch_size, seq_len, num_heads, head_dim_v), dtype=torch.float16
            )
            k_caches = torch.zeros(
                (batch_size, block_size, num_heads, head_dim), dtype=torch.float16
            )
            v_caches = torch.zeros(
                (batch_size, block_size, num_heads, head_dim_v), dtype=torch.float16
            )
            ...
            kv_seq_length = q_seq_length + torch.randint(
                0, seq_len // 2, (batch_size,), dtype=torch.int32
            )
            block_offsets = torch.randint(
                0, block_size, (batch_size, block_size), dtype=torch.int32
            )
