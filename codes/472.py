...
def matmul_triton(
    ...
    assert (
        A.shape[2] == B.shape[1]
    ), f"Dimensions are not compatible for matrix multiplication, provided: {A.shape}, {B.shape}"
    ...
    grid = lambda meta: (
        batch_size,
        triton.cdiv(seq_len, meta["bsy"]),
        triton.cdiv(dim_out, meta["bsx"]),
    )
...
if __name__ == "__main__":
    ...
    ), f"Data does not match, diff: {torch.max(torch.abs(y_pytorch-y_triton))}"
    ...
    def benchmark(batch_size, seq_len, din, dout, provider):
        ...
        if provider == "triton":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: matmul_triton(x, y), warmup=50, quantiles=quantiles
            )
        if provider == "torch":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: torch.matmul(x, y), warmup=50, quantiles=quantiles
            )

        def gbps(ms):
            return 2 * seq_len * din * batch_size * 1e-12 / (ms * 1e-3)

        return gbps(ms), gbps(max_ms), gbps(min_ms)

    benchmark.run(print_data=True, save_path="./benchmarks/matmul3/")
