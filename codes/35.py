...
def gemm_rs_intra_node_op(
    ...
    matmul_fuse_scatter(
        a, b, scatter_bufs_ptr, rank, num_ranks, transpose_weight=transpose_weight
    )
...
def create_gemm_rs_intra_node_context(
    max_M,
    N,
    output_dtype,
    rank,
    ...
    sync_bufs_ptr = torch.tensor(
        [t.data_ptr() for t in sync_bufs],
        device=torch.cuda.current_device(),
        requires_grad=False,
    )

    scatter_bufs = pyrocshmem.hipipc_create_tensor_list(
        tp_group, [max_M, N], output_dtype
    )
    scatter_bufs_ptr = torch.tensor(
    ...
    scatter_streams_pool = [torch.cuda.Stream(priority=-1) for i in range(num_ranks)]
