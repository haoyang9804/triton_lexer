...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "kcache_copy_triton", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        for i in range(2, 11):
            ...
            block_tables = torch.randint(
                0, block_size, (bsz, block_size), dtype=torch.int32
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        total_bytes = (
            k.numel() * k.element_size() + k_cache.numel() * k_cache.element_size()
        )
