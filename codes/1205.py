...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        if visual_model_path is not None:
            ...
            ), f"LLaVA-OneVsion is not support multi image inference currently."
            ...
            if self.model_type == "mllama" or self.model_type == "llava_onevision":
                self.vision_preprocessor = VisionPreProcessor(
                    self.model_type,
                    AutoProcessor.from_pretrained(tokenizer_dir),
                    model_config,
                )
    ...
    def execute(self, requests):
        ...
        for idx, request in enumerate(requests):
            ...
            if prompt_table_extra_id is not None:
                ...
                assert (
                    prompt_table_extra_id.shape[0] == batch_size
                ), "Prompt table extra id must have the same batch size as Query"
                assert (
                    prompt_table_extra_id.shape[1] == 1
            ...
            if self.is_multimodal and (img_urls or image_bytes or video_bytes):
                ...
                assert (
                    self.model_type != "llava_onevision"
    ...
    def _create_request(self, query, visual_tokens=None):
        ...
        if self.is_multimodal:
            ...
            if self.max_num_images and self.max_num_images > 1:
                ...
                start_ids = self._setup_fake_prompts(
                    query.shape[0], batch_split_prompts
                )
            elif self.model_type == "llava_onevision":
                fake_prompt_ids = []
                extra_id = np.array(
                ...
                start_ids = [
                    np.concatenate(
                        (pre_prompt_id, prompt_id, extra_id, ids, post_prompt_id),
                        axis=0,
                    )
                ...
                start_ids = [
                    np.concatenate(
                        (pre_prompt_id, fake_prompt_id, ids, post_prompt_id), axis=0
                    )
                    for ids in start_ids
                ]
...
class VisionPreProcessor:
    ...
    def __init__(
        self, vision_model_type, vision_model_processor, preprocessor_model_config={}
    ):
        ...
        self.load_images_tensor = lambda tensor: (
            tensor
    ...
    def mllama_process(self, queries, img_urls=None, image_bytes=None):
        ...
        if img_urls is not None or image_bytes is not None:
            if img_urls is not None:
                ...
                images = [
                    self.load_images_from_urls(urls) for urls in img_urls.as_numpy()
                ]
            else:
                images = [
                    img
                    for img_list in self.load_images_tensor(image_bytes)
                    for img in img_list
                ]
    ...
    def llava_onevision_process_image(self, queries, img_urls=None, image_bytes=None):
        ...
        ), f"Image must have the same batch size as Query."
        preprocessor_outputs = {}
        ...
        preprocessor_outputs["PIXEL_VALUES"] = [
            torch.nn.functional.pad(
                image,
                (0, 0, 0, 0, 0, 0, 0, max_patch - image.shape[1], 0, 0),
                mode="constant",
            )
    ...
    def llava_onevision_process_video(self, queries, video_bytes=None):
        ...
        assert (
            len(queries) == batch_size
        ), f"Video must have the same batch size as Query."
        preprocessor_outputs = {}
        preprocessor_outputs["PIXEL_VALUES"] = []
