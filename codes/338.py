...
def benchmark(T, provider):
    ...
    if provider.startswith("abc"):
        s = torch.randn(
            B, H, T, M, device=device, requires_grad=requires_grad, dtype=dtype
        )
    ...
    if provider == "abc":
        results = triton.testing.do_bench(
            lambda: chunk_abc(q, k, v, s), quantiles=quantiles
        )
    elif provider == "gla":
        results = triton.testing.do_bench(
        ...
        results = triton.testing.do_bench(
            lambda: chunk_abc(q, k, v, s)[0].backward(do), quantiles=quantiles
        )
