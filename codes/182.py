...
def early_config_prune(configs, named_args, **kwargs):
    ...
    N, H, V = (
        named_args["x"].shape[0],
        named_args["x"].shape[1],
        named_args["logits"].shape[1],
    )
    pruned_configs = []
    for config in configs:
        accept = True
        accept = accept and (
            config.kwargs["SPLIT_NV"] * config.kwargs["V_BLOCK_SIZE"] < V
        )
        accept = accept and (
            config.kwargs["SPLIT_NV"] * config.kwargs["N_BLOCK_SIZE"] < N
        )
