...
def _inner_function(
    ...
    loss = (
        F.cross_entropy(logits, y_block.view(-1), ignore_index=ignore_index)
        / num_blocks
    )
    ...
    logit_ent = (-probs * logits.log_softmax(dim=-1)[y_block != ignore_index]).sum(
        dim=-1
    ).mean() / num_blocks
    return (
        loss,
...
def reference_torch_checkpoint(
    x,
    y,
    A,
    ...
    loss, z_reg, lmax, lent, lnorm = (
        0.0,
        0.0,
        torch.tensor(-10e5, dtype=x.dtype),
        0.0,
        0.0,
    )
...
def early_config_prune(configs, named_args, **kwargs):
    ...
    N, H, V = (
        named_args["x_ptr"].shape[0],
        named_args["x_ptr"].shape[1],
        named_args["A_t_ptr"].shape[1],
    )
    pruned_configs = []
    ...
    if dtype not in [torch.float16, torch.float32]:
        configs = [
            config
            for config in configs
            if (config.kwargs.get("SPLIT_N", 1) == 1)
            or (config.kwargs.get("SPLIT_N", 1) == 1)
        ]
...
fwd_configs = [
    triton.Config(
        {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 1},
        num_warps=8,
        num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 32},
        num_warps=4,
        num_stages=3,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 16},
        num_warps=8,
        num_stages=3,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 1},
        num_warps=4,
        num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 16},
        num_warps=8,
        num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 1},
        num_warps=4,
        num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 16},
        num_warps=4,
        num_stages=3,
    ),
    triton.Config(
    ...
    triton.Config(
        {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 32},
        num_warps=8,
        num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 128, "H_BLOCK_SIZE": 64, "GROUP_SIZE": 1},
        num_warps=8,
        num_stages=2,
    ),
    triton.Config(
...
def linear_xent_bwd_kernel_matmul_t_epilogue_dx(
    ...
    if SPLIT_V == 1:
        ...
        tl.store(
            x_grad_block_ptr,
            (x_grad_acc / R / logit_scale).to(x_grad_ptr.type.element_ty),
        )
...
def linear_xent_bwd_kernel_matmul_t_epilogue_dA(
    ...
    for _ in range(0, tl.cdiv(N_group, N_BLOCK_SIZE * SPLIT_N)):
        ...
        if monitoring:
            logit_entropy += tl.sum(
                tl.where(y == ignore_index, 0.0, tl.sum(-softmax_z * logprobs, axis=1))
            )
        if z_regularization > 0:
            softmax_z += 2.0 * z_regularization * lse[:, None] * softmax_z
        z_grad = softmax_z - tl.where(mask, 1.0, 0.0)
    ...
    def forward(
        ...
        with torch.cuda.device(x.device.index):
            ...
            with torch.no_grad():
                ...
                x_grad = torch.zeros_like(
                    x, dtype=torch.float32 if At.dtype == torch.bfloat16 else At.dtype
                )
                ...
                logit_ent_local = torch.zeros(
                    V // 128, H // 64, dtype=torch.float32, device=x.device
                )
                logit_norm_local = torch.zeros(
                    N_group, V // 128, dtype=torch.float32, device=x.device
                )
                ...
                for idx_N_group in range(math.ceil(N / N_group)):
                    ...
                    if z_regularization > 0:
                        lse_sum += (
                            lse_global.sum() + z_regularization * z_reg_block
                        ) / reduction
                    else:
...
if __name__ == "__main__":
    ...
    loss, z_reg_ref, logit_max_ref, logit_ent_ref, logit_norm_ref = (
        reference_torch_checkpoint(
            x.float(),
            y,
            A.float(),
            ignore_index=ignore_index,
            z_regularization=z_reg_const,
            logit_scale=logit_scale,
        )
    )
    loss.backward()
    ...
    print(
        torch.dist(z_reg, z_reg_ref),
        torch.dist(lmax, logit_max_ref),
        torch.dist(lent, logit_ent_ref),
        torch.dist(lnorm, logit_norm_ref),
    )
