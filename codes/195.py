...
from tao_triton.python.postprocessing.classification_postprocessor import (
...
from tao_triton.python.postprocessing.visual_changenet_postprocessor import (
    VisualChangeNetPostprocessor,
)
from tao_triton.python.postprocessing.visual_changenet_postprocessor import (
    de_norm,
    make_numpy_grid,
)
from tao_triton.python.postprocessing.peoplesegnet_postprocessor import (
    PeoplesegnetPostprocessor,
)
from tao_triton.python.postprocessing.retinanet_postprocessor import (
    RetinanetPostprocessor,
)
from tao_triton.python.postprocessing.multitask_classification_postprocessor import (
    MultitaskClassificationPostprocessor,
)
from tao_triton.python.postprocessing.pose_classification_postprocessor import (
...
from tao_triton.python.postprocessing.centerpose_postprocessor import (
    CenterPosePostprocessor,
)
from tao_triton.python.postprocessing.foundationpose_postprocessor import (
    FoundationposePostprocessor,
)
...
from tao_triton.python.model.multitask_classification_model import (
...
def vis_inputs(input_array, output_path, id_):
    ...
    file_name = os.path.join(output_path, "img_" + str(id_) + ".jpg")
...
def parse_command_line(args=None):
    ...
    parser.add_argument(
        "--obj_file", type=str, default="", help="Path to CAD model with OBJ format."
    )
    parser.add_argument(
        "--intrinsic_file",
        type=str,
...
def main():
    ...
    if FLAGS.mode.lower() == "foundationpose":
        ...
        triton_model = TRITON_MODEL_DICT[FLAGS.mode.lower()].from_metadata(
            modelA_metadata, modelA_config, modelB_metadata, modelB_config
        )
    ...
    if FLAGS.mode.lower() == "pose_classification":
        ...
        if os.path.splitext(FLAGS.image_filename)[-1] == ".json":
            ...
            pose_sequences, action_data = pose_cls_dataset_convert(
                FLAGS.image_filename, FLAGS.dataset_convert_config
            )
        ...
        file_list_dir_rgb = os.path.join(FLAGS.image_filename, FLAGS.img_dirs[0])
        file_list_dir_depth = os.path.join(FLAGS.image_filename, FLAGS.img_dirs[1])
        if os.path.exists(file_list_dir_rgb) and os.path.exists(file_list_dir_depth):
            ...
            frames = [
                ...
                for f in img_name
                if os.path.isfile(os.path.join(file_list_dir_rgb, f))
        ...
        if os.path.isdir(FLAGS.image_filename):
            if FLAGS.mode.lower() == "visualchangenet":
                file_list_dir_a = os.path.join(FLAGS.image_filename, FLAGS.img_dirs[0])
                ...
                if os.path.exists(file_list_dir_a) and os.path.exists(file_list_dir_b):
                    ...
                    frames = [
                        ...
                        for f in img_name
                        if os.path.isfile(os.path.join(file_list_dir_a, f))
                        and os.path.splitext(f)[-1] in [".jpg", ".jpeg", ".png"]
                    ]
                else:
                    raise NotImplementedError(
    ...
    args_postprocessor = []
    if FLAGS.mode.lower() == "pose_classification":
        args_postprocessor = [FLAGS.batch_size, pose_sequences, FLAGS.output_path]
        ...
        args_postprocessor = [FLAGS.output_path]
    else:
        args_postprocessor = [
            FLAGS.batch_size,
            frames,
            ...
            if FLAGS.mode.lower() == "pose_classification":
                ...
                for idx in range(FLAGS.batch_size):
                    ...
                    if repeated_data is None:
                        ...
                        repeated_data = np.concatenate(
                            (repeated_data, pose_sequence), axis=0
                        )
                ...
                for idx in range(FLAGS.batch_size):
                    ...
                    if (
                        FLAGS.mode.lower() == "yolov3"
                        or FLAGS.mode.lower() == "retinanet"
                    ):
                        img = frame._load_img()
                        ...
                        repeated_image_data.append(
                            triton_model.preprocess(frame.as_numpy(img))
                        )
        ...
        while processed_request < sent_count:
            ...
            if os.path.splitext(FLAGS.image_filename)[-1] == ".json":
                ...
                postprocessor.apply(
                    ...
                    img_name=frames[
                        FLAGS.batch_size
                        * processed_request : FLAGS.batch_size
                        * (processed_request + 1)
                    ],
                )
            else:
        ...
        for idb in range(len(action_data)):
            for idf in range(len(action_data[idb]["batches"])):
                for idp in range(len(action_data[idb]["batches"][idf]["objects"])):
                    action_data[idb]["batches"][idf]["objects"][idp].pop(
                        "segment_id", None
                    )
        with open(output_file, "w") as outfile:
            json.dump(
                action_data, outfile, sort_keys=True, indent=2, ensure_ascii=False
            )
    logger.info("PASS")
