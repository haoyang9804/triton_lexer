    ...
    try:
        with grpcclient.InferenceServerClient(grpc_url) as client:
            if client.is_server_ready():
                raise RuntimeError(
                    f"Another tritonserver is already running on {grpc_url}"
                )
...
def send_triton_request(
    ...
    if not client.is_server_live():
        raise ValueError(
            "Client could not establish commuincation with Triton Inference Server."
        )

    if isinstance(inputs, TensorTable):
        triton_inputs = triton.convert_table_to_triton_input(
            schema, inputs, grpcclient.InferInput
        )
    else:
        triton_inputs = triton.convert_df_to_triton_input(
            schema, inputs, grpcclient.InferInput
        )
    ...
    response = client.infer(
        triton_model, triton_inputs, request_id=request_id, outputs=outputs
    )
