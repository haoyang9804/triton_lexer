...
def intra_chunk_preprocess_bwd_fn(
    q, k, w, beta, dq, dk, dh, dA_local, A, L, D, do, scale, cu_seqlens=None
):
    BT = A.shape[-1]
    HQ = q.shape[-2]
    B, T, H, K = k.shape
    ...
    dbeta = torch.empty(
        B, T, HQ, device=q.device, dtype=k.dtype if G == 1 else torch.float32
    )
    dw = torch.empty(
        B, T, HQ, K, device=q.device, dtype=k.dtype if G == 1 else torch.float32
    )
