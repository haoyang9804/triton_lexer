...
from TritonBench_v1.chunk_delta_fwd import chunk_fwd_h_fn
...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "chunk_delta_fwd", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 11):
            ...
            initial_state = (
                torch.rand(B, H, K, V, dtype=torch.float16) if i % 2 == 0 else None
            )
            final_state = (
                torch.rand(B, H, K, V, dtype=torch.float16) if i % 2 == 0 else None
            )
