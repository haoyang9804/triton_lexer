...
def benchmark(T, provider):
    ...
    if provider.startswith("naive_ln"):
        norm = nn.LayerNorm(D, elementwise_affine=True, bias=True).to(
            device=device, dtype=dtype
        )
        results = triton.testing.do_bench(lambda: norm(x), quantiles=quantiles)
    if provider.startswith("fused_ln"):
        norm = LayerNorm(D, elementwise_affine=True, bias=True).to(
            device=device, dtype=dtype
        )
        results = triton.testing.do_bench(lambda: norm(x), quantiles=quantiles)
    ...
    if provider.startswith("fused_gn"):
        norm = GroupNorm(4, D, elementwise_affine=True, bias=True).to(
            device=device, dtype=dtype
        )
        results = triton.testing.do_bench(lambda: norm(x), quantiles=quantiles)
    if provider.startswith("naive_ln_bwd"):
        ...
        results = triton.testing.do_bench(
            lambda: norm(x).backward(x), quantiles=quantiles
        )
