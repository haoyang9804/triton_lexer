...
from .kernels import matmul, softmax, add, matmul3, LayerNormTriton, Conv2DTriton
...
class LinearWithBias(nn.Module):
    def __init__(
        self, input_dim: int, output_dim: int, activation: Optional[str] = None
    ):
        super().__init__()
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_scores = matmul3(
            q, k, apply_scaling=True, scale_factor=1 / math.sqrt(self.d_out)
        )
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        assert (
            d_in % num_heads == 0
        ), f"Input dimension should be equally divided amongst all heads. d_in%num_heads needs to be 0. Current: {d_in%num_heads}"
        assert (
            d_in / num_heads == d_out
        ), f"`d_out` is not equal to `d_in/num_heads`. Current: {d_in/num_heads}, {d_out}"
        ...
        self.attention = nn.ModuleList(
            [SelfAttention(d_in=d_in, d_out=d_out) for _ in range(self.num_heads)]
        )
...
class Transformer(nn.Module):
    def __init__(self, num_heads: int, d_in: int, d_out: int):
        super().__init__()
        self.num_heads = num_heads
        self.d_in = d_in
        self.d_out = d_out
...
class Encoder(nn.Module):
    def __init__(self, num_layers: int, num_heads: int, hidden_dim: int, d_out: int):
        ...
        self.layer = nn.ModuleList(
            Transformer(num_heads=self.num_heads, d_in=self.hidden_dim, d_out=d_out)
...
class VIT(nn.Module):
    def __init__(
        ...
        ), f"Hidden dimension should be divisible by number of heads, provided: {self.hidden_dim} {self.num_heads}"
    ...
    def forward(self, x):
        assert x.shape[1:] == (
            ...
            self.height,
            self.width,
        ), f"Image size {x.shape[1:]} not matching with the model input size: {3, self.height, self.width}"
...
if __name__ == "__main__":
    ...
    model = transfer_pretrained_weights(
        pretrained_model=pretrained_model, custom_model=model
    )
    ...
    def benchmark(batch_size, model1, model2, provider):
        ...
        logger.info(
            f"Benchmarking for batch size: {batch_size} and provider: {provider}"
        )

        if provider == "triton":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: model1(inp), quantiles=quantiles
            )
        if provider == "hf":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: model2(inp), quantiles=quantiles
            )

        return ms, min_ms, max_ms

    benchmark.run(print_data=True, save_path="./benchmarks/model/")
