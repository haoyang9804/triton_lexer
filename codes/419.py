...
def rwkv_mix_fwd(x, x_prev, x_k):
    ...
    def grid(meta):
        return ((total_elements + meta["BLOCK_SIZE"] - 1) // meta["BLOCK_SIZE"], 1, 1)
...
def rwkv_relu_and_square_fwd(x: torch.Tensor, inplace: bool = True):
    ...
    def grid(meta):
        return ((output.numel() + meta["BLOCK_SIZE"] - 1) // meta["BLOCK_SIZE"], 1, 1)
...
def rwkv_channel_mixing_bwd(
    grad_output, x, x_prev, x_k, key_weight, value_weight, k1, k1_K, k, inplace=True
):
    ...
    def grid(meta):
        return (
            (batch_size * seq_len * n_embd + meta["BLOCK_SIZE"] - 1)
            // meta["BLOCK_SIZE"],
            1,
            1,
        )
...
def channel_mixing_rwkv7(
    ...
    return (
        Rwkv7ChannelMixing.apply(x, x_prev, x_k, key_weight, value_weight, inplace),
        x[-1, :],
    )
