...
def add_kernel(
    ...
    data_offset = (
        row_offset[:, None] * input_row_stride + col_offset[None, :] * input_col_stride
    )
...
def add_triton(
    ...
    assert (
        input1.is_contiguous() and input2.is_contiguous()
    ), "Input matrix needs to be contiguous"
    assert (
        len(input1.shape) == 3
    ), f"Only 3 dimensional input shapes are supported, provided: {input1.shape}"
    assert (
        input1.shape == input2.shape
    ), f"Input shapes need to be same, provided {input1.shape}, {input2.shape}"
    ...
    grid = lambda meta: (
        B,
        triton.cdiv(N, meta["bs_row"]),
        triton.cdiv(D, meta["bs_col"]),
    )
...
if __name__ == "__main__":
    ...
    benchmark.run(print_data=True, save_path="./benchmarks/add/")
