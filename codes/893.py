...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "triton_conv2d_fwd", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        output_tensor_size = (
            self.call_op(input_tensor).numel() * input_tensor[0].element_size()
        )
    ...
    def get_tflops(self, input_tensor, runtime):
        ...
        out_height = (in_height + 2 * input_tensor[6] - kernel_height) // input_tensor[
            4
        ] + 1
        out_width = (in_width + 2 * input_tensor[7] - kernel_width) // input_tensor[
            5
        ] + 1
        FLOPS = (
            2
