...
def full_moe_align_block_size(
    ...
    sorted_ids = torch.empty(
        (
            (num_tokens_per_rank * topk_ids.shape[1] + num_experts * (block_size - 1))
            * num_ranks,
        ),
        dtype=torch.int32,
    ...
    for iter in range(num_iterations):
        ...
        for i in range(1, num_experts + 1):
            cumsum[i] = (
                cumsum[i - 1]
                + ceil_div(token_cnts[num_experts, i - 1], block_size) * block_size
            )
        ...
        for j in range(num_experts):
            ...
            for i in range(start_idx, min(numel, start_idx + tokens_per_thread)):
                ...
                sorted_ids[sorted_ids_idx + rank_post_pad] = (
                    i + iter * num_tokens_per_iteration
                )
...
def precompute_context_helper(
    ...
    (ctx.full_topk_ids, ctx.full_topk_weight) = select_experts(
        pg, num_ranks, topk, input_dtype, device, router_logits
    )
    ...
    ) = full_moe_align_block_size(
        ctx.full_topk_ids, BLOCK_M, E, num_ranks, num_tokens_per_rank
    )
    EM = full_num_tokens_post_padded_list.cpu().tolist()[0]
...
def create_moe_rs_context(
    ...
    rs_buffers: List[torch.Tensor] = pynvshmem.nvshmem_create_tensor_list_intra_node(
        [max_token_num, hidden_dim], input_dtype
    )
    rs_buffer_ptrs: torch.Tensor = torch.tensor(
        [t.data_ptr() for t in rs_buffers], device=device
    )
    ...
    GEMM_BLOCK_K = 32
    dataflow_config = DataflowConfig(
        GEMM_BLOCK_M, GEMM_BLOCK_N, GEMM_BLOCK_K, 8, 4, 4, RS_BLOCK_M, RS_BLOCK_N
    )
    ...
    with torch.device(torch.cuda.current_device()):
        ...
        barriers_gemm_scatter_counter_ptrs = torch.tensor(
            [ptr.data_ptr() for ptr in barriers_gemm_scatter_counter]
        ).cuda()
        ...
        barriers_gemm_scatter_ready_ptrs = torch.tensor(
            [ptr.data_ptr() for ptr in barriers_gemm_scatter_ready]
        ).cuda()
        ...
        rs_per_node_signal_buffer = pynvshmem.nvshmem_create_tensor(
            [world_size], torch.uint64
        )
    ...
    return MoEReduceRSContext(
        ...
        p2p_stream,
        dataflow_config,
        barriers_gemm_scatter_counter,
        barriers_gemm_scatter_counter_ptrs,
...
def kernel_producer_group_gemm_tp_scatter_input(
    ...
    c_ptrs = (
        c_ptr + offs_token[:, None] * stride_out_m + offs_cn[None, :] * stride_out_n
    )
    ...
    remote_barrier_ready_ptr = tl.load(barriers_ready_ptrs + local_rank).to(
    ...
    if thread_id == 0:
        if atomic_add(counter_ptr, value, "gpu", "relaxed") == threshold - 1:
            dl.notify(
                remote_barrier_ready_ptr + offs_ready,
                rank,
                signal=1,
                sig_op="add",
                comm_scope="gpu",
    ...
    for m in range(num_block_per_pid_m):
        for n in range(num_block_per_pid_n):
            ...
            if use_tl_reduce:
                ...
                src_ptrs = (
                    consumer_output_ptr + offs_in_m[:, None] * N + offs_in_n[None, :]
                )
                token = dl.wait(
                    barrier_gemm_scatter_ready + to_rank, 1, "gpu", "acquire"
                )
                ...
                for i in range(REDUCE_LENGTH):
                    ...
                    offs_in_m = (
                        mid * BLOCK_M * REDUCE_LENGTH + offs_m_reduce * REDUCE_LENGTH
                    )
                    offs_in_n = nid * BLOCK_N + tl.arange(0, BLOCK_N)
                    src_ptrs = (
                        consumer_output_ptr
            ...
            dst_ptr = tl.load(remote_buffer_ptrs + to_rank_local).to(
                tl.pointer_type(dtype)
            )
            offs_out_m = (
                to_node * num_blocks_m_per_node
                + local_rank * num_blocks_m_per_rank
                + mid % num_blocks_m_per_rank
...
def kernel_consumer_reduce(
    ...
    for rid in range(0, local_world_size):
        ...
        full_offs_m = (
            pid_m * BLOCK_M + tl.arange(0, BLOCK_M) + swizzle_rid * m_per_rank
        ) % M
        offs_n = (pid_n * BLOCK_N + tl.arange(0, BLOCK_N)) % N
...
def topk_reduce_scatter_reduce_for_each_node(
    ...
    with torch.cuda.stream(reduction_stream):
        ...
        for n in range(0, nnodes):
            ...
            rs_buffer_cur_node = rs_buffers[local_rank][
                cur_node_id * M_per_node : (cur_node_id + 1) * M_per_node
            ]
            rs_per_node_buffer_cur_node = rs_per_node_buffer[
            ...
            set_signal(
                rs_per_node_signal_buf[cur_node_id].data_ptr(),
                1,
                reduction_stream,
                require_i64=True,
            )
...
def p2p_inter_node(
    ...
    with torch.cuda.stream(stream):
        ...
        output[M_per_rank * node_id : M_per_rank * (node_id + 1)].copy_(
            input[M_per_rank * node_id : M_per_rank * (node_id + 1)]
        )
...
def consumer_reduce_scatter_reduce_2d(
    ...
    barrier_all_on_stream(None, rs_stream)
    output = torch.empty(
        (M_per_rank, N), dtype=local_tensor.dtype, device=local_tensor.device
    )
    ring_reduce(
        p2p_result,
        output,
...
def moe_reduce_rs(
    ...
    dtype = a.dtype
    assert (
        dtype == torch.float16 or dtype == torch.float8_e4m3fn
    ), "Currently only used for float16 or float8_e4m3fn"
    ...
    with torch.cuda.stream(torch.cuda.current_stream()):
        ...
        full_num_tokens_post_padded_list = (
            ctx.precompute_ctx.full_num_tokens_post_padded_list
        )
