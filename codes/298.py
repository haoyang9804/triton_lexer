...
def get_arguments():
    ...
    parser.add_argument(
        "--mode", type=str, default="fwd+bwd", choices=["fwd", "bwd", "fwd+bwd"]
    )
    parser.add_argument(
        "--attn_method",
        type=str,
...
def get_tensors(batch_size, seq_len, head_size, dim):
    q = torch.randn(
        (batch_size, seq_len, head_size, dim),
...
def run_flash_attn(
    batch_size, head_size, seq_len, dim, causal, mode, impl="triton", warmup=20, rep=100
):
    ...
    if mode == "fwd":
        return triton.testing.do_bench(
            fn, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8]
        )
    elif mode == "bwd":
        o = fn()
        ...
        q20_fwd, median_fwd, q80_fwd = triton.testing.do_bench(
            fn, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8]
        )
        o = fn()
        do = torch.randn_like(o)
        fn = lambda: o.backward(do, retain_graph=True)
        q20_bwd, median_bwd, q80_bwd = triton.testing.do_bench(
            fn, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8]
        )
...
def run_hyper_attn(
    batch_size, head_size, seq_len, dim, causal, mode, smooth_block, warmup=20, rep=100
):
...
def main():
    ...
    print(
        f"mode: {mode}, attn_method: {attn_method}, batch_size: {batch_size}, head_size: {head_size}, dim: {dim}"
    )
    ...
    for seq_len in seq_lens:
        if attn_method == "flash":
            ms = run_flash_attn(
                batch_size, head_size, seq_len, dim, causal, mode=args.mode
            )
        elif attn_method == "flash-cuda":
            ms = run_flash_attn(
                batch_size, head_size, seq_len, dim, causal, mode=args.mode, impl="cuda"
            )
