...
def compare(x, y, extra_str=""):
    ...
    print(
        f"{extra_str}Max difference: {diff.max().item()}, Mean difference: {diff.mean().item()}"
    )
...
def test_grpo_loss(
    B, T, V, temperature, num_iteration, beta, eps_low, eps_high, dtype, atol, rtol
):
    _input = torch.randn(B, T + 1, V, device=device, dtype=dtype)
    ...
    ref_logp = (
        torch.randn(B, T, device=device, dtype=torch.float32) if beta != 0.0 else None
    )
    old_logp = (
        torch.randn(B, T, device=device, dtype=torch.float32)
        if num_iteration > 1
        else None
    )
