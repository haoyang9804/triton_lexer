...
def get_sampling_config_from_request(request, batch_size=1, batch_index=0):
    ...
    kwargs["top_k"] = get_input_scalar_by_name(
        request, "runtime_top_k", batch_size, batch_index
    )
    kwargs["top_p"] = get_input_scalar_by_name(
        request, "runtime_top_p", batch_size, batch_index
    )
    kwargs["top_p"] = (
    ...
    kwargs["min_length"] = get_input_scalar_by_name(
        request, "min_length", batch_size, batch_index
    )
    ...
    kwargs["num_return_sequences"] = (
        get_input_scalar_by_name(
            request, "num_return_sequences", batch_size, batch_index
        )
        or 1
    )
...
def get_external_draft_tokens_config_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    draft_input_ids = get_input_tensor_by_name(
        request, "draft_input_ids", batch_size, batch_index
    )
    if draft_input_ids is not None:
        kwargs["tokens"] = draft_input_ids[0].tolist()
    draft_logits = get_input_tensor_by_name(
        request, "draft_logits", batch_size, batch_index
    )
...
def get_prompt_tuning_config_from_request(
    request, batch_size=1, batch_index=0, input_length=0
):
    ...
    if prompt_embedding_table is not None:
        if isinstance(prompt_embedding_table, np.ndarray):
            kwargs["embedding_table"] = from_numpy(prompt_embedding_table).squeeze(
                dim=0
            )
...
def get_lora_config_from_request(request, batch_size=1, batch_index=0):
    kwargs = {}
    kwargs["task_id"] = get_input_scalar_by_name(
        request, "lora_task_id", batch_size, batch_index
    )
    lora_weights = get_input_tensor_by_name(
        request, "lora_weights", batch_size, batch_index
    )
    if lora_weights is not None:
        kwargs["weights"] = from_numpy(lora_weights).squeeze(dim=0)
    lora_config = get_input_tensor_by_name(
        request, "lora_config", batch_size, batch_index
    )
...
def get_guided_decoding_params_from_request(request, batch_size=1, batch_index=0):
    ...
    if guided_decoding_guide_type is not None:
        guided_decoding_guide_type = guided_decoding_guide_type.squeeze(axis=0)[
            0
        ].decode()
...
def convert_request(request, exclude_input_from_output, decoupled):
    ...
    for batch_index in range(0, batch_size):
        input_token_ids = get_input_tensor_by_name(
            request, "input_ids", batch_size, batch_index
        )[0]
        if input_token_ids is None:
            raise pb_utils.TritonModelException("A value is required for input_ids")
        ...
        input_length = get_input_scalar_by_name(
            request, "input_lengths", batch_size, batch_index
        )
        ...
        inputs["end_id"] = get_input_scalar_by_name(
            request, "end_id", batch_size, batch_index
        )
        inputs["pad_id"] = get_input_scalar_by_name(
            request, "pad_id", batch_size, batch_index
        )
        inputs["stop_words"] = convert_word_list(
            get_input_tensor_by_name(
                request, "stop_words_list", batch_size, batch_index
            )
        )
        ...
        if req_exclude_input_from_output is None:
            ...
            output_config.exclude_input_from_output = (
                ...
                if exclude_input_from_output is not None
                else False
            )
        else:
        ...
        if encoder_input_features is not None:
            if isinstance(encoder_input_features, np.ndarray):
                encoder_input_features = from_numpy(encoder_input_features).squeeze(
                    dim=0
                )
...
def convert_response(
    ...
    if batch_size > 1:
        output_tensors.append(
            pb_utils.Tensor(
                "batch_index", np.expand_dims(np.array([batch_index], np.int32), 0)
            )
        )
    ...
    if result.request_perf_metrics is not None:
        ...
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array([kv_cache_metrics.num_new_allocated_blocks], np.int32), 0
                ),
            )
        )
        output_tensors.append(
            pb_utils.Tensor(
                ...
                np.expand_dims(
                    np.array([kv_cache_metrics.num_reused_blocks], np.int32), 0
                ),
            )
        )
        output_tensors.append(
...
def convert_timestamp_to_seconds(timestamp: str):
    return int(
        datetime.datetime.strptime(timestamp, "%m-%d-%Y %H:%M:%S.%f").timestamp()
    )
...
class TritonPythonModel:
    ...
    def get_kv_cache_config(self, model_config):
        ...
        max_attention_window_size = get_parameter(
            model_config, "max_attention_window_size"
        )
    ...
    def get_parallel_config(self, model_config):
        ...
        if self.use_orchestrator_mode:
            ...
            spawn_processes = (
                os.environ.get("TRTLLM_ORCHESTRATOR_SPAWN_PROCESSES", "1") == "1"
            )
    ...
    def get_decoding_config(self, model_config):
        ...
        kwargs = {
            ...
            "eagle_config": (
                None if eagle_choices is None else trtllm.EagleConfig(eagle_choices)
            ),
    ...
    def get_extended_runtime_perf_knob_config(self, model_config):
        kwargs = {
            ...
            "cuda_graph_cache_size": get_parameter(
                model_config, "cuda_graph_cache_size", int
            ),
        }
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
    ...
    def get_guided_decoding_config(self, model_config):
        ...
        guided_decoding_backend = get_parameter(
            model_config, "guided_decoding_backend", str
        )
    ...
    def initialize(self, args):
        ...
        for output in model_config["output"]:
            if (
                output["name"] == "context_logits"
                or output["name"] == "generation_logits"
            ):
        ...
        with self.lock:
            ...
            for (
                ...
                if (
                    ...
                    and executor_request.streaming == False
                ):
        ...
        while self.running:
            for response in self.executor.await_responses(
                ...
                with self.lock:
                    self.req_id_to_request_data[
                        req_id
                    ].num_output_tokens += output_length
