...
@util.report(
    "leaky relu forward", ["x_size"], [128 * i for i in range(1, 21)], {"y_size": 64}
)
def bench_leaky_relu_forward(y_size, x_size, dtype, backend):
    ...
    if backend == "torch":
        return triton.testing.do_bench_cudagraph(
            lambda: torch.nn.functional.leaky_relu(input)
        )
    else:
        return triton.testing.do_bench_cudagraph(
            lambda: trident.function.leaky_relu(input)
        )
