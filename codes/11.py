...
class Yolov8_grpc:
    ...
    def _init_triton_client(self, url: str) -> None:
        try:
            ...
            logger.error(
                f"Triton channel creation failed: {str(e)}",
                extra={"error_type": "yolov8_grpc_error"},
            )
            sys.exit()
    ...
    def _predict(self, input_images: NDArray) -> torch.tensor:
        ...
        return torch.from_numpy(np.copy(results.as_numpy(self.output_name))).to(
            self.device
        )

    def _postprocess(
        self, preds: torch.tensor, origin_h: int, origin_w: int
    ) -> List[YOLOScores]:
        preds_list = non_max_suppression(
            preds, self.conf_thresh, self.iou_thresh, nc=self.nc
        )
        results = []
        for pred in preds_list:
            ...
            if self.task == "kpt":
                pred_kpts = (
                    pred[:, 6:].view(len(pred), *self.kpt_shape)
                    if len(pred)
                    else pred[:, 6:]
                )
