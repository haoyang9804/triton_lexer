...
def initialize_block_scaled(M, N, K, block_scale_type="nvfp4", compute_reference=False):
    ...
    epsilon = 1e-8
    a_scale = (
        torch.rand((M // 128, K // VEC_SIZE // 4, 32, 4, 4), device=device) + epsilon
    )
    b_scale = (
        torch.rand((N // 128, K // VEC_SIZE // 4, 32, 4, 4), device=device) + epsilon
    )
    ...
    if compute_reference:
        ...
        def unpack_scale(packed):
            ...
            return (
                packed.permute(0, 3, 2, 1, 4)
                .reshape(num_chunk_m * 128, num_chunk_k * 4)
                .contiguous()
            )

        a_scale_ref = unpack_scale(a_scale_ref).repeat_interleave(VEC_SIZE, dim=1)[
            :M, :K
        ]
        b_scale_ref = (
            unpack_scale(b_scale_ref)
            .repeat_interleave(VEC_SIZE, dim=1)
            .T.contiguous()[:K, :N]
        )
        reference = torch.matmul(
            a_ref.to(torch.float32) * a_scale_ref, b_ref * b_scale_ref
        )
...
def validate_block_scaled(M, N, K, block_scale_type="nvfp4"):
    ...
    a_desc, a_scale, b_desc, b_scale, configs, reference = initialize_block_scaled(
        M, N, K, block_scale_type, compute_reference=True
    )
    output = block_scaled_matmul(
        a_desc, a_scale, b_desc, b_scale, torch.float16, M, N, K, configs
    )
    torch.testing.assert_close(
        reference, output.to(torch.float32), atol=1e-3, rtol=1e-3
    )
...
def bench_block_scaled(K, block_scale_type="nvfp4", reps=10):
    ...
    a_desc, a_scale, b_desc, b_scale, configs, _ = initialize_block_scaled(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format",
        type=str,
        choices=["mxfp4", "nvfp4", "mxfp8", "mixed"],
        default="nvfp4",
    )
