...
class TensorRTStableDiffusionInpaintPipeline:
    ...
    def to(
        ...
        unet_shape_dict = {
            "sample": (2, 9, latent_height, latent_width),
            "encoder_hidden_states": (2, 77, 768),
            "latent": (2, 4, latent_height, latent_width),
        }
        self.engine["unet"].allocate_buffers(
            shape_dict=unet_shape_dict, device=self.torch_device
        )
        return self
    ...
    def triton_encode_image(self, init_image):
        vae_encoder_inputs = []
        vae_encoder_inputs.append(
            httpclient.InferInput(
                "images", init_image.shape, "FP32"
            ).set_data_from_numpy(init_image.cpu().numpy(), binary_data=False)
        )
        vae_encoder_outputs = []
    ...
    def triton_encode_prompt(self, prompt, negative_prompt):
        ...
        text_encoder_inputs = []
        text_encoder_inputs.append(
            httpclient.InferInput("input_ids", text_input_ids_inp.shape, "INT32")
        )
        text_encoder_inputs[0].set_data_from_numpy(
            text_input_ids_inp.numpy().astype(np.int32), binary_data=False
        )
        text_encoder_outputs = []
        text_encoder_outputs.append(httpclient.InferRequestedOutput("text_embeddings"))
        ...
        text_encoder_inputs.append(
            httpclient.InferInput("input_ids", uncond_input_ids_inp.shape, "INT32")
        )
        text_encoder_inputs[0].set_data_from_numpy(
    ...
    def triton_denoise_latent(
        ...
        for step_index, timestep in enumerate(timesteps):
            ...
            unet_inputs.append(
                pb_utils.Tensor("encoder_hidden_states", text_embeddings.cpu().numpy())
            )

            inference_request = pb_utils.InferenceRequest(
                model_name="unet", requested_output_names=["latent"], inputs=unet_inputs
            )
    ...
    def triton_decode_latent(self, latents):
        vae_decoder_inputs = []
        vae_decoder_inputs.append(
            httpclient.InferInput("latent", latents.shape, "FP32").set_data_from_numpy(
                latents.cpu().numpy(), binary_data=False
            )
        )
        vae_decoder_outputs = []
    ...
    def __call__(
        ...
        with torch.inference_mode(), torch.autocast("cuda"), trt.Runtime(TRT_LOGGER):
            ...
            text_embeddings = self.triton_encode_prompt(prompt, negative_prompt)
