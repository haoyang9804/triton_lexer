...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "quantize_kv_copy", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(4, 16):
            ...
            Out_scale = torch.empty(
                (seq_len, head_num, head_dim // 8), dtype=torch.float16
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        total_bytes = K.numel() * K.element_size() + K.numel() // 8 * 4
