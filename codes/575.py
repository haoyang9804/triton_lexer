...
def online_softmax2(x: torch.Tensor) -> torch.Tensor:
    ...
    for r in range(num_rows):
        ...
        for c in range(num_cols):
            ...
            normalizer = normalizer * torch.exp(prev_max - max_so_far) + torch.exp(
                curr - max_so_far
            )
...
def f_online_softmax(x: torch.Tensor) -> torch.Tensor:
    ...
    for row in range(row_count):
        ...
        for col in range(col_count):
            ...
            norm_term = norm_term * torch.exp(old_row_max - row_max) + torch.exp(
                val - row_max
            )
...
for row in range(row_count):
    ...
    for col in range(col_count):
        ...
        normalizer_term = normalizer_term * torch.exp(
            old_row_max - row_max
        ) + torch.exp(val - row_max)
        if old_row_max != row_max:
            print("new max discovered")
        print(f"current row max: {row_max}, denominator: {normalizer_term}")

    online_softmax[row, :] = (
        torch.exp(long_input_vec[row, :] - row_max) / normalizer_term
    )
