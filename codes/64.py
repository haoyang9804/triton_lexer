...
def test_nvshmem_basic():
    ...
    try:
        torch.testing.assert_close(
            output,
            torch.tensor(
                [RANK, RANK, LOCAL_RANK, WORLD_SIZE, WORLD_SIZE, LOCAL_WORLD_SIZE],
                dtype=torch.int32,
                device="cuda",
            ),
        ), output
...
def test_nvshmemx_getmem_with_scope(N, dtype: torch.dtype = torch.int8):
    ...
    for scope in ["block", "warp", "thread"]:
        for nbi in [True, False]:
            ...
            pynvshmem.nvshmem_barrier_all()
            t_expected = (
                torch.arange(1, WORLD_SIZE + 1, dtype=dtype, device="cuda")
                .reshape((WORLD_SIZE, 1))
                .repeat(1, N // WORLD_SIZE)
                .flatten()
            )
            try:
...
def test_nvshmem_signal():
            ...
            for n in range(iters):
                if mype == 0:
                    libshmem_device.signal_wait_until(
                        t, libshmem_device.NVSHMEM_CMP_EQ, 1 + n
                    )
    ...
    if pynvshmem.nvshmem_my_pe() == 0:
        try:
            torch.testing.assert_close(
                t.to(torch.int32),
                torch.ones([1], dtype=torch.int32, device="cuda") * 100,
            )
        except Exception as e:
...
def test_nvshmemx_putmem_signal_with_scope(N, dtype: torch.dtype = torch.int8):
    ...
    @triton.jit
    def _nvshmemx_putmem_signal(
        ptr, signal, bytes_per_rank, scope: tl.constexpr, nbi: tl.constexpr
    ):
        mype = libshmem_device.my_pe()
        pid = tl.program_id(axis=0)
        thread_idx = tid(axis=0)
    ...
    for scope in ["block", "warp", "thread"]:
        for nbi in [True, False]:
            ...
            try:
                torch.testing.assert_close(t, t_expected)
                torch.testing.assert_close(
                    signal, torch.ones((WORLD_SIZE,), dtype=torch.uint64, device="cuda")
                )
            except Exception as e:
                print(f" ❌ {api} failed")
                print(t.reshape(WORLD_SIZE, -1))
...
def test_nvshmem_barrier_sync_quiet_fence():
    ...
    _nvshmem_barrier_sync_quiet_fence_with_team[(1,)](
        pynvshmem.NVSHMEMX_TEAM_NODE, num_warps=4
    )
    torch.cuda.synchronize()
...
def test_nvshmem_broadcast(N, dtype: torch.dtype = torch.int8):
    ...
    def _nvshmem_broadcast(dst, src, nbytes, scope: tl.constexpr):
        ...
        wid = thread_idx // 32
        if scope == "block":
            libshmem_device.broadcast_block(
                libshmem_device.NVSHMEM_TEAM_WORLD, dst, src, nbytes, 0
            )
        if scope == "warp":
            if wid == 0:
                libshmem_device.broadcast_warp(
                    libshmem_device.NVSHMEM_TEAM_WORLD, dst, src, nbytes, 0
                )
                __syncthreads()
        if scope == "thread":
            if thread_idx == 0:
                libshmem_device.broadcast(
                    libshmem_device.NVSHMEM_TEAM_WORLD, dst, src, nbytes, 0
                )
            __syncthreads()
...
def test_nvshmem_fcollect(N, dtype: torch.dtype = torch.int8):
    ...
    for scope in ["block", "warp", "thread"]:
        ...
        torch.cuda.synchronize()
        t_expected = (
            torch.ones_like(dst).reshape((WORLD_SIZE, -1))
            * torch.arange(1, 1 + WORLD_SIZE, device="cuda").to(dtype)[:, None]
        ).flatten()
        try:
            torch.testing.assert_close(dst, t_expected)
        except Exception as e:
...
def test_nvshmem_mc_ptr(N, dtype: torch.dtype = torch.int8):
    ...
    if not _if_nvls_supported():
        print(
            "not support MultiCast memory. only works on NVLS hardware and NVSHMEM_DISABLE_CUDA_VMM=0"
        )
        return
...
def test_nvshmemi_putmem_rma_signal_with_scope(N, dtype: torch.dtype = torch.int8):
    ...
    @triton.jit
    def _nvshmemi_putmem_rma_signal_kernel(
        ptr, signal, bytes_per_rank, scope: tl.constexpr, nbi: tl.constexpr
    ):
        mype = libshmem_device.my_pe()
        pid = tl.program_id(axis=0)
