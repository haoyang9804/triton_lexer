...
def kernel_dispatch_token(
    ...
    index_elem_size = (
        tl.constexpr(topk_indices_buf.dtype.element_ty.primitive_bitwidth) // 8
    )
    ...
    for node_offset in range(0, nnodes):
        if node_offset != nnodes - 1:
            ...
            for req_id in range(global_warp_id, num_tokens, total_warps):
                start_index = ld_b32(
                    send_reqs_for_nodes + target_node * max_tokens * 2 + req_id
                )
                end_index = ld_b32(
                    send_reqs_for_nodes
                    + target_node * max_tokens * 2
                    + max_tokens
                    + req_id
                )
                ...
                src_ptr = (
                    input_buf
                    + node_id * max_tokens * hidden_size
                    + start_index * hidden_size
                )
                if end_index > start_index:
                    libshmem_device.putmem_nbi_warp(
                        src_ptr, src_ptr, msg_size, target_rank
                    )
            ...
            if pid == 0:
                ...
                libshmem_device.putmem_nbi_block(
                    send_req_dst_ptr, send_req_src_ptr, msg_size, target_rank
                )
            ...
            count = tl.atomic_add(
                counter_ptr + target_node, 1, scope="gpu", sem="release"
            )
        ...
        if node_offset > 0:
            if thread_idx == 0:
                libshmem_device.signal_wait_until(
                    signals_buf + src_send_node, libshmem_device.NVSHMEM_CMP_EQ, 1
                )
            __syncthreads()
        ...
        for token_offset in range(global_warp_id, token_num, total_warps):
            for j in range(topk):
                expert_idx = ld_b32(
                    topk_indices_buf
                    + (src_send_node * max_tokens + token_offset) * topk
                    + j
                )
...
def kernel_combine_token(
    ...
    tl.static_assert(
        hidden_size <= BLOCK_SIZE, "BLOCK_SIZE must be larger than hidden_size"
    )
    WARP_SIZE = 32
    ...
    for node_offset in range(1, nnodes):
        ...
        for token_idx in range(pid, token_num, num_pid):
            ...
            for j in range(topk):
                expert_idx = tl.load(
                    topk_indices_buf + (target_node * max_tokens + token_idx) * topk + j
                )
                ...
                if expert_node_idx == node_id:
                    token_scatter_idx = tl.load(
                        token_dst_scatter_idx
                        + (target_node * max_tokens + token_idx) * topk
                        + j
                    )
            ...
            tl.store(
                intra_node_reduce_buf
                + (target_node * max_tokens + token_idx) * hidden_size
                + token_block_offs,
                token_accum,
                mask=token_mask,
            )
        ...
        count = tl.atomic_add(counter_ptr + target_node, 1, scope="gpu", sem="release")
        while ld_acquire(counter_ptr + target_node, "gpu") != num_pid:
            pass

        for req_id in range(global_warp_id, token_num, total_warps):
            start_index = ld_b32(
                send_reqs_in_dispatch + target_node * max_tokens * 2 + req_id
            )
            end_index = ld_b32(
                send_reqs_in_dispatch
                + target_node * max_tokens * 2
            ...
            if end_index > start_index:
                src_ptr = (
                    intra_node_reduce_buf
                    + target_node * max_tokens * hidden_size
                    + start_index * hidden_size
                )
                dst_ptr = (
                    send_buf
    ...
    for token_idx in range(pid, num_dispatch_token_cur_rank, num_pid):
        ...
        for j in range(topk):
            expert_idx = tl.load(
                topk_indices_buf + (node_id * max_tokens + token_idx) * topk + j
            )
            expert_rank = expert_idx // expert_per_rank
            expert_node_idx = expert_rank // local_world_size
            ...
            if expert_node_idx == node_id:
                token_scatter_idx = tl.load(
                    token_dst_scatter_idx
                    + (node_id * max_tokens + token_idx) * topk
                    + j
                )
                remote_input_ptr = dl.symm_at(input_buf, expert_rank)
                remote_input_ptr = tl.multiple_of(remote_input_ptr, 32)
...
def kernel_get_ag_splits_and_recv_offset(
    ...
    for target_rank in range(pid, world_size, num_pid):
        libshmem_device.signal_wait_until(
            splits_signal_buf + target_rank, libshmem_device.NVSHMEM_CMP_EQ, 1
        )
        ...
        splits_cur_rank = tl.load(
            full_splits_buf + target_rank * num_experts + offs, mask=mask
        )
    ...
    count = tl.atomic_add(grid_sync_counter + 0, 1, scope="gpu", sem="relaxed")
    while ld_acquire(grid_sync_counter + 0, "gpu") != num_pid:
        pass

    for ep_rank in range(pid, world_size, num_pid):
        splits_cur_rank = tl.load(
            recv_buf_offset_per_expert + ep_rank * num_experts + offs, mask=mask
        )
...
def bincount(input, length, output=None, output_dtype=torch.int32, num_sm=16):
    ...
    n = input.size(0)
    num_warps = 8
    kernel_bincount[(num_sm,)](
        n, input, output, BLOCK_SIZE=num_warps * 32, num_warps=num_warps
    )
    return output


def get_dispatch_send_reqs_for_target_node(
    token_node_idx, traget_node_id, index_type=torch.int32
):
    send_token_mask = (token_node_idx == traget_node_id).any(dim=1).to(torch.int32)

    padded_mask = torch.zeros(
        (send_token_mask.shape[0] + 2,),
        dtype=send_token_mask.dtype,
...
def get_ag_splits_and_recv_offset_for_dispatch(
    ...
    num_input_tokens_per_rank = torch.empty(
        (world_size,), dtype=torch.int32, device=torch.cuda.current_device()
    )
    ...
    counter_workspace = torch.zeros(
        (num_grid_sync,), dtype=torch.int32, device=torch.cuda.current_device()
    )
