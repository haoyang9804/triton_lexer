...
def kldiv_forward_triton(y_pred, y_true, log_target, reduction, eps):
    ...
    num_warps = (
        4
        if BLOCK_SIZE < 2048
        else 8 if BLOCK_SIZE < 8192 else 16 if BLOCK_SIZE < 32768 else 32
    )
...
def test_kldiv():
    ...
    y_pred = torch.tensor(
        [[0.2, 0.3, 0.5], [0.1, 0.6, 0.3]], device="cuda", dtype=torch.float32
    ).log()
    y_true = torch.tensor(
        [[0.1, 0.4, 0.5], [0.2, 0.5, 0.3]], device="cuda", dtype=torch.float32
    )
    eps = 1e-6

    results = {}
    for i, reduction in enumerate(["none", "sum", "mean", "batchmean"]):
        output = kldiv_forward_triton(
            y_pred, y_true, log_target=False, reduction=reduction, eps=eps
        )
        results[f"test_case_{i+1}"] = output

    y_true_log = y_true.log()
    output_log_target = kldiv_forward_triton(
        y_pred, y_true_log, log_target=True, reduction="sum", eps=eps
    )
    ...
    backward_output = kldiv_backward_triton(
        target, grad_output, new_grads, log_target=False
    )
    results["test_case_6"] = backward_output

    target_log = target.log()
    backward_output_log_target = kldiv_backward_triton(
        target_log, grad_output, new_grads, log_target=True
    )
