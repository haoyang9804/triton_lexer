...
def fused_recurrent_bwd_kernel(
    ...
    p_dk = (
        dk + (i_bh + i_v * B * H) * s_qk_h + i_k * BK + tl.arange(0, BK) + (T - 1) * K
    )
    p_dv = (
        dv + (i_bh + i_k * B * H) * s_vo_h + i_v * BV + tl.arange(0, BV) + (T - 1) * V
    )
    if IS_HEADWISE_BETA:
        p_dbeta = (
            dbeta
            + (i_bh + i_k * B * H + i_v * B * H * NK) * s_vo_h
            + tl.arange(0, BV)
            + (T - 1) * V
        )
    else:
    ...
    if USE_DHT:
        ...
        d_h += tl.load(p_ht, mask=mask_bk[:, None] & mask_bv[None, :], other=0).to(
            tl.float32
        )
    ...
    if USE_DH0:
        ...
        tl.store(
            p_dh0,
            d_h.to(p_dh0.dtype.element_ty),
            mask=mask_bk[:, None] & mask_bv[None, :],
        )
...
class FusedRecurrentFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, q, k, v, beta, scale=None, initial_state=None, output_final_state=False
    ):
...
def fused_recurrent_delta_rule(
    ...
    o, final_state = FusedRecurrentFunction.apply(
        q, k, v, beta, scale, initial_state, output_final_state
    )
    return o, final_state
...
def test_fused_recurrent_delta_rule_with_backward():
    ...
    beta_headwise = torch.randn(
        B, H, T, V, dtype=torch.float32, device="cuda", requires_grad=True
    )
    beta_non_headwise = torch.randn(
        B, H, T, dtype=torch.float32, device="cuda", requires_grad=True
    )
