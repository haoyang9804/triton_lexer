...
def fwd_pre(g, B, H, T, S, BT):
    ...
    chunk_gated_abc_fwd_kernel_cum[grid](
        g_org, g, g.stride(1), g.stride(2), g.stride(3), T=T, S=S, BT=BT
    )
    return g
...
def test_fwd_pre_inner():
    ...
    results["test_case_1"] = fwd_inner(
        q, k, v, g_cum, B, H, T, K, V, BT, BK, BV, gatek=False
    )

    results["test_case_2"] = fwd_inner(
        q, k, v, g_cum, B, H, T, K, V, BT, BK, BV, gatek=True, h0=h0
    )

    results["test_case_3"] = fwd_inner(
        q, k, v, g_cum, B, H, T, K, V, BT, BK, BV, gatek=False, h0=h0, ht=ht
    )

    results["test_case_4"] = fwd_inner(
        q, k, v, g_cum, B, H, T, K, V, BT, BK, BV, gatek=True, ht=ht
    )
