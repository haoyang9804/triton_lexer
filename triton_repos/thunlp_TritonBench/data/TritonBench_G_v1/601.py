...
def test_destindex_copy_quantize_kv():
    ...
    Out_scale = torch.empty(
        (seq_len, head_num, head_dim // quant_group_dim),
        dtype=torch.float32,
        device="cuda",
    )

    destindex_copy_quantize_kv(K, DestLoc, Out, Out_scale)
    result_case_1 = {"Out": Out, "Out_scale": Out_scale}

    batch_size_small = 1
    K_small = torch.randn(
        (batch_size_small, head_num, head_dim), dtype=torch.float32, device="cuda"
    )
    DestLoc_small = torch.randint(
        0, seq_len, (batch_size_small,), dtype=torch.int32, device="cuda"
    )
    Out_small = torch.empty_like(K_small, dtype=torch.int8)
    Out_scale_small = torch.empty(
        (batch_size_small, head_num, head_dim // quant_group_dim),
        dtype=torch.float32,
        device="cuda",
    )

    destindex_copy_quantize_kv(K_small, DestLoc_small, Out_small, Out_scale_small)
    result_case_2 = {"Out": Out_small, "Out_scale": Out_scale_small}

    DestLoc_varied = torch.randint(
        0, seq_len, (seq_len // 2,), dtype=torch.int32, device="cuda"
    )
    Out_varied = torch.empty_like(K, dtype=torch.int8)
    Out_scale_varied = torch.empty(
        (seq_len // 2, head_num, head_dim // quant_group_dim),
        dtype=torch.float32,
        device="cuda",
    )

    destindex_copy_quantize_kv(K, DestLoc_varied, Out_varied, Out_scale_varied)
    result_case_3 = {"Out": Out_varied, "Out_scale": Out_scale_varied}

    try:
        head_dim_invalid = 15
        K_invalid = torch.randn(
            (seq_len, head_num, head_dim_invalid), dtype=torch.float32, device="cuda"
        )
        DestLoc_invalid = torch.randint(
            0, seq_len, (seq_len,), dtype=torch.int32, device="cuda"
        )
        Out_invalid = torch.empty_like(K_invalid, dtype=torch.int8)
        Out_scale_invalid = torch.empty(
            (seq_len, head_num, head_dim_invalid // quant_group_dim),
            dtype=torch.float32,
            device="cuda",
        )

        destindex_copy_quantize_kv(
            K_invalid, DestLoc_invalid, Out_invalid, Out_scale_invalid
        )
