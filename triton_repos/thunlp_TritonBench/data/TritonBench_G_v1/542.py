...
def fused_recurrent_rwkv6_fwd_kernel(
    ...
    p_q = (
        q + i_bh * s_k_h + i_k * BK + tl.arange(0, BK) + ((T - 1) * K if REVERSE else 0)
    )
    p_k = (
        k + i_bh * s_k_h + i_k * BK + tl.arange(0, BK) + ((T - 1) * K if REVERSE else 0)
    )
    p_v = (
        v + i_bh * s_v_h + i_v * BV + tl.arange(0, BV) + ((T - 1) * V if REVERSE else 0)
    )
    p_o = (
        o
        + (i_bh + i_k * B * H) * s_v_h
    ...
    p_w = (
        w + i_bh * s_k_h + i_k * BK + tl.arange(0, BK) + ((T - 1) * K if REVERSE else 0)
    )
...
def fused_recurrent_rwkv6(
    ...
    o, final_state = FusedRecurrentRWKV6Function.apply(
        r, k, v, w, u, scale, initial_state, output_final_state
    )
...
def test_fused_recurrent_rwkv6():
    ...
    o, final_state = fused_recurrent_rwkv6(
        r, k, v, w, u, scale=0.5, initial_state=None, output_final_state=False
    )
    ...
    o, final_state = fused_recurrent_rwkv6(
        r, k, v, w, u, scale=0.5, initial_state=initial_state, output_final_state=False
    )
    results["test_case_2"] = {"output": o.shape, "final_state": final_state}

    o, final_state = fused_recurrent_rwkv6(
        r, k, v, w, u, scale=0.5, initial_state=None, output_final_state=True
    )
    results["test_case_3"] = {"output": o.shape, "final_state": final_state.shape}

    o, final_state = fused_recurrent_rwkv6(
        r, k, v, w, u, scale=0.5, initial_state=initial_state, output_final_state=True
    )
