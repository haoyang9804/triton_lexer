...
def test_layer_norm_fwd_bwd():
    ...
    y, mean, rstd, residual_out = _layer_norm_fwd(
        x, weight, bias, eps, residual=None, is_rms_norm=False
    )
    ...
    y, mean, rstd, residual_out = _layer_norm_fwd(
        x, weight, bias, eps, residual=None, is_rms_norm=True
    )
    results["test_case_3"] = (y, mean, rstd, residual_out)

    dy = torch.randn_like(y)
    dx, dw, db, dresidual_in = _layer_norm_bwd(
        dy, x, weight, bias, eps, mean, rstd, is_rms_norm=True
    )
    results["test_case_4"] = (dx, dw, db, dresidual_in)

    residual = torch.randn_like(x)
    y, mean, rstd, residual_out = _layer_norm_fwd(
        x, weight, bias, eps, residual=residual, is_rms_norm=False
    )
    results["test_case_5"] = (y, mean, rstd, residual_out)

    dy = torch.randn_like(y)
    dx, dw, db, dresidual_in = _layer_norm_bwd(
        dy, x, weight, bias, eps, mean, rstd, dresidual=residual, is_rms_norm=False
    )
    ...
    dy = torch.randn_like(y)
    dx, dw, db, dresidual_in, recomputed_y = _layer_norm_bwd(
        dy, x, weight, bias, eps, mean, rstd, recompute_output=True
    )
    ...
    dy = torch.randn_like(y)
    dx, dw, db, dresidual_in, recomputed_y = _layer_norm_bwd(
        dy, x, weight, bias, eps, mean, rstd, dresidual=residual, recompute_output=True
    )
