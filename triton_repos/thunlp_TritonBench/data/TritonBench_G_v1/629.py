...
def test_token_softmax_reducev_fwd():
    ...
    logics = torch.randn(
        (num_heads, batch_size * max_input_len), dtype=torch.float32, device="cuda"
    )
    v = torch.randn(
        (num_heads, max_input_len, d_model), dtype=torch.float32, device="cuda"
    )
    o = torch.empty(
    ...
    b_seq_len = torch.tensor(
        [max_input_len, max_input_len], dtype=torch.int32, device="cuda"
    )
    token_softmax_reducev_fwd(
        logics, v, o, b_loc, b_start_loc, b_seq_len, max_input_len, other_kv_index
    )
