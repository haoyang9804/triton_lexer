...
def test_flash_decode_stage2():
    ...
    mid_out_logexpsum_1 = torch.randn(
        batch_size, head_num, seq_block_num, dtype=torch.float32, device="cuda"
    )
    O_1 = torch.empty(
        batch_size, head_num, head_dim, dtype=torch.float32, device="cuda"
    )

    flash_decode_stage2(mid_out_1, mid_out_logexpsum_1, B_Seqlen_1, O_1, block_seq)
    results["test_case_1"] = O_1.clone().cpu()

    B_Seqlen_2 = torch.tensor([0, 0], dtype=torch.int32, device="cuda")
    mid_out_2 = torch.randn(
        batch_size,
        head_num,
        seq_block_num,
        head_dim,
    ...
    mid_out_logexpsum_2 = torch.randn(
        batch_size, head_num, seq_block_num, dtype=torch.float32, device="cuda"
    )
    O_2 = torch.empty(
        batch_size, head_num, head_dim, dtype=torch.float32, device="cuda"
    ...
    B_Seqlen_3 = torch.tensor([8, 8], dtype=torch.int32, device="cuda")
    mid_out_3 = torch.randn(
        batch_size,
        head_num,
        seq_block_num,
        head_dim,
    ...
    mid_out_logexpsum_3 = torch.randn(
        batch_size, head_num, seq_block_num, dtype=torch.float32, device="cuda"
    )
    O_3 = torch.empty(
        batch_size, head_num, head_dim, dtype=torch.float32, device="cuda"
    ...
    B_Seqlen_4 = torch.tensor([32, 64], dtype=torch.int32, device="cuda")
    mid_out_4 = torch.randn(
        batch_size,
        head_num,
        seq_block_num,
        head_dim,
    ...
    mid_out_logexpsum_4 = torch.randn(
        batch_size, head_num, seq_block_num, dtype=torch.float32, device="cuda"
    )
    O_4 = torch.empty(
        batch_size, head_num, head_dim, dtype=torch.float32, device="cuda"
