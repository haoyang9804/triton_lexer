...
def _copy_to_kvcache_seqlen1_kernel(
    ...
    for split_x in tl.static_range(HEAD_DIM // KCACHE_X):
        offsets_dmodel_x_partition = tl.arange(
            split_x * KCACHE_X, (split_x + 1) * KCACHE_X
        )
        offsets_k = (
            cur_seq_idx * stride_kt
            + cur_kv_head_idx * stride_kh
            + offsets_dmodel_x_partition * stride_kd
        )
        k = tl.load(K + offsets_k)
        offsets_v = (
            cur_seq_idx * stride_vt
...
def test_copy_kv_to_blocked_cache():
    ...
    copy_kv_to_blocked_cache(
        k, v, k_cache, v_cache, kv_lengths, block_tables, use_new_kcache_layout=False
    )
    ...
    v_cache_new_layout = torch.zeros(
        max_blocks_per_sequence,
        num_kv_heads,
        block_size,
        head_dim,
