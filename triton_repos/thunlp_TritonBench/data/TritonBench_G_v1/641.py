...
def test_token_att_fwd2():
    ...
    out = torch.zeros(
        (batch_size, num_heads, d_model), dtype=torch.float32, device="cuda"
    )
    Req_to_tokens = torch.randint(
        0, seq_len, (batch_size, seq_len), dtype=torch.int32, device="cuda"
    )
    ...
    v_2 = torch.rand(
        (num_heads, seq_len_2, d_model), dtype=torch.float32, device="cuda"
    )
    out_2 = torch.zeros(
        (batch_size, num_heads, d_model), dtype=torch.float32, device="cuda"
    )
    Req_to_tokens_2 = torch.randint(
        0, seq_len_2, (batch_size, seq_len_2), dtype=torch.int32, device="cuda"
    )
    B_Seqlen_2 = torch.full((batch_size,), seq_len_2, dtype=torch.int32, device="cuda")

    token_att_fwd2(
        prob_2, v_2, out_2, Req_to_tokens_2, B_req_idx, B_Start_Loc, B_Seqlen_2
    )
    torch.cuda.synchronize()
    ...
    out_3 = torch.zeros(
        (batch_size_3, num_heads, d_model), dtype=torch.float32, device="cuda"
    )
    Req_to_tokens_3 = torch.randint(
        0, seq_len, (batch_size_3, seq_len), dtype=torch.int32, device="cuda"
    )
    ...
    token_att_fwd2(
        prob_3, v_3, out_3, Req_to_tokens_3, B_req_idx_3, B_Start_Loc_3, B_Seqlen_3
    )
    torch.cuda.synchronize()
