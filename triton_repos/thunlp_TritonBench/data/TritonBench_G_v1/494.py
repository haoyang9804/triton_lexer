...
def get_xine_cache(
    ...
    if is_prompts:
        ...
        cos_output = torch.empty(
            (total_length, hidden_dim), dtype=cos_cache.dtype, device=cos_cache.device
        )
        sin_output = torch.empty(
            (total_length, hidden_dim), dtype=sin_cache.dtype, device=sin_cache.device
        )
        ...
        cos_output = torch.empty(
            (num_seqs, hidden_dim), dtype=cos_cache.dtype, device=cos_cache.device
        )
        sin_output = torch.empty(
            (num_seqs, hidden_dim), dtype=sin_cache.dtype, device=sin_cache.device
        )
...
def test_get_xine_cache():
    ...
    lengths = torch.randint(
        1, max_length, (num_seqs,), dtype=torch.int32, device="cuda"
    )
    cos_cache = torch.randn(
        (max_length, hidden_dim), dtype=torch.float32, device="cuda"
    )
    ...
    for i, is_prompts in enumerate(is_prompts_list, start=1):
        cos_output, sin_output = get_xine_cache(
            lengths, cos_cache, sin_cache, is_prompts=is_prompts
        )
