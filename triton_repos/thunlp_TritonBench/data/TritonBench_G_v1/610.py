...
def rbe_triton(
    ...
    cos, sin = get_freq_multi_tokens(
        offs_cn=offs_n, starting_idx=start_block, theta=THETA, NB_TOKENS=BLOCK_SIZE_M
    )
...
def rms_matmul_rbe_wrapper(
    ...
    assert weight.dtype in [
        torch.float16,
        torch.int8,
    ], "Only torch.float16 or torch.int8 are supported for weight"

    if rms_w.dtype != weight.dtype:

        rms_w = rms_w.to(weight.dtype)
    ...
    out_ptr = triton.reinterpret(
        out, tl.float8e5 if out.dtype == torch.int8 else tl.float16
    )
...
def test_rms_matmul_rbe():
    ...
    use_rbe = False
    start_pos = 0
    out = rms_matmul_rbe_wrapper(
        x, weight, rms_w, use_rbe, start_pos, n_heads, head_dim
    )
    ...
    use_rbe = False
    out = rms_matmul_rbe_wrapper(
        x, weight_int8, rms_w, use_rbe, start_pos, n_heads, head_dim
    )
