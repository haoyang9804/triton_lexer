...
def softmax(input: torch.Tensor, mask: torch.Tensor = None, dim=-1) -> torch.Tensor:
    if mask is not None:
        ...
        assert (
            input.shape[-1] == mask.shape[-1]
        ), "the last dimensions should be the same for input and mask"
    assert (
        dim == -1 or dim == len(input.shape) - 1
    ), "currently softmax layer only support last dimention"
    ...
    if mask is not None:
        ...
        assert (
            input.shape[0] == mask.shape[0]
        ), "the fist dimention of mask and input should be the same"
...
def test_softmax():
    ...
    mask_tensor_6 = torch.randint(
        0, 2, (100000, 256), dtype=torch.float16, device="cuda"
    )
