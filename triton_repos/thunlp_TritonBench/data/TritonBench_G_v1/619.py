...
def rotary_embedding_kernel(
    ...
    off_cos_sin = (
        tokens_range[:, None] * cos_token_stride + dim_range0[None, :] * cos_stride
    )
    loaded_cos = tl.load(
        cos + off_cos_sin, mask=(tokens_range[:, None] < q_total_tokens), other=0.0
    )
    loaded_sin = tl.load(
        sin + off_cos_sin, mask=(tokens_range[:, None] < q_total_tokens), other=0.0
    )
    ...
    loaded_q0 = tl.load(
        q + off_q0,
        mask=(
            (cur_head_idx < Q_HEAD_NUM) & (tokens_range[:, None, None] < q_total_tokens)
        ),
        other=0.0,
    )
    loaded_q1 = tl.load(
...
def fused_rotary_embedding_kernel_v2(
    ...
    off_q0 = (
        block_token_index * q_token_stride
        + block_head_index * q_head_stride
        + dim_range0 * head_dim_stride
    )
    off_q1 = (
        block_token_index * q_token_stride
        + block_head_index * q_head_stride
        + dim_range1 * head_dim_stride
    )
    off_k0 = (
        block_token_index * k_token_stride
        + block_head_index * k_head_stride
        + dim_range0 * head_dim_stride
    )
    off_k1 = (
        block_token_index * k_token_stride
    ...
    loaded_cos = tl.load(
        cos + off_cos_sin, mask=(block_token_index < q_total_tokens), other=0.0
    )
    loaded_sin = tl.load(
        sin + off_cos_sin, mask=(block_token_index < q_total_tokens), other=0.0
    )
    ...
    block_ids = tl.load(
        block_table_ptr + last_block_idx * btb_stride,
...
def test_rotary_embedding():
    ...
    q = torch.randn(
        (total_tokens, head_num, head_dim), dtype=torch.float32, device="cuda"
    )
    k = torch.randn(
        (total_tokens, kv_head_num, head_dim), dtype=torch.float32, device="cuda"
    )
    ...
    block_tables = torch.randint(0, num_blocks, (batch_size, num_blocks), device="cuda")
    kv_lengths = torch.randint(1, total_tokens, (batch_size,), device="cuda")
