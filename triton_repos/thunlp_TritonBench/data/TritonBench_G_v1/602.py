...
def test_destindex_copy_quantize_kv():
    ...
    result_1 = {"value_dest": value_dest.clone(), "scale_dest": scale_dest.clone()}
    ...
    scale_dest = torch.randn((B * N_CTX, H, 1), dtype=torch.float16).cuda()
    destindex_copy_quantize_kv(src, dest_loc, value_dest, scale_dest)
    result_2 = {"value_dest": value_dest.clone(), "scale_dest": scale_dest.clone()}

    src = torch.randn((B * N_CTX, H, D), dtype=torch.float32).cuda()
    destindex_copy_quantize_kv(src, dest_loc, value_dest, scale_dest)
    result_3 = {"value_dest": value_dest.clone(), "scale_dest": scale_dest.clone()}
    ...
    scale_dest = torch.randn((B * N_CTX, H, 1), dtype=torch.float16).cuda()
    destindex_copy_quantize_kv(src, dest_loc, value_dest, scale_dest)
    result_4 = {"value_dest": value_dest.clone(), "scale_dest": scale_dest.clone()}
