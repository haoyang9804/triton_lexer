...
def _fwd_recurrence(
    ...
    S = (
        ...
        + tl.arange(0, BLOCK_MODEL_V)[None, :]
    )
    O = (
        O
        + offset_bh * NUM_BLOCK * D_MODEL_K * D_MODEL_V
...
def _bwd_recurrence(
    ...
    DG = (
        DG
        + offset_bh * NUM_BLOCK * NUM_K * NUM_V
        + offset_d * NUM_V
        + offset_s
        + (NUM_BLOCK - 2) * NUM_K * NUM_V
    )
...
def test_chunk_gate_recurrent():
    ...
    kv = torch.randn(
        B, H, N, D_k, D_v, device="cuda", dtype=torch.float32, requires_grad=True
    )
    cross_decay = torch.randn(
        B, H, N, device="cuda", dtype=torch.float32, requires_grad=True
    )

    last_kv = torch.randn(
        B, H, D_k, D_v, device="cuda", dtype=torch.float32, requires_grad=True
    )
    ...
    result = {
        "test_case_1": (
            kv.grad is not None,
            cross_decay.grad is not None,
            last_kv.grad is not None,
        ),
