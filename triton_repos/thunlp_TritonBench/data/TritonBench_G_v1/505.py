...
class ChunkRetentionFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, q, k, v, initial_state, output_final_state, scale, checkpoint_level
    ):
        BT = 64
...
def chunk_retention(
    ...
    assert (
        q.dim() == k.dim() == v.dim() == 4
    ), "q, k, v must have 4 dimensions (b, h, l, d)"
...
def test_chunk_retention_with_backward():
    ...
    initial_state = torch.randn(
        B, H, K, V, dtype=torch.float32, requires_grad=True, device="cuda"
    )
    ...
    for checkpoint_level in [0, 1]:
        o, _ = chunk_retention(
            q, k, v, output_final_state=False, checkpoint_level=checkpoint_level
        )
        loss = o.sum()
        loss.backward()
        q.grad.zero_()
