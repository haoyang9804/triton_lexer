...
def decoding_fused_rotary_embedding_kernel(
    ...
    if handle_kv:
        ...
        block_ids = tl.load(
            BLOCK_TABLES + cur_token_idx * bts_stride + last_block_idx * btb_stride
        )
...
def test_decoding_fused_rotary_embedding():
    ...
    q = torch.randn(
        (total_tokens, q_head_num, head_dim), dtype=torch.float32, device="cuda"
    )
    k = torch.randn(
    ...
    block_tables = torch.randint(
        0, num_blocks, (batch_size, num_blocks), dtype=torch.int32, device="cuda"
    )
    kv_lengths = torch.randint(
        1, total_tokens, (batch_size,), dtype=torch.int32, device="cuda"
    )
    ...
    x = 16
    k_cache = torch.zeros(
        (num_blocks, kv_head_num, head_dim // x, block_size, x),
        dtype=torch.float32,
        device="cuda",
    )
    v_cache = torch.zeros(
