...
def test_cross_entropy_loss():
    ...
    label_smoothing = 0.1
    losses_smoothing, z_losses_smoothing = cross_entropy_loss(
        logits, labels, label_smoothing=label_smoothing
    )

    ignored_index = 5
    labels[0] = ignored_index
    losses_ignored, z_losses_ignored = cross_entropy_loss(
        logits, labels, ignored_index=ignored_index
    )

    logit_scale = 0.5
    losses_logit_scale, z_losses_logit_scale = cross_entropy_loss(
        logits, labels, logit_scale=logit_scale
    )

    lse_square_scale = 0.1
    losses_lse_square, z_losses_lse_square = cross_entropy_loss(
        logits, labels, lse_square_scale=lse_square_scale
    )
