...
def _attn_bwd(
    ...
    kv_adj = (stride_kh * ((bhid % H) // KV_GROUP) + stride_kz * (bhid // H)).to(
        tl.int64
    )
    pid = tl.program_id(0)
...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        dk = k.new_zeros(dk_size)
        dv = v.new_zeros(dv_size)
...
def test_op(Z, H, N_CTX, HEAD_DIM, KV_GROUP, causal, dtype=torch.float16):
    ...
    k = (
        torch.empty((Z, KV_NUM, N_CTX, HEAD_DIM), dtype=dtype, device="cuda")
        .normal_(mean=0.0, std=0.5)
        .requires_grad_()
    )
    v = (
