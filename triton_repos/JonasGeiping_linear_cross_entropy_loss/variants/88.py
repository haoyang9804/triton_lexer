...
def compiled_helper_version(x, y, At, lse_global, xgrad, Atgrad):
    ...
    for v_idx in range(V // V_chunk_size):
        for n_idx in range(N // N_chunk_size):
            x_chunk = x[
                n_idx * N_chunk_size : (n_idx + 1) * N_chunk_size, :
            ].contiguous()
            A_v_t = At[
                :, v_idx * V_chunk_size : (v_idx + 1) * V_chunk_size
            ].contiguous()
            ...
            xgrad[n_idx * N_chunk_size : (n_idx + 1) * N_chunk_size, :] += (
                x_grad_chunk / N
            )
            Atgrad[:, v_idx * V_chunk_size : (v_idx + 1) * V_chunk_size] += (
                A_v_grad.T / N
            )
