...
@triton.autotune(
    configs=[
        ...
        triton.Config(
            {"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 16, "H_BLOCK_SIZE": 256}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 16, "H_BLOCK_SIZE": 16}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128}, num_warps=4
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 16, "H_BLOCK_SIZE": 256}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 512, "N_BLOCK_SIZE": 16, "H_BLOCK_SIZE": 16}, num_warps=8
        ),
        triton.Config(
            {"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128}, num_warps=8
        ),
    ],
    key=["V", "N", "H"],
    ...
    def forward(
        ...
        for idx_N_group, x_n_chunk in enumerate(x.split(N_group)):
            with torch.cuda.device(x.device.index):
                ...
                if x.requires_grad:
                    x_grad[
                        N_group * idx_N_group : x_n_chunk.shape[0] * (idx_N_group + 1)
                    ] = (z_grad @ At.T)
