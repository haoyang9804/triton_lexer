...
@triton.autotune(
    configs=[
        triton.Config(
            {"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 64, "V_TILES": 1}
        ),
        triton.Config(
...
class LinearCrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(
        ...
        lse_local = -10e5 * torch.ones(
            N_group, V // 16, dtype=torch.float32, device=x.device
        )
        losses = torch.zeros(
            N_group // 16, V // 16, dtype=torch.float32, device=x.device
        )
