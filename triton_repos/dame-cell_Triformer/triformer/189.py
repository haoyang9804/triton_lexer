...
class CrossEntropyLossFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            logits.shape[0] == targ.shape[0]
        ), f"Number of tokens in logits and targ is not equal: {(logits.shape, targ.shape) = }"
        assert reduction in ("mean", "sum"), reduction
        assert (
            n_chunks > 0 and n_tokens % n_chunks == 0
        ), f"n_tokens must be divisible by n_chunks: {n_tokens}, {n_chunks}"
        ...
        divisor = (
            (targ != ignore_index).sum()
            if reduction == "mean"
            else torch.ones(1, dtype=logits.dtype, device=logits.device)
        )
...
class TritonCrossEntropyLoss(torch.nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        return CrossEntropyLossFunction.apply(
            logits, targets, self.ignore_index, self.reduction, self.n_chunks
        )
