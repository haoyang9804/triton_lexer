...
class FusedAttention(Function):
    ...
    def backward(ctx, out_grad):
        ...
        return (
            Tensor(dq, device=device, requires_grad=False),
            Tensor(dk, device=device, requires_grad=False),
            Tensor(dv, device=device, requires_grad=False),
        )
...
def scaled_dot_product_attention(q, k, v):
    ...
    if seq_len not in [128, 256, 512, 1024, 2048, 4096, 8192] or head_dim not in [
        32,
        64,
        128,
        256,
