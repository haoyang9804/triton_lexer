...
@triton.jit
def maximum_scalar_kernel(
    x_ptr, scalar, output_ptr, n_elements, BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(axis=0)
    block_start = pid * BLOCK_SIZE
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
...
def iadd(x, y):
    ...
    if x.shape != output_shape:
        raise ValueError(
            "In-place addition requires x to have the same shape as the output."
        )
...
def from_numpy(data, device_id=0, dtype=None):
    ...
    arr = (
        ...
        .to(torch.device("cuda:" + str(device_id)))
        .to(torch_dtype)
    )
    return arr
...
def array(shape, device_id=0, dtype=None):
    if dtype is None or dtype == genesis.float32:
        arr = torch.empty(
            shape, device=torch.device("cuda:" + str(device_id)), dtype=torch.float32
        )
    elif dtype == genesis.float16:
        arr = torch.empty(
            shape, device=torch.device("cuda:" + str(device_id)), dtype=torch.float16
        )
    elif dtype == genesis.bfloat16:
        arr = torch.empty(
            shape, device=torch.device("cuda:" + str(device_id)), dtype=torch.bfloat16
        )
    return arr
