...
def matmul_kernel(
    ...
    input_data_ptr = (
        input_ptr + input_batch_offset + input_row_offset + input_col_offset
    )
    input_data = tl.load(input_data_ptr, mask=(input_row_mask & input_col_mask))

    weight_row_offset = tl.arange(0, num_input_cols)
    weight_row_mask = weight_row_offset[:, None] < num_input_cols
    weight_row_offset = weight_row_offset[:, None] * weight_row_stride

    weight_col_offset = col_idx * BLOCK_SIZE_COL + tl.arange(0, BLOCK_SIZE_COL)
    weight_col_mask = weight_col_offset < num_output_cols
    weight_col_offset = weight_col_offset[None, :] * weight_col_stride

    weight_data_ptr = weight_ptr + weight_row_offset + weight_col_offset
    weight_data = tl.load(weight_data_ptr, mask=(weight_row_mask & weight_col_mask))

    result = tl.dot(input_data, weight_data)
    ...
    output_data_ptr = (
        output_ptr + output_batch_offset + output_row_offset + output_col_offset
    )
...
def matmul(input: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    assert (
        input.is_cuda
    ), "Inputs are not on GPU, ensure the input matrix is loaded on the GPU"
    assert (
        weight.is_cuda
    ), "Weights are not on GPU, ensure the weight matrix is loaded on the GPU"
    assert (
        input.shape[-1] == weight.shape[-2]
    ), "Input and weight matrix are not compatible"
    ...
    grid = lambda meta: (
        B,
        triton.cdiv(N, meta["BLOCK_SIZE_ROW"]),
        triton.cdiv(D_out, meta["BLOCK_SIZE_COL"]),
    )
...
if __name__ == "__main__":
    ...
    assert torch.allclose(
        out_triton, out_torch, atol=1e-2
    ), "Torch and triton implementation does not match"
