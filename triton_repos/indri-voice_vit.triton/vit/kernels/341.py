...
if __name__ == "__main__":
    ...
    def benchmark(B, N, D, provider):
        ...
        if provider == "triton":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: layernorm_triton(a, weight=weight, bias=bias, eps=eps),
                quantiles=quantiles,
            )
        if provider == "torch":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: torch.nn.functional.layer_norm(a, _shape, weight, bias, eps).to(
                    dtype
                ),
                quantiles=quantiles,
            )
    ...
    benchmark.run(print_data=True, save_path="./benchmarks/layernorm/")
