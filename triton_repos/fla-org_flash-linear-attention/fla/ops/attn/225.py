...
@triton.jit
def parallel_attn_bwd_kernel_preprocess(o, do, delta, B: tl.constexpr, V: tl.constexpr):
    i_n = tl.program_id(0)
    o_d = tl.arange(0, B)
    m_d = o_d < V
...
class ParallelAttentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, scale, cu_seqlens):
        ...
        g_cumsum = (
            chunk_global_cumsum(g, cu_seqlens=cu_seqlens) if g is not None else None
        )
