...
def fwd_inner_chunk(
    ...
    p_g = tl.make_block_ptr(
        g + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
    )
...
class FusedChunkGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, scale, initial_state, output_final_state):
        ...
        prepare_qg_kg[grid](
            q, k, g, q_g, k_g, scale, T=T, K=K, BT=BT, BK=BK, num_warps=1
        )
    ...
    def backward(ctx, do, dht=None):
        ...
        bwd_inner_chunk[grid](
            q, k, g, dA2, dq2, dk2, T=T, K=K, BT=BT, BK=BK, num_warps=1, num_stages=3
        )
