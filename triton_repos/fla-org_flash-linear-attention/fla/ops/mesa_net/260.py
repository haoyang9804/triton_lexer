...
def chunk_fwd_mesa_cg_dim64_kernel(
    ...
    b_m = tl.where(
        ...
        safe_exp(b_g[:, None] - b_g[None, :]) * b_beta[None, :],
        0,
    )
    ...
    b_q = tl.load(p_q, boundary_check=(0, 1))
    b_x += b_q / (
        b_h_diagonal[None, :] * b_g_exp_q
        + tl.dot(b_m.to(b_k.dtype), (b_k * b_k).to(b_k.dtype))
        + b_lamb[None, :]
        + 1e-5
    )
    ...
    p_q_final = tl.make_block_ptr(
        q_final, (T, K), (H * K, 1), (i_t * BT, 0), (BT, BK), (1, 0)
    )
...
def chunk_mesa_cg_fwd(
    ...
    assert (
        chunk_size <= 64 or K <= 64
    ), "either chunk size or head dimension must be no greater than 64"
