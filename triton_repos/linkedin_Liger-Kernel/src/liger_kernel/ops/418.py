...
def _sparsemax_backward_kernel(
    ...
    for i in tl.range(0, tl.cdiv(n_cols, BLOCK_SIZE)):
        ...
        o_val = tl.load(
            o_row + offs_iter, mask=mask_iter, other=0.0, cache_modifier=".ca"
        ).to(tl.float32)
    ...
    for i in tl.range(0, tl.cdiv(n_cols, BLOCK_SIZE)):
        ...
        gi_val = tl.where(
            supp,
            go_val
            - tl.cast(go_sum / tl.maximum(supp_cnt, 1e-6), gi_row.dtype.element_ty).to(
                tl.float32
            ),
            0.0,
        )
        tl.store(
            gi_row + offs_iter,
            gi_val.to(gi_row.dtype.element_ty),
            mask=mask_iter,
            cache_modifier=".wb",
        )
