...
def _softmax_multi_block_forward_kernel(
    ...
    for start in tl.range(0, n_cols, BLOCK_SIZE):
        ...
        tl.store(
            Y_ptr + row_id * Y_row_stride + idx, yblk, mask=mask, cache_modifier=".cs"
        )
...
def _softmax_single_block_backward_kernel(
    ...
    y = tl.load(
        y_ptr + row_id * y_stride + offs, mask=mask, other=0.0, cache_modifier=".ca"
    )
...
def _softmax_multi_block_backward_kernel(
    ...
    for start in tl.range(0, n_cols, BLOCK_SIZE):
        ...
        y_blk = tl.load(
            y_ptr + row_id * y_stride + idx, mask=mask, other=0.0, cache_modifier=".ca"
        )
    ...
    for start in tl.range(0, n_cols, BLOCK_SIZE):
        ...
        tl.store(
            dx_ptr + row_id * dx_stride + idx, dx_blk, mask=mask, cache_modifier=".wb"
        )
