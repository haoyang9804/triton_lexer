...
class DropoutKernel:
    ...
    @triton.jit
    def backward(
        grad_input_ptr, grad_output_ptr, output_ptr, size, p, block_size: tl.constexpr
    ):
        pid = tl.program_id(0)
        offset = pid * block_size
...
class DropoutFunction(autograd.Function):
    ...
    def backward(ctx: Any, *grad_outputs: Any) -> Any:
        ...
        DropoutKernel.backward[grid](
            grad_input, grad_output, output, size, ctx.p, block_size
        )
