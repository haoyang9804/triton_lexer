...
def contextual_autotune(is_dist=False, n_repeat=5, n_warmup=3):
    ...
    def decor(fn):
        return ContextualAutoTuner(
            fn, is_dist=is_dist, n_repeat=n_repeat, n_warmup=n_warmup
        )

    return decor


def _do_bench_iterator(
        ...
        try:
            ...
            for j in range(n_repeat):
                ...
                if j < n_repeat - 1:
                    ...
                    times = [
                        (e.synchronize(), s.elapsed_time(e))[-1]
                        for s, e in zip(start_event, end_event)
                    ]
                    yield ret, i, j, triton.testing._summarize_statistics(
...
def _contextual_tuning_run(self: Autotuner, *args, **kwargs):
    ...
    if ctx is None or ctx.finished:
        ...
        bench_fns = [
            _bench_fn(self, *args, config=config, **kwargs) for config in pruned_configs
        ]

        bench_iter = _do_bench_iterator(
            bench_fns,
            n_repeat=ctx_tuner.n_repeat,
            n_warmup=ctx_tuner.n_warmup,
            return_mode="mean",
        )
        ...
        if cfg_i >= len(ctx.configs) - 1:
            ...
            if ctx_tuner.is_dist:
                ...
                torch.distributed.all_reduce(
                    times_tensor, torch.distributed.ReduceOp.MAX
                )
                ctx.config_times = times_tensor.tolist()

            (self.best_config_id, self.best_config), self.best_time = min(
                zip(ctx.okay_configs, ctx.config_times), key=lambda t: t[-1]
            )
