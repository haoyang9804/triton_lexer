...
def parallel_simple_gla_fwd(
    ...
    grid = (NK * NV, NT, B * H)
    o = torch.empty(
        NK, *v.shape, dtype=v.dtype if NK == 1 else torch.float, device=q.device
    )
...
def parallel_simple_gla_bwd(
    ...
    dg = (
        torch.empty(NK * NV, *g.shape, dtype=torch.float, device=q.device)
        if g is not None
        else None
    )
    ...
    dv = dv.sum(0)
    dg = (
        chunk_global_cumsum(dg.sum(0), reverse=True, cu_seqlens=cu_seqlens)
        if g is not None
        else None
    )
    return dq, dk, dv, dg
...
class ParallelSimpleGLAFunction(torch.autograd.Function):
    ...
    def backward(ctx, do, da=None):
        ...
        return (
            ...
            dk.to(k),
            dv.to(v),
            dg.to(ctx.dtype) if dg is not None else None,
            None,
            None,
            None,
        )
...
def parallel_simple_gla(
    ...
    if output_attentions:
        assert (
            cu_seqlens is None
        ), "output_attentions=True is not supported with variable-length sequences"

    if scale is None:
        scale = k.shape[-1] ** -0.5
    o, attn = ParallelSimpleGLAFunction.apply(
        q, k, v, g, scale, output_attentions, cu_seqlens
    )
    if head_first:
