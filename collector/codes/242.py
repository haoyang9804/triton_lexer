...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        causal_type, window_left, window_right = translate_causal(
        ...
        if dropout_p > 0.0:
            ...
            philox_offset1 = torch.tensor(
                [DEFAULT_PHILOX_OFFSET_1], device=q.device, dtype=torch.uint64
            )
            philox_offset2 = DEFAULT_PHILOX_OFFSET_2
            philox_seed_output = torch.tensor([0], device=q.device, dtype=torch.uint64)
            philox_offset_output = torch.tensor(
                [0], device=q.device, dtype=torch.uint64
            )
        else:
        ...
        if return_encoded_softmax:
            ...
            debug_simulate_encoded_softmax[grid](
                encoded_softmax,
    ...
    def backward_split(ctx, do, _, fwd_tuning_result):
        ...
        max_seqlen_k = k.shape[2]
        causal_type, window_left, window_right = translate_causal(
            ctx.causal, max_seqlen_q, max_seqlen_k
        )
        ...
        if k.requires_grad and v.requires_grad and q.requires_grad:
            if ctx.autotune:
                ...
                grid_fuse = lambda META: (
                    triton.cdiv(max_seqlen_k, META["BLOCK_N"])
                    + triton.cdiv(max_seqlen_q, META["BLOCK_N"])
