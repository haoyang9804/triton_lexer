...
def layer_norm_gated_bwd(
    ...
    dx = (
        torch.empty_like(x)
        if x_dtype is None
        else torch.empty(T, D, dtype=x_dtype, device=x.device)
    )
    dg = (
        torch.empty_like(g)
        if x_dtype is None
    ...
    y = (
        torch.empty(T, D, dtype=dy.dtype, device=dy.device)
        if recompute_output
        else None
    )
    ...
    db = (
        torch.empty((NS, D), dtype=torch.float, device=bias.device)
        if bias is not None
        else None
    )
    grid = (NS,)
    ...
    if has_residual and dx.dtype == x.dtype:
        dresidual_in = dx
    return (
        (dx, dg, dw, db, dresidual_in)
        if not recompute_output
        else (dx, dg, dw, db, dresidual_in, y)
    )
...
class LayerNormGatedLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            residual_out, g, norm_weight, norm_bias, linear_weight, mean, rstd
        )
        ctx.x_shape_og = x_shape_og
        ctx.g_shape_og = g_shape_og
        ctx.eps = eps
...
def layer_norm_gated(
    ...
    return LayerNormGatedFunction.apply(
        x, g, weight, bias, activation, residual, eps, prenorm, residual_in_fp32, False
    )
...
def rms_norm_gated(
    ...
    return LayerNormGatedFunction.apply(
        x, g, weight, bias, activation, residual, eps, prenorm, residual_in_fp32, True
    )
