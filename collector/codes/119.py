...
@triton.autotune(
    configs=[
        triton.Config(
            ...
            num_stages=3,
            num_warps=4,
            pre_hook=init_to_zero(["ddA_cumsum_f_ptr", "ddA_cumsum_b_ptr"]),
        ),
        triton.Config(
...
def _chunk_scan_fwd(
    cb, x, dt, dA_cumsum_f, dA_cumsum_b, C, states_f, states_b, D=None, z=None
):
...
def _chunk_scan_bwd_dstates(C, dA_cumsum_f, dA_cumsum_b, dout, dtype=None):
    ...
    with torch.cuda.device(C.device.index):
        _chunk_scan_bwd_dstates_kernel[grid_dstates](
            ...
            C.stride(2),
            C.stride(3),
            dprev_states_f.stride(0),
            dprev_states_f.stride(1),
            dprev_states_f.stride(2),
            dprev_states_f.stride(3),
            dprev_states_f.stride(4),
            dprev_states_b.stride(0),
            dprev_states_b.stride(1),
            dprev_states_b.stride(2),
            dprev_states_b.stride(3),
            dprev_states_b.stride(4),
            dA_cumsum_f.stride(0),
            dA_cumsum_f.stride(2),
            dA_cumsum_f.stride(1),
            dA_cumsum_f.stride(3),
...
def _chunk_scan_bwd_dC(
    prev_states_f, prev_states_b, dA_cumsum_f, dA_cumsum_b, dout, C=None, ngroups=1
):
    ...
    if C is not None:
        ...
        ddA_cumsum_prev_f = torch.empty(
            batch, nheads, nchunks, chunk_size, device=dout.device, dtype=torch.float32
        )
        ddA_cumsum_prev_f_strides = (
            ddA_cumsum_prev_f.stride(0),
            ddA_cumsum_prev_f.stride(2),
            ddA_cumsum_prev_f.stride(1),
            ddA_cumsum_prev_f.stride(3),
        )
        ddA_cumsum_prev_b = torch.empty(
            batch, nheads, nchunks, chunk_size, device=dout.device, dtype=torch.float32
        )
        ddA_cumsum_prev_b_strides = (
            ddA_cumsum_prev_b.stride(0),
            ddA_cumsum_prev_b.stride(2),
            ddA_cumsum_prev_b.stride(1),
            ddA_cumsum_prev_b.stride(3),
        )
    else:
        C_strides = (0, 0, 0, 0)
    ...
    with torch.cuda.device(dout.device.index):
        _chunk_scan_bwd_dc_kernel[grid_dc](
            ...
            dout.stride(2),
            dout.stride(3),
            prev_states_f.stride(0),
            prev_states_f.stride(1),
            prev_states_f.stride(2),
            prev_states_f.stride(3),
            prev_states_f.stride(4),
            prev_states_b.stride(0),
            prev_states_b.stride(1),
            prev_states_b.stride(2),
            prev_states_b.stride(3),
            prev_states_b.stride(4),
            *C_strides,
            dA_cumsum_f.stride(0),
            dA_cumsum_f.stride(2),
            dA_cumsum_f.stride(1),
...
def _chunk_scan_bwd_ddAcs_stable_fwd(x, dt, dA_cumsum, dout, cb):
    ...
    BLOCK_SIZE_M_actual = _chunk_scan_bwd_ddAcs_stable_fwd_kernel.best_config.kwargs[
        "BLOCK_SIZE_M"
    ]
    n_valid_blocks = (chunk_size + BLOCK_SIZE_M_actual - 1) // BLOCK_SIZE_M_actual
    ddA_cumsum = ddA_cumsum[:, :, :, :n_valid_blocks].sum(dim=3)
...
def _chunk_scan_bwd_ddAcs_stable_bwd(x, dt, dA_cumsum, dout, cb):
    ...
    BLOCK_SIZE_M_actual = _chunk_scan_bwd_ddAcs_stable_bwd_kernel.best_config.kwargs[
        "BLOCK_SIZE_M"
    ]
