...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "context_attn_llama", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 12):
            ...
            q = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype).normal_(
                mean=0.1, std=0.2
            )
            k = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype).normal_(
                mean=0.4, std=0.2
            )
            v = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype).normal_(
                mean=0.3, std=0.2
            )
            o = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype).normal_(
                mean=0.3, std=0.2
            )
    ...
    def to_cuda(self, input_tensor):
        ...
        return tuple(
            [
                ...
                b_start_loc.cuda(),
                b_seq_len.cuda(),
                b_prompt_cache_len.cuda(),
                max_input_len,
                req_to_token_indexs.cuda(),
            ]
        )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        total_bytes = 4 * q.numel() * q.element_size()
        GBPS = total_bytes / (runtime / 1000) / 1e9
        return GBPS

    def get_tflops(self, input_tensor, runtime):
        q, _, _, _, _, _, _, _, _, _ = input_tensor
        FLOPS = 2 * q.size(0) * q.size(1) * q.size(2)
