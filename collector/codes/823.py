...
from TritonBench_v1.matmul_dequantize_int4 import (
...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "matmul_dequantize_int4", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_gbps(self, input_tensor, runtime):
        ...
        mem_access = (
            ...
            + int_b.numel() * int_b.element_size()
            + b_scale.numel() * b_scale.element_size()
            + b_zero_point.numel() * b_zero_point.element_size()
            + a.shape[0] * int_b.shape[1] * 2
        )
