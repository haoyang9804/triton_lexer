...
class MyTestCase(unittest.TestCase):
    def test_1_validation(self):
        ...
        self.lsh = AngularLSHTriton(num_projs=num_projs, dim=(1, 1, dim)).to(
        ...
        print(
            "difference between torch and triton hashes: ",
            (query_hash_buckets.float() - query_hash_buckets_triton.float()).norm(),
        )
    ...
    def test_2_runtime(self):
        ...
        tim_py_q20, tim_py_q50, tim_py_q80 = triton.testing.do_bench(
            test_fn1, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8]
        )
        print(
            f"pytorch runtime: {tim_py_q50:.5f} ms ({tim_py_q20:.5f}, {tim_py_q80:.5f})"
        )

        def test_fn2():
            query_hash_buckets_triton = self.lsh.hash_triton(query)

        tim_tr_q20, tim_tr_q50, tim_tr_q80 = triton.testing.do_bench(
            test_fn2, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8]
        )
        print(
            f"triton  runtime: {tim_tr_q50:.5f} ms ({tim_tr_q20:.5f}, {tim_tr_q80:.5f})"
        )
