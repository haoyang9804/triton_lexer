...
def benchmark(T, provider):
    ...
    if provider == "gsa_recurrent":
        return triton.testing.do_bench(
            lambda: fused_recurrent_gsa(q, k, v, s, f), quantiles=quantiles
        )
    if provider == "gsa_chunk":
        return triton.testing.do_bench(
            lambda: chunk_gsa(q, k, v, s, f), quantiles=quantiles
        )
    elif provider == "gla":
        return triton.testing.do_bench(
        ...
        return triton.testing.do_bench(
            lambda: fused_recurrent_gsa(q, k, v, s, f)[0].backward(do),
            quantiles=quantiles,
        )
    elif provider == "gsa_chunk_bwd":
        return triton.testing.do_bench(
            lambda: chunk_gsa(q, k, v, s, f)[0].backward(do), quantiles=quantiles
        )
