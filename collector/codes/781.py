...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "fused_activation", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(12, 30):
            ...
            bias = torch.rand(size // 4, dtype=torch.float16)
    ...
    def get_gbps(self, input_tensor, runtime):
        in_out_tensor, bias, in_tensor = input_tensor
        total_bytes = (
            in_out_tensor.numel() + bias.numel() + in_tensor.numel()
        ) * in_out_tensor.element_size()
        GBPS = total_bytes / (runtime / 1000) / 1e9
        return GBPS

    def get_tflops(self, input_tensor, runtime):
        in_out_tensor, bias, in_tensor = input_tensor
        FLOPS = 2 * in_out_tensor.numel()
