...
def get_max_tensorcore_tflops(dtype, clock_rate_mhz, device=None):
    ...
    logging.info(
        f"num_subcores: {num_subcores} clock_rate: {clock_rate_mhz} ops_per_sub_core: {ops_per_sub_core}"
    )
    return tflops
...
def get_tflops_approx(
    device: torch.dtype, num_ctas: int, num_warps: int, dtype: torch.dtype
):
...
def get_tensorcore_dtype_support(device_id=0):
    ...
    DTYPE_MAP = {
        (8, 0): [torch.float16, torch.bfloat16, torch.float32, torch.int8],
        (8, 6): [torch.float16, torch.bfloat16, torch.float32, torch.int8],
        (8, 9): [
            torch.float16,
            torch.bfloat16,
            torch.float32,
