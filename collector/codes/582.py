...
class TorchMultiTokenAttention(torch.nn.Module):
    def __init__(self, C_in, C_out, K, groups, bias, dtype, device):
        ...
        self.weight = torch.nn.Parameter(
            torch.empty(C_out, C_in // groups, K, K, dtype=dtype, device=device)
        )
        self.bias = (
            torch.nn.Parameter(torch.empty(C_out, dtype=dtype, device=device))
            if bias
            else None
        )
        self.K = K
...
def bench_speed_multi_token_attention(
    ...
    if mode == "forward":
        ms_50, ms_20, ms_80 = triton.testing.do_bench(
            fwd, grad_to_none=[x], rep=100, quantiles=QUANTILES
        )
    elif mode == "backward":
        y = fwd()
        ms_50, ms_20, ms_80 = triton.testing.do_bench(
        ...
        ms_50, ms_20, ms_80 = triton.testing.do_bench(
            full, grad_to_none=[x], rep=100, quantiles=QUANTILES
        )
...
def bench_memory_multi_token_attention(
    input: SingleBenchmarkRunInput,
) -> SingleBenchmarkRunOutput:
    L = input.x
    provider = input.kernel_provider
