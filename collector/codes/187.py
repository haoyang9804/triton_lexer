...
@triton.jit
def rms_kernel(
    audios, audios_real_lens, audios_max_len, batch_idx, BLOCK_SIZE_RMS: tl.constexpr
):
    ...
    for offset in range(0, audios_max_len, BLOCK_SIZE_RMS):
        ...
        audios_vals = tl.load(
            audios + batch_idx * audios_max_len + audios_block_ptr, mask=audios_mask
        )
...
@triton.autotune(
    configs=[
        ...
        for (block_size_sum, num_warps) in itertools.product([512, 1024], [2, 4, 8, 16])
    ],
...
def sum_with_snr_triton(
    ...
    assert (
        samples.is_contiguous() and samples_noise.is_contiguous()
    ...
    sum_with_snr_kernel[grid](
        ...
        output,
        BLOCK_SIZE_RMS=max(
            1024, triton.next_power_of_2(max(T, samples_noise.shape[1]) // 1024)
        ),
    )
