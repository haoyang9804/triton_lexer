...
def cp_engine_producer_all_gather_full_mesh_push(
    ...
    with torch.cuda.stream(stream):
        for dst_rank in push_order:
            dst = remote_tensor_buffers[dst_rank][
...
def cp_engine_producer_all_gather_ring_push_numa_2d(
    ...
    NUMA_WORLD_SIZE = get_numa_world_size()
    assert (
        num_ranks % NUMA_WORLD_SIZE == 0
    ), f"num_ranks {num_ranks} should be divisible by NUMA {NUMA_WORLD_SIZE}"
    ...
    ) % NUMA_WORLD_SIZE + numa_node_id * NUMA_WORLD_SIZE
    with torch.cuda.stream(stream):
        if for_correctness:
    ...
    with torch.cuda.stream(intranode_stream):
        ...
        for n in range(nnodes):
            ...
            if n != 0:
                ...
                rank_to_inter_node = (
                    rank - n * num_local_ranks + num_ranks
                ) % num_ranks
                segment = rank
...
def cp_engine_producer_all_gather_full_mesh_pull_inter_node(
    ...
    with torch.cuda.stream(intranode_ag_stream):
        ...
        for i in range(1, n_nodes):
            recv_rank = (
                local_rank + (node_rank + n_nodes - i) % n_nodes * local_world_size
            )
