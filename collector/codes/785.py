...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "fused_recurrent_retention", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def call_op(self, input_tensor):
        ...
        return fused_recurrent_retention(
            q, k, v, initial_state, output_final_state=True
        )

    def get_gbps(self, input_tensor, runtime):
        q, k, v, initial_state = input_tensor
        total_bytes = (
            q.numel() + k.numel() + v.numel() + initial_state.numel()
        ) * q.element_size()
