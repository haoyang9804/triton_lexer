...
def benchmark(T, provider):
    ...
    g = (
        F.logsigmoid(torch.randn(B, H, T, D, device=device, dtype=dtype))
        .clamp_min(-5)
    ...
    if provider == "torch":
        ...
        results = triton.testing.do_bench(
            lambda: naive_retention(q, k, v), quantiles=quantiles
        )
    elif provider == "recurrent_gla":
        results = triton.testing.do_bench(
            lambda: fused_recurrent_gla(q, k, v, g), quantiles=quantiles
        )
    elif provider == "fused_chunk_gla":
        results = triton.testing.do_bench(
            lambda: fused_chunk_gla(q, k, v, g), quantiles=quantiles
        )
    elif provider == "chunk_retention":
        results = triton.testing.do_bench(
            lambda: chunk_retention(q, k, v), quantiles=quantiles
        )
        ...
        results = triton.testing.do_bench(
            lambda: parallel_retention(q, k, v), quantiles=quantiles
        )
        ...
        results = triton.testing.do_bench(
            lambda: fused_recurrent_gla(q, k, v, g)[0].backward(do), quantiles=quantiles
        )
    elif provider == "fused_chunk_gla_bwd":
        results = triton.testing.do_bench(
            lambda: fused_chunk_gla(q, k, v, g)[0].backward(do), quantiles=quantiles
        )
        ...
        results = triton.testing.do_bench(
            lambda: parallel_retention(q, k, v)[0].backward(do), quantiles=quantiles
        )
    return results
