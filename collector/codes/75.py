...
def cp_engine_producer_all_gather_put(
    local_tensor,
    ag_buffer,
    signal_buffer,
    M_per_rank,
    ...
    for i in range(1, local_world_size):
        ...
        src_ptr = (
            ag_buffer[local_rank].data_ptr() + segment * local_tensor.element_size()
        )
        dst_ptr = (
            ag_buffer[local_dst_rank].data_ptr() + segment * local_tensor.element_size()
        )
    ...
    for i in range(1, n_nodes):
        ...
        for j in range(1, local_world_size):
            ...
            dst_ptr = (
                ag_buffer[local_dst_rank].data_ptr()
                + recv_segment * local_tensor.element_size()
            )
...
def ag_gemm_persistent_op(
    ...
    num_ag_sms = n_nodes - 1
    num_gemm_sms = (
        torch.cuda.get_device_properties("cuda").multi_processor_count - num_ag_sms
    )
...
if __name__ == "__main__":
    ...
    set_signal(
        ctx.barrier_tensor[rank].data_ptr(), 1, torch.cuda.current_stream(), True
    )

    ag_gemm_persistent_op(
        A,
        B,
        C,
        ctx.rank,
