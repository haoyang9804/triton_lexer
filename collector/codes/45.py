...
class AllToAllContext:
    ...
    def __init__(
        ...
        self.recv_buf = pynvshmem.nvshmem_create_tensor(
            [WORLD_SIZE * max_m * 2, hidden], dtype
        )
        self.scale_send_buf = pynvshmem.nvshmem_create_tensor([max_m], scale_dtype)
        self.scale_recv_buf = pynvshmem.nvshmem_create_tensor(
            [WORLD_SIZE * max_m * 2], scale_dtype
        )
        self.split_send_buf = pynvshmem.nvshmem_create_tensor(
            [num_tot_experts], torch.int32
        )
        self.split_recv_buf = pynvshmem.nvshmem_create_tensor(
            [num_tot_experts * 2], torch.int32
        )
