...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        + ([("cyan", "-"), ("cyan", "dotted")] if HAS_FLASH else []),
...
def benchmark(T, provider):
    ...
    if provider == "torch":
        ...
        if T > 1024:
            return results
        results = triton.testing.do_bench(
            lambda: naive_parallel_based(q, k, v).backward(do), quantiles=quantiles
        )
    elif provider == "fused_chunk_bwd":
        results = triton.testing.do_bench(
            lambda: fused_chunk_based(q, k, v).backward(do), quantiles=quantiles
        )
        ...
        results = triton.testing.do_bench(
            lambda: naive_chunk_based(q, k, v).backward(do), quantiles=quantiles
        )
    return results
