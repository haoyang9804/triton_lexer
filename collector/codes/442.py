...
def test_parallel(
    ...
    q = (
        torch.linspace(0, 1, steps=T, dtype=dtype, device="cuda")[perm_q]
        .view(1, T, 1, 1)
        .expand(B, T, HQ, D)
        .clone()
        .requires_grad_(True)
    ...
    k = (
        torch.linspace(0, 1, steps=T, dtype=dtype, device="cuda")[perm_k]
        .view(1, T, 1, 1)
        .expand(B, T, H, D)
        .clone()
        .requires_grad_(True)
    ...
    v = (
        torch.linspace(0, 1, steps=T, dtype=dtype, device="cuda")[perm_v]
        .view(1, T, 1, 1)
        .expand(B, T, H, D)
        .clone()
        .requires_grad_(True)
