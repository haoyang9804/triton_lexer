...
class TestFusedMLP:
    ...
    def test_triton_vs_pytorch_accuracy(self, N, lin_layer):
        ...
        assert torch.allclose(
            output_triton, ref_mlp_out, atol=1e-1
        ), f"max diff: {torch.max(torch.abs(output_triton - ref_mlp_out))}"

        print(
            "triton",
            triton.testing.do_bench(
                lambda: triton_llama_mlp(
                    x=x, lin1=lin1_w, lin2=lin2_w, rms_wts=rms_wts
                ),
                warmup=50,
            ),
        )
        print(
            "pytorch",
            triton.testing.do_bench(
                lambda: mlp_pytorch(x=x, lin1=lin1_w, lin2=lin2_w, rms_wts=rms_wts)
            ),
        )
