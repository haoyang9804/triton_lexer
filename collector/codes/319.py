...
class FlashAttention:
    ...
    def _compute_triton(
        ...
        if adjust_sharindgs:
            ...
            target_gpu_idx = int(
                os.environ.get("GPU_IDX_FLASH_ATTN", free_gpu_in_process())
            )
            devices = jax.local_devices(
                process_index=jax.process_index(), backend="gpu"
            )
    ...
    def __repr__(self):
        return f"FlashAttention(platform={self.config.platform}, backend={self.config.backend})"
