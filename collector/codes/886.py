...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "token_attn_llama2", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(5, 16):
            ...
            self.input_tensors.append(
                (q, k, att_out, B_Loc, B_Start_Loc, B_Seqlen, max_input_len)
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        q, k, att_out, B_Loc, B_Start_Loc, B_Seqlen, max_input_len = input_tensor
        total_bytes = (
            q.numel() + k.numel() + att_out.numel()
        ) * q.element_size() + B_Loc.numel() * B_Loc.element_size()
