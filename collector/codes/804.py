...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "kv_cache_copy", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 18):
            ...
            self.input_tensors.append(
                (k, v, k_cache, v_cache, kv_lengths, block_tables)
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        k, v, k_cache, v_cache, kv_lengths, block_tables = input_tensor
        total_bytes = (
            k.numel() + v.numel() + k_cache.numel() + v_cache.numel()
        ) * k.element_size()
