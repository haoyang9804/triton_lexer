...
def attn_fwd(
    ...
    tl.static_assert(
        BIAS_TYPE == 0 or BIAS_TYPE == 1, f"Unsupported BIAS_TYPE {BIAS_TYPE}"
    )
    BATCH = tl.num_programs(2)
    L_not_null = L.cast(dtype=tl.uint64, bitcast=True) != 0
    ...
    if PERSISTENT and not unsupported_by_persistent:

        num_tiles_per_head = tl.cdiv(Max_seqlen_q, BLOCK_M)
        num_tiles_per_sample = num_tiles_per_head * Num_head_q
        num_tiles_total = num_tiles_per_sample * Batch
        if PERSISTENT_DYNAMIC:
            tile_id = persistent_atomic_counter.atomic_add(1)
        else:
            tile_id = tl.program_id(0)

    continue_condition: tl.int1 = True
        ...
        if continue_condition:
            ...
            if IS_CAUSAL:
                ...
                if (lb_empty and fb_empty) and rb_empty:
                    acc0, acc1, acc2 = composed_zeros_2d(
                        BLOCK_M,
                        BLOCK_DMODEL0,
                        BLOCK_DMODEL1,
                        BLOCK_DMODEL2,
                    ...
                    if L_not_null:
                        l_ptrs = (
                            L
                            + off_z * Num_head_q * Max_seqlen_q
                            + off_h_q * Max_seqlen_q
                            + offs_m
                        )
            ...
            if continue_condition:
                ...
                if ENABLE_DROPOUT:
                    batch_philox_offset = (
                        philox_offset_base
                        + off_zh * Max_seqlen_q * philox_offset_stride
                    )
                else:
                ...
                if RETURN_ENCODED_SOFTMAX:
                    encoded_sm_base = (
                        encoded_softmax + off_zh * Max_seqlen_q * Max_seqlen_k
                    )
                else:
                ...
                if IS_CAUSAL:
                    if causal_start_idx > start_M and causal_start_idx < end_M:
                        ...
                        z = 0.0
                        acc0, acc1, acc2 = composed_casual_mask(
                            acc0,
                            acc1,
                            acc2,
                            mask_m_offsets,
