...
def layer_norm_kernel(
    x_ptr, weight_ptr, bias_ptr, y_ptr, N, eps: tl.constexpr, BLOCK_SIZE: tl.constexpr
):
    row_idx = tl.program_id(0)
...
def cross_entropy_loss_kernel(
    logits_ptr, targets_ptr, loss_ptr, n_classes, n_elements, BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(0)
...
class TritonSoftmax(nn.Module):
    def forward(self, x):
        ...
        grid = lambda meta: (B,)
        softmax_kernel[grid](
            y, x, x.stride(0), y.stride(0), N, BLOCK_SIZE=triton.next_power_of_2(N)
        )
        y = y + 1e-8
...
class TritonCrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets):
        ...
        cross_entropy_loss_kernel[grid](
            logits, targets, loss, n_classes, n_elements, BLOCK_SIZE=1024
        )
...
class TritonLayerNorm(nn.Module):
    ...
    def forward(self, x):
        assert (
            x.shape[-len(self.normalized_shape) :] == self.normalized_shape
        ), "Input shape does not match normalized_shape."
        y = torch.empty_like(x)
        ...
        layer_norm_kernel[grid](
            x_, self.weight, self.bias, y_, N, eps=self.eps, BLOCK_SIZE=128
        )
        return y
...
class NanoGPT(nn.Module):
    ...
    def forward(self, idx):
        ...
        assert (
            T <= self.seq_length
        ), f"Input sequence length {T} exceeds model's maximum sequence length {self.seq_length}"
    ...
    def compute_loss(self, logits, targets):
        return triton_cross_entropy_loss(
            logits.view(-1, logits.size(-1)), targets.view(-1)
        )


def train(
    model, train_data, val_data, batch_size, seq_length, learning_rate, num_epochs
):
    ...
    def get_batch(split):
        ...
        return x.to(model.token_embedding.weight.device), y.to(
            model.token_embedding.weight.device
        )
    ...
    for epoch in range(num_epochs):
        for _ in range(100):
            ...
            if iter_num % 10 == 0:
                ...
                print(
                    f"iter {iter_num}: loss {loss.item():.4f}, time {dt*1000:.2f}ms, mfu {mfu*100:.2f}%"
                )
