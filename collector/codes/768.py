...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "dropout_triton", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 30):
            ...
            x_keep = torch.rand(size, dtype=torch.float16) > 0.5
    ...
    def get_gbps(self, input_tensor, runtime):
        x, x_keep = input_tensor
        total_bytes = (
            x.numel() * x.element_size() * 2 + x_keep.numel() * x_keep.element_size()
        )
