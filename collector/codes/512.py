...
class Attention:
    ...
    def forward(
        ...
        if dropout_p > language.eps:
            dropout_mask = (
                tl.rand(seed, tl.arange(0, x_block_size) + y_offset) > dropout_p
            )
    ...
    def backward(
        ...
        for n_block in range(0, num_y_blocks):
            ...
            for m_offset in range(n_offset, y_size, y_block_size):
                ...
                score += (
                    language.dot(query, tl.trans(key), use_accelerator, dtype)
                    * score_scale
                )
                ...
                grad_value += language.dot(
                    tl.trans(alpha), grad_output, use_accelerator, dtype
                )
                delta = tl.load(delta_block_ptr)
                grad_alpha = (
                    tl.zeros((y_block_size, y_block_size), dtype) - delta[:, None]
                )
                grad_alpha += language.dot(
                    grad_output, tl.trans(value), use_accelerator, dtype
                )
                grad_softmax = (alpha * grad_alpha * softmax_scale).to(dtype)
                grad_key += language.dot(
                    tl.trans(grad_softmax), query, use_accelerator, dtype
                )
                ...
                grad_query_block_ptr = tl.advance(
                    grad_query_block_ptr, (y_block_size, 0)
                )
                grad_output_block_ptr = tl.advance(
                    grad_output_block_ptr, (y_block_size, 0)
                )
                ...
                if mask_ptr is not None:
                    tl.store(
                        grad_mask_block_ptr, (grad_softmax / softmax_scale).to(dtype)
                    )
                    mask_block_ptr = tl.advance(mask_block_ptr, (y_block_size, 0))
                    grad_mask_block_ptr = tl.advance(
                        grad_mask_block_ptr, (y_block_size, 0)
                    )
