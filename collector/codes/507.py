...
def linear_configs(
    m_block_size_list: List[int],
    n_block_size_list: List[int],
    k_block_size_list: List[int],
):
    configs = []
    for m_block_size in m_block_size_list:
        for n_block_size in n_block_size_list:
            for k_block_size in k_block_size_list:
                num_warps, num_stages = num_warps_and_stages_for_linear(
...
class Linear:
    @staticmethod
    @util.autotune(
        linear_configs([16, 64, 128], [32, 64, 128], [32, 64]),
        ["m_size", "n_size", "k_size"],
    )
    @triton.heuristics(
        {
            "require_m_boundary_check": lambda args: args["m_size"]
            % args["m_block_size"],
            "require_n_boundary_check": lambda args: args["n_size"]
            % args["n_block_size"],
            "require_k_boundary_check": lambda args: args["k_size"]
            % args["k_block_size"],
        }
    )
    @triton.jit
    ...
    @staticmethod
    @util.autotune(
        linear_backward_configs([64, 128], [32, 64], [32, 64, 128]),
        ["m_size", "n_size", "k_size"],
    )
    @triton.heuristics(
        {
