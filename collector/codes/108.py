...
from ssd.bi.ssd_combined import (
    _mamba_chunk_scan_combined_bwd as _mamba_chunk_scan_combined_bwd_bi,
)
...
def bi_fwd(dout, out, x, dt, A, B, C, D, delta_bias, z, chunk_size, delta_softplus):
    dx, ddt, dA, dB, dC, dD, dz, ddt_bias = _mamba_chunk_scan_combined_bwd_bi(
        dout=dout,
        out=out,
        x=x,
        dt=dt,
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_log=True,
        line_arg="provider",
        line_vals=["Causal Mamba2", "Bi-Mamba2"],
        line_names=["Causal Mamba2", "Bi-Mamba2"],
        styles=[("blue", "-"), ("green", "-")],
        ylabel="ms",
        plot_name="Bidirectional Mamba Performance",
        args={},
    )
)
def benchmark(seqlen, provider):
    dout, out, x, dt, A, B, C, D, delta_bias, z, chunk_size, delta_softplus = init(
        seqlen
    )
    ...
    if provider == "Causal Mamba2":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: uni_fwd(
                dout, out, x, dt, A, B, C, D, delta_bias, z, chunk_size, delta_softplus
            ),
            quantiles=quantiles,
            rep=2000,
            warmup=500,
