...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "lora_expand_gemv", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(4, 16):
            ...
            lora_b_weights = torch.rand(
                (lora_num, rank, hidden_size), dtype=torch.float16
            )
    ...
    def to_cuda(self, input_tensor):
        ...
        return (
            ...
            lora_b_weights.cuda(),
            output_tensor.cuda(),
            lora_indices_tensor.cuda(),
        )

    def call_op(self, input_tensor):
        inputs, lora_b_weights, output_tensor, lora_indices_tensor = input_tensor
        _bgmv_expand(
            inputs, lora_b_weights, output_tensor, lora_indices_tensor, add_inputs=True
        )
    ...
    def get_tflops(self, input_tensor, runtime):
        inputs, lora_b_weights, output_tensor, lora_indices_tensor = input_tensor
        FLOPS = 2 * inputs.size(0) * inputs.size(1) * lora_b_weights.size(2)
