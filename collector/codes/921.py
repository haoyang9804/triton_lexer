    ...
    def get_do_bench_config(self, warmup=None, rep=None):
        ...
        if self.input_tensors == []:
            raise NotImplementedError(
                "You must implement this method to get input_tensors"
            )
        ...
        for t in range(1, 11):
            ...
            if previous_ms is not None:
                relative_change = (
                    abs(ms - previous_ms) / abs(previous_ms)
                    if previous_ms != 0
                    else float("inf")
                )
        ...
        raise NotImplementedError(
            "You must implement this method to make the runtime stable"
        )
    ...
    def run_benchmark(self):
        ...
        for input_tensor_ in self.input_tensors:
            try:
                ...
                result = {
                    "input_size": [
                        item.shape if type(item) == torch.Tensor else item
