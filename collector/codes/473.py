...
def patching_kernel(
    ...
    img_g = tl.load(
        image_ptr + batch_offset + data_offset + image_stride0, mask=data_mask
    )
    img_b = tl.load(
        image_ptr + batch_offset + data_offset + image_stride0 * 2, mask=data_mask
    )
    ...
    tl.store(
        out_ptr + batch_offset + out_offset + P * P, tl.ravel(img_g), mask=out_mask
    )
    tl.store(
        out_ptr + batch_offset + out_offset + P * P * 2, tl.ravel(img_b), mask=out_mask
    )
...
def patching_triton(image: torch.Tensor, patch_size: int) -> torch.Tensor:
    ...
    assert (
        len(image.shape) == 4
    ), "The provided matrix for patching should be 4 dimensional (B, C, H, W)"

    B, C, H, W = image.shape
    N = int((H * W) / (patch_size * patch_size))

    block_x, block_y = triton.next_power_of_2(patch_size), triton.next_power_of_2(
        patch_size
    )
    grid = lambda meta: (B, triton.cdiv(H, block_y), triton.cdiv(W, block_x))

    output = torch.empty(size=(B, N, patch_size * patch_size * C)).to(
...
def patching_torch(matrix: torch.Tensor, patch_size: int) -> torch.Tensor:
    patches = matrix.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)
...
if __name__ == "__main__":
    ...
    print(
        f"Batch size: {batch_size}, Height: {height}, width: {width}, channels: {channels}, P: {patch_size}"
    )

    A = (
        torch.arange(1, batch_size * height * width * channels + 1, dtype=torch.float32)
    ...
    assert torch.allclose(
        patches_pytorch, patches_triton, atol=1e-2, rtol=0
    ), "Data does not match"
    ...
    def benchmark(batch_size, height, patch_size, provider):
        ...
        a = (
            torch.arange(
                1, batch_size * height * width * channels + 1, dtype=torch.float32
            )
            .view(batch_size, channels, height, width)
            .to(device)
        )
        ...
        if provider == "triton":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: patching_triton(a, patch_size), quantiles=quantiles
            )
        if provider == "torch":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: patching_torch(a, patch_size), quantiles=quantiles
            )

        def gbps(ms):
            return 2 * a.nelement() * a.element_size() * 1e-9 / (ms * 1e-3)

        return gbps(ms), gbps(max_ms), gbps(min_ms)

    benchmark.run(print_data=True, save_path="./benchmarks/patching/")
