...
def _chunk_cumsum_fwd(
    ...
    dA_f_cumsum = torch.empty(
        batch, nheads, nchunks, chunk_size, device=dt.device, dtype=torch.float32
    )
    dA_b_cumsum = torch.empty(
        batch, nheads, nchunks, chunk_size, device=dt.device, dtype=torch.float32
    )
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_kernel[grid_chunk_cs](
            ...
            dt_out.stride(1),
            dt_out.stride(3),
            dA_f_cumsum.stride(0),
            dA_f_cumsum.stride(2),
            dA_f_cumsum.stride(1),
            dA_f_cumsum.stride(3),
            dA_b_cumsum.stride(0),
            dA_b_cumsum.stride(2),
            dA_b_cumsum.stride(1),
            dA_b_cumsum.stride(3),
            dt_softplus,
...
def _chunk_state_bwd_db(
    x, dt, dA_cumsum_f, dA_cumsum_b, dstates_f, dstates_b, B=None, ngroups=1
):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    ...
    if B is not None:
        ...
        ddA_cumsum_f = torch.empty(
            batch, nheads, nchunks, chunk_size, device=x.device, dtype=torch.float32
        )
        ddA_cumsum_f_strides = (
            ddA_cumsum_f.stride(0),
            ddA_cumsum_f.stride(2),
            ddA_cumsum_f.stride(1),
            ddA_cumsum_f.stride(3),
        )
        ddA_cumsum_b = torch.empty(
            batch, nheads, nchunks, chunk_size, device=x.device, dtype=torch.float32
        )
        ddA_cumsum_b_strides = (
            ddA_cumsum_b.stride(0),
            ddA_cumsum_b.stride(2),
            ddA_cumsum_b.stride(1),
            ddA_cumsum_b.stride(3),
        )
    else:
        B_strides = (0, 0, 0, 0)
        ddA_cumsum_f = None
