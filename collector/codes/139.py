...
@pytest.fixture
def create_chunk_state_scan_tensors(
...
def chunk_state_compare(create_chunk_state_scan_tensors):
    ...
    B_ref, x_ref, delta_ref, dA_cumsum_f_ref, dA_cumsum_b_ref = create_ref_tensors(
        *create_chunk_state_scan_tensors
    )

    states_f = _chunk_state_fwd(B, x, delta, dA_cumsum_f)
    states_b = _chunk_state_fwd(B, x, delta, dA_cumsum_b, reverse=True)

    states_f_ref, states_b_ref = _chunk_state_fwd_bi(
        B_ref, x_ref, delta_ref, dA_cumsum_f_ref, dA_cumsum_b_ref
    )
