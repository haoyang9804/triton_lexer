...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        x_names=["T", "D"],
        x_vals=[
            (128 * 2**i, d) for d in [256, 512, 1024, 2048, 4096] for i in range(1, 10)
        ],
        line_arg="provider",
        line_vals=[
            "causal_conv1d_fwd",
            "causal_conv1d_cuda_fwd",
            "causal_conv1d_fwdbwd",
            "causal_conv1d_cuda_fwdbwd",
        ],
        line_names=[
            "causal_conv1d_fwd",
            "causal_conv1d_cuda_fwd",
...
def benchmark(T, D, provider):
    ...
    if provider.startswith("causal_conv1d_fwd"):
        results = triton.testing.do_bench(
            lambda: causal_conv1d(
                x, weight, bias, activation="swish", cu_seqlens=cu_seqlens
            ),
            quantiles=quantiles,
        )
        ...
        results = triton.testing.do_bench(
            lambda: rearrange(
                causal_conv1d_fn(
                    ...
                    bias=bias,
                    activation="swish",
                    seq_idx=prepare_sequence_ids(cu_seqlens)
                    .to(torch.int32)
                    .unsqueeze(0),
                ),
                "b d t -> b t d",
