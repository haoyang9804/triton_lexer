...
def conv2d_triton(
    ...
    assert (
        len(input.shape) == 4
    ), f"Input needs to be 4 dimensional, provided: {input.shape}"
    assert (
        len(kernel.shape) == 4
    ), f"Kernel size needs to be 4 dimensional, provided: {kernel.shape}"
    assert (
        bias.shape[0] == kernel.shape[0]
    ...
    assert (
        height % kernel_height == 0 and width % kernel_width == 0
    ...
    output = torch.empty(
        (batch_size, num_kernels, height // kernel_height, width // kernel_width),
        device=device,
        dtype=dtype,
    )
...
if __name__ == "__main__":
    ...
    input = torch.randint(0, 10, (batch_size, channels, height, width)).to(
        device, dtype
    )
    kernel = torch.randint(0, 10, (kernels, channels, kernel_height, kernel_width)).to(
        device, dtype
    )
    ...
    def benchmark(batch_size, kernels, provider):
        ...
        input = torch.randint(0, 5, (batch_size, channels, height, width)).to(
            device, dtype
        )
        kernel = torch.randint(
            0, 5, (kernels, channels, kernel_height, kernel_width)
        ).to(device, dtype)
        bias = torch.randn(kernels).to(device, dtype)
        ...
        if provider == "triton":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: conv2d_triton(input, kernel, bias), quantiles=quantiles
            )
        if provider == "torch":
            ms, min_ms, max_ms = triton.testing.do_bench(
                lambda: conv_layer(input), quantiles=quantiles
            )

        def gbps(ms):
            return 2 * (input.nelement()) * input.element_size() * 1e-9 / (ms * 1e-3)

        return gbps(ms), gbps(max_ms), gbps(min_ms)

    benchmark.run(print_data=True, save_path="./benchmarks/conv2d/")
