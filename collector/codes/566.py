...
class TestRMSNorm:
    ...
    def test_triton_vs_pytorch_accuracy(self, N):
        ...
        layer_weight = torch.ones(
            layer_weight_size, requires_grad=False, device="cuda", dtype=torch.float32
        )
        ...
        print(
            f"Timing: {triton_time=}, {native_rms_time=}, faster = {(triton_time-native_rms_time)/native_rms_time*-100}"
        )

        assert_expected(
            triton_out[0:5000, ...], expected_rms[0:5000, ...], rtol=0.01, atol=0.0001
        )

        dy = 0.2 * torch.randn_like(sample_x)
