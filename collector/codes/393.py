...
def chunk_gla_fwd(
    ...
    A = chunk_gla_fwd_intra_gk(
        q=q, k=k, g=g_cumsum, scale=scale, cu_seqlens=cu_seqlens, chunk_size=BT
    )
...
def chunk_gla_bwd(
    ...
    dv = chunk_gla_bwd_dv(
        k=k, g=g_cumsum, A=A, do=do, dh=dh, cu_seqlens=cu_seqlens, chunk_size=BT
    )

    dA = chunk_gla_bwd_dA(v=v, do=do, scale=scale, cu_seqlens=cu_seqlens, chunk_size=BT)
    dq, dk = chunk_gla_bwd_dqk_intra(
        q=q, k=k, g=g_cumsum, dA=dA, cu_seqlens=cu_seqlens, chunk_size=BT
    )
...
def chunk_gla(
    ...
    o, final_state = ChunkGLAFunction.apply(
        q, k, v, g, scale, initial_state, output_final_state, cu_seqlens
    )
