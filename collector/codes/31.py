...
NVSHMEMI_AMO_SIGNAL_SET = NVSHMEM_SIGNAL_SET
NVSHMEMI_AMO_SIGNAL_ADD = NVSHMEM_SIGNAL_ADD
NVSHMEMI_AMO_END_OF_NONFETCH = 11
...
def putmem_signal_nbi(
    dest, source, nbytes, sig_addr, signal, sig_op, pe, _builder=None
):
    return _putmem_signal_impl(
        dest,
        source,
...
def broadcast(team, dest, source, nelems, pe_root, _builder=None):
    return extern_call(
        "libnvshmem_device",
        "",
        [team, dest, source, tl.cast(nelems, tl.uint64, _builder=_builder), pe_root],
        {
            (
                tl.int32,
...
def fcollect(team, dest, source, nelems, _builder=None):
    return extern_call(
        "libnvshmem_device",
        "",
        [team, dest, source, tl.cast(nelems, tl.uint64, _builder=_builder)],
        {
            (tl.int32, tl.pointer_type(tl.int8), tl.pointer_type(tl.int8), tl.uint64): (
                "nvshmem_int8_fcollect",
                (tl.int32),
            ),
            (
                tl.int32,
...
def putmem_rma(dest, source, nbytes, pe, _builder=None):
    return _putmem_rma_impl(
        dest, source, nbytes, pe, core.constexpr(""), _builder=_builder
    )
...
def putmem_rma_warp(dest, source, nbytes, pe, _builder=None):
    return _putmem_rma_impl(
        dest, source, nbytes, pe, core.constexpr("_warp"), _builder=_builder
    )
...
def putmem_rma_block(dest, source, nbytes, pe, _builder=None):
    return _putmem_rma_impl(
        dest, source, nbytes, pe, core.constexpr("_block"), _builder=_builder
    )
...
@core.extern
def putmem_signal_rma(
    dest, source, nbytes, sig_addr, signal, sig_op, pe, _builder=None
):
    return _putmem_signal_rma_impl(
        dest,
        source,
        nbytes,
        sig_addr,
...
@core.extern
def putmem_signal_rma_nbi_warp(
    dest, source, nbytes, sig_addr, signal, sig_op, pe, _builder=None
):
    return _putmem_signal_rma_impl(
        dest,
...
@core.extern
def putmem_signal_rma_nbi_block(
    dest, source, nbytes, sig_addr, signal, sig_op, pe, _builder=None
):
    return _putmem_signal_rma_impl(
        dest,
