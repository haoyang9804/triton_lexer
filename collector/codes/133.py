...
@pytest.fixture
def create_chunk_scan_bwd_dc_tensors(
...
def chunk_scan_bwd_dc_compare(create_chunk_scan_bwd_dc_tensors):
    prev_states_f, prev_states_b, dA_cumsum_f, dA_cumsum_b, dout, C = (
        create_chunk_scan_bwd_dc_tensors
    )
    (
        prev_states_f_ref,
        prev_states_b_ref,
        ...
        dout_ref,
        C_ref,
    ) = create_ref_tensors(*create_chunk_scan_bwd_dc_tensors)

    dC_f, ddA_cumsum_f = _chunk_scan_bwd_dC(prev_states_f, dA_cumsum_f, dout, C=C)
    dC_b, ddA_cumsum_b = _chunk_scan_bwd_dC(
        prev_states_b.flip([1]), dA_cumsum_b.flip([2, 3]), dout.flip([1]), C=C.flip([1])
    )

    dC_b = dC_b.flip([1])
    ddA_cumsum_b = ddA_cumsum_b.flip([2, 3])

    dC_ref, ddA_cumsum_f_ref, ddA_cumsum_b_ref = _chunk_scan_bwd_dC_bi(
        prev_states_f_ref,
        prev_states_b_ref,
        dA_cumsum_f_ref,
        dA_cumsum_b_ref,
