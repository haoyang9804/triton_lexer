...
def trace_inst(se: SymbolicExecutor, pstate: ProcessState, inst: Instruction):
    logging.debug(
        f"[tid:{inst.getThreadId()}] 0x{inst.getAddress():x}: {inst.getDisassembly()}"
    )


def post_exec_hook(se: SymbolicExecutor, pstate: ProcessState):
    logging.debug(
        f"seed:{se.seed.hash} ({repr(se.seed.content)})   [exitcode:{se.exitcode}]"
    )


def hook_sscanf4(
    se: SymbolicExecutor, pstate: ProcessState, rtn_name: str, address: Addr
):
    ...
    cells = {
        i: pstate.read_symbolic_memory_byte(str_addr + i).getAst() for i in range(10)
    }
                ...
                ast.ite(
                    ...
                    ast.ite(
                        ...
                        ast.ite(
                            ...
                            ast.ite(
                                ast.lnot(
                                    ast.land([cells[6] >= 0x30, cells[6] <= 0x39])
                                ),
                                multiply(ast, cells, 6),
                                ast.ite(
                                    ast.lnot(
                                        ast.land([cells[7] >= 0x30, cells[7] <= 0x39])
                                    ),
                                    multiply(ast, cells, 7),
                                    ast.ite(
                                        ast.lnot(
                                            ast.land(
                                                [cells[8] >= 0x30, cells[8] <= 0x39]
                                            )
                                        ),
...
config = Config(skip_unsupported_import=True, seed_format=SeedFormat.COMPOSITE)
...
dse.callback_manager.register_pre_imported_routine_callback(
    "__isoc99_sscanf", hook_sscanf4
)
