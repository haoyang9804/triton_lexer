...
def layer_norm_bwd(
    ...
    dw = (
        dw.view(G, -1, D).sum(1).to(weight).view_as(weight)
        if weight is not None
        else None
    )
...
class LayerNormLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            res_out, norm_weight, norm_bias, linear_weight, mean, rstd
        )
        ctx.x_shape_og = x_shape_og
        ctx.eps = eps
        ctx.is_rms_norm = is_rms_norm
    ...
    def backward(ctx, dout, *args):
        ...
        dlinear_weight = torch.einsum(
            "bo,bi->oi", dout, y.view(-1, linear_weight.shape[-1])
        )
        return (
            dx.reshape(ctx.x_shape_og),
            dnorm_weight,
