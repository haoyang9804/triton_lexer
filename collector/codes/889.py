...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "token_softmax_bloom", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 16):
            ...
            Logics = torch.rand(
                (head_num, batch_size * max_seq_len), dtype=torch.float32
            )
            B_Start_Loc = torch.arange(
                0, batch_size * max_seq_len, max_seq_len, dtype=torch.int32
            )
            B_Seqlen = torch.full((batch_size,), max_seq_len, dtype=torch.int32)
            Prob_Out = torch.empty_like(Logics)

            self.input_tensors.append(
                (Logics, B_Start_Loc, B_Seqlen, Prob_Out, max_seq_len)
            )
    ...
    def get_gbps(self, input_tensor, runtime):
        Logics, B_Start_Loc, B_Seqlen, Prob_Out, max_seq_len = input_tensor
        total_bytes = (
            Logics.numel() + B_Start_Loc.numel() + B_Seqlen.numel() + Prob_Out.numel()
        ) * Logics.element_size()
