...
def wrap_to_grpc_infer_result(
    ...
    request_id: str,
    outputs_dict: typing.Dict[str, np.ndarray],
):
    raw_output_contents = [
        output_data.tobytes() for output_data in outputs_dict.values()
    ]
...
def wrap_to_http_infer_result(
    ...
    return tritonclient.http.InferResult.from_response_body(
        response_body, False, len(header)
    )
...
def extract_array_from_grpc_infer_input(input_: tritonclient.grpc.InferInput):
    np_array = np.frombuffer(
        input_._raw_content,
        dtype=tritonclient.utils.triton_to_np_dtype(input_.datatype()),
    )
...
def extract_array_from_http_infer_input(input_: tritonclient.http.InferInput):
    np_array = np.frombuffer(
        input_._raw_data, dtype=tritonclient.utils.triton_to_np_dtype(input_.datatype())
    )
    np_array = np_array.reshape(input_.shape())
    return np_array
...
def check_all_expected_calls_made(
    ...
    if not any_order:
        ...
        for expected_call in expected_calls:
            assert (
                expected_call in mock_calls
            ), f"Expected call not found: {expected_call}; mock_calls={mock_calls}"
