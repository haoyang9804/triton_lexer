...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "attention_score", dtype=dtype, is_backward=is_backward, **kwargs
        )
    ...
    def get_input_tensors(self):
        ...
        for i in range(2, 20):
            size = 2**i
            q = torch.rand((1, 1, size, 64), dtype=torch.float32)
    ...
    def get_gbps(self, input_tensor, runtime):
        q, k, m = input_tensor
        total_bytes = (q.numel() + k.numel() + m.numel()) * q.element_size() + q.size(
            0
        ) * q.size(1) * k.size(2) * q.element_size()
