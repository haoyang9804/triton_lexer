...
def _selective_log_softmax_kernel(
    ...
    for start in range(0, N, BLOCK_N):
        ...
        logits = (
            tl.load(LOGITS + cols, mask=cols < N, other=float("-inf")).to(tl.float32)
            / TEMPERATURE
        )
...
def fused_selective_log_softmax(
    logits: torch.Tensor, input_ids: torch.Tensor, temperature: float = 0.9, mask=None
):
...
class GrpoLossFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            (ref_logp is not None and ref_logp.is_contiguous()) if beta != 0.0 else True
        )
        ...
        ctx.save_for_backward(
            logits, old_logp, ref_logp, completion_ids, advantages, completion_mask, lse
        )
