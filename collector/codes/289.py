...
@triton.jit
def triton_modulation_scale_shift(
    x_ptr,
    modulation_ptr,
    output_ptr,
    batch_size,
    ...
    if is_mod1:
        shift = tl.load(
            modulation_ptr + (modulation_offset + head_size * 0),
            None,
            eviction_policy="evict_last",
        )
        scale = tl.load(
            modulation_ptr + (modulation_offset + head_size * 1),
            None,
            eviction_policy="evict_last",
        )
    else:
        shift = tl.load(
            modulation_ptr + (modulation_offset + head_size * 3),
            None,
            eviction_policy="evict_last",
        )
        scale = tl.load(
            modulation_ptr + (modulation_offset + head_size * 4),
            None,
            eviction_policy="evict_last",
        )
...
@triton.jit
def triton_modulation_gate_proj(
    img_ptr,
    mod_ptr,
    proj_ptr,
    output_ptr,
    ...
    mod_gate = tl.load(
        mod_ptr + (modulation_offset + head_size * 2),
        None,
        eviction_policy="evict_last",
    ).to(tl.float32)
