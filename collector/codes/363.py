...
class ChunkABCFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, s, initial_state, output_final_state):
        ...
        def fwd_inner(
            q, k, v, z, B, H, T, K, V, BT, BK, BV, NT, normk=False, h0=None, ht=None
        ):
        ...
        if output_final_state:
            final_state = (
                q.new_empty(B, H, K, M, dtype=torch.float),
...
def chunk_abc(
    ...
    o, final_state = ChunkABCFunction.apply(
        q, k, v, s, initial_state, output_final_state
    )
    if not head_first:
        o = o.transpose(1, 2)
