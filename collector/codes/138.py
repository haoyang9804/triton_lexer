...
@pytest.fixture
def create_chunk_state_bwd_db_tensors(
...
def chunk_state_bwd_db_compare(create_chunk_state_bwd_db_tensors):
    x, dt, dA_cumsum_f, dA_cumsum_b, B, dstates_f, dstates_b = (
        create_chunk_state_bwd_db_tensors
    )
    (
        x_ref,
        dt_ref,
        ...
        dstates_f_ref,
        dstates_b_ref,
    ) = create_ref_tensors(*create_chunk_state_bwd_db_tensors)

    db_f, ddA_cumsum_f = _chunk_state_bwd_db(x, dt, dA_cumsum_f, dstates_f, B=B)
    db_b, ddA_cumsum_b = _chunk_state_bwd_db(
        x.flip([1]),
        dt.flip([2, 3]),
    ...
    db_ref, ddA_cumsum_f_ref, ddA_cumsum_b_ref = _chunk_state_bwd_db_bi(
        x_ref,
        dt_ref,
        dA_cumsum_f_ref,
        dA_cumsum_b_ref,
