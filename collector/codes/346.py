...
def benchmark(T, provider):
    ...
    B = int(os.getenv("BENCH_B", "8"))
    H = int(os.getenv("BENCH_H", "64"))
    D = int(os.getenv("BENCH_D", "64"))
    with torch.no_grad():
        q = torch.randn(
            B, T, H, D, device=device, requires_grad=requires_grad, dtype=dtype
        )
        k = torch.randn(
            B, T, H, D, device=device, requires_grad=requires_grad, dtype=dtype
        )
        ...
        if provider.startswith("rwkv7"):
            q = (
                torch.empty(B, T, H, D, device=device)
                .uniform_(-1, 1)
                .to(dtype=dtype)
                .requires_grad_(True)
            )
            ...
            a = -kk.clone().requires_grad_(True)
            a_scale = (
                torch.empty(B, T, H, D, device=device).uniform_(0, 0.1).to(dtype=dtype)
            )
            b = (kk * a_scale).requires_grad_(True)
    ...
    if provider == "rwkv6":
        results = triton.testing.do_bench(
            lambda: chunk_rwkv6(q, k, v, w, u), quantiles=quantiles
        )
    elif provider == "rwkv7":
        results = triton.testing.do_bench(
            lambda: chunk_rwkv7(q, w, k, v, a, b), quantiles=quantiles
        )
    elif provider == "gla":
        results = triton.testing.do_bench(
            lambda: chunk_gla(q, k, v, g), quantiles=quantiles
        )
    elif provider == "rwkv6_bwd":
        results = triton.testing.do_bench(
            lambda: chunk_rwkv6(q, k, v, w, u)[0].backward(do), quantiles=quantiles
        )
    elif provider == "rwkv7_bwd":
        results = triton.testing.do_bench(
            lambda: chunk_rwkv7(q, w, k, v, a, b)[0].backward(do), quantiles=quantiles
        )
    elif provider == "gla_bwd":
        results = triton.testing.do_bench(
            lambda: chunk_gla(q, k, v, g)[0].backward(do), quantiles=quantiles
        )
