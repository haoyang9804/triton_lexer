...
@pytest.fixture
def create_chunk_state_bwd_dcb_tensors(
    ...
    return (x, dt, dA_cumsum_f, dA_cumsum_b, dout, ngroups)


def create_ref_tensors(x, dt, dA_cumsum_f, dA_cumsum_b, dout, ngroups):
...
def chunk_state_bwd_dcb_compare(create_chunk_state_bwd_dcb_tensors):
    ...
    x_ref, dt_ref, dA_cumsum_f_ref, dA_cumsum_b_ref, dout_ref, ngroups_ref = (
        create_ref_tensors(*create_chunk_state_bwd_dcb_tensors)
    )

    dCB_f = _chunk_scan_bwd_dcb(x, dt, dA_cumsum_f, dout, ngroups=ngroups)
    dCB_b = _chunk_scan_bwd_dcb(
        x.flip([1]),
        dt.flip([2, 3]),
    ...
    dCB_ref = _chunk_scan_bwd_dcb_bi(
        x_ref, dt_ref, dA_cumsum_f_ref, dA_cumsum_b_ref, dout_ref, ngroups=ngroups_ref
    )
