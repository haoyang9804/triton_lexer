...
def causal_conv1d_bwd(
    ...
    dw = (
        weight.new_empty(B * NT, *weight.shape, dtype=torch.float)
        if weight is not None
        else None
    )
    db = (
        bias.new_empty(B * NT, *bias.shape, dtype=torch.float)
        if bias is not None
        else None
    )
...
def causal_conv1d_varlen_states_fwd_kernel(
    x, cache, offsets, D, W, BD: tl.constexpr, BW: tl.constexpr
):
    ...
    b_x = tl.load(
        x + o_t * D + o_d[:, None], mask=(o_t >= bos) & (o_d[:, None] < D), other=0
    )
    tl.store(
        cache + i_n * D * W + o_d[:, None] * W + o_w,
        b_x,
        mask=(o_d[:, None] < D) & (o_w >= 0),
    )


@input_guard
def causal_conv1d_varlen_states_fwd(
    x: torch.Tensor, cache: torch.Tensor, cu_seqlens: torch.Tensor, state_len: int
) -> torch.Tensor:
    N, D, W = len(cu_seqlens) - 1, x.shape[-1], state_len
    cache = (
        torch.empty(N, D, W, dtype=x.dtype, device=x.device) if cache is None else cache
    )
    ...
    causal_conv1d_varlen_states_fwd_kernel[grid](
        x=x, cache=cache, offsets=cu_seqlens, D=D, W=W, BW=BW, BD=BD
    )
    return cache
        ...
        **kwargs,
        ...
        if cache is not None:
            if cu_seqlens is not None:
                ...
                cache[:, :, -min(W, T) :].copy_(
                    rearrange(x[..., -min(W, T) :, :], "n w d -> n d w")
                )
        ...
        if self.use_fast_conv1d:
            ...
            if cu_seqlens is not None:
                raise ValueError(
                    "`cu_seqlens` is not supported for the naive Pytorch version"
                )
