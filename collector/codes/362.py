...
def token_shift_ref(
    x: torch.Tensor, cu_seqlens: Optional[torch.Tensor] = None
) -> torch.Tensor:
...
def token_shift_bwd(
    dy: torch.Tensor, N: int, T: int, cu_seqlens: Optional[torch.Tensor] = None
) -> torch.Tensor:
    D = dy.shape[2]
    BD = triton.next_power_of_2(D)
...
def token_shift(x: torch.Tensor, cu_seqlens: Optional[torch.LongTensor] = None):
