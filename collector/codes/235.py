...
@triton.jit
def composed_offs_1d(D0: tl.constexpr, D1: tl.constexpr, D2: tl.constexpr):
...
def composed_load(
    ...
    if TRANSPOSED:
        ...
        x_mask = (
            ptrs_mask & (COLS0[:, None] < i_cols)
            if PADDED_COL and D1 == 0
            else ptrs_mask
        )
        y_mask = (
            ptrs_mask & (COLS1[:, None] < i_cols)
            if PADDED_COL and D2 == 0
            else ptrs_mask
        )
        z_mask = (
            ptrs_mask & (COLS2[:, None] < i_cols) if PADDED_COL and True else ptrs_mask
        )
    else:
        ptrs_mask = ROWS[:, None] < i_rows if PADDED_ROW or PADDED_COL else None
...
def composed_advance(
    x, y, z, advance, D0: tl.constexpr, D1: tl.constexpr, D2: tl.constexpr
):
    x = x + advance
...
def composed_dot_both(
    lx, ly, lz, rx, ry, rz, acc, D0: tl.constexpr, D1: tl.constexpr, D2: tl.constexpr
):
...
def composed_dot_rhs(
    ...
    SUB16D: tl.constexpr = ((D0 > 0 and D0 < 16) or (D1 > 0 and D1 < 16)) or (
        D2 > 0 and D2 < 16
    )
    lhs = (
        tl.trans(lhs).to(rx.dtype)
        if TRANSPOSE_LHS and SUB16D
        else (tl.trans(lhs.to(rx.dtype)) if TRANSPOSE_LHS else lhs)
    )
...
def composed_mul_lhs(
    lx, ly, lz, rhs, D0: tl.constexpr, D1: tl.constexpr, D2: tl.constexpr
):
    lx = lx * rhs
...
def composed_mul_acc(
    lx, ly, lz, rhs, x, y, z, D0: tl.constexpr, D1: tl.constexpr, D2: tl.constexpr
):
    x += lx * rhs
