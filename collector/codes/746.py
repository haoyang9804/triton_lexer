...
from TritonBench_v1.chunk_gla_fwd import (
    chunk_fwd_intra_gated_gk_fn,
    chunk_fwd_o_gated_gk_fn,
)
...
class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__(
            "chunk_gla_fwd", dtype=dtype, is_backward=is_backward, **kwargs
        )

    def get_input_tensors(self):
        self.input_tensors = []
        for i in range(2, 12):
            B, H, T, K, V = 8, 8, 2**i, 32, 32
    ...
    def get_gbps(self, input_tensor, runtime):
        q, k, g, v, h, scale, BT = input_tensor
        total_bytes = (
            q.numel() + k.numel() + g.numel() + v.numel() + h.numel()
        ) * q.element_size()
