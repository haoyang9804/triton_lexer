...
def add_continuous(
    ...
    add_continuous_kernel[(num_ctas,)](
        lhs,
        rhs,
        out,
        out.numel(),
        num_warps=num_warps,
        BLOCK_SIZE=num_warps * 32 * 8 * 4,
    )
    return out
...
def reduce_scatter_ring_push_1d_intra_node_ce(
    ...
    if output:
        assert (
            output.dtype == input_tensor.dtype
    ...
    with torch.cuda.stream(stream):
        for stage in range(num_ranks):
            ...
            if stage != 0:
                ...
                output = (
                    output if output is not None and stage == num_ranks - 1 else buffer
                )
                add_continuous(src, buffer, output)
...
def reducer_scatter_for_each_node_ring(
    input: torch.Tensor, stream: torch.cuda.Stream, ctx: ReduceScatter2DContext
):
        ...
        for n in range(0, nnodes):
            ...
            scatter_buf = reduce_scatter_ring_push_1d_intra_node_ce(
                ...
                [
                    x[
                        cur_node_id
                        * local_world_size : (cur_node_id + 1)
                        * local_world_size
                    ]
    ...
    with torch.cuda.stream(stream):
        for i in range(0, local_world_size):
            ...
            if overlap_with_gemm:
                wait_eq(
                    signal_base_ptr + nbytes_per_scatter_signal * remote_local_rank,
                    1,
                    stream,
                    True,
                )
            remote_buf_ptr = (
                scatter_bufs_intra_node[remote_local_rank].data_ptr() + remote_offset
            )
...
def ring_reduce(
    ...
    if torch.cuda.get_device_capability()[0] >= 9:
        ...
        return ring_reduce_non_tma(
            input, output, begin_idx, num_splits, 16 if num_sms == -1 else num_sms
        )
