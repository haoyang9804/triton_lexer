...
def chunk_gsa_bwd_k(
    ...
    dg = dgv.add_(
        chunk_local_cumsum(dg, chunk_size=BT, reverse=True, cu_seqlens=cu_seqlens)
    )
...
def chunk_gsa_bwd(
    ...
    if q.shape[1] != k.shape[1]:
        dk, dv, ds, dg = map(
            lambda x: reduce(x, "b (h g) ... -> b h ...", "sum", h=k.shape[1]),
            (dk, dv, ds, dg),
        )
    dg = dg.to(s.dtype)
...
def chunk_gsa(
    ...
    if head_first:
        ...
        q, k, v, s, g = map(
            lambda x: rearrange(x, "b h t ... -> b t h ..."), (q, k, v, s, g)
        )
    ...
    o, *final_state = ChunkGSAFunction.apply(
        q, k, v, s, g, scale, hk0, hv0, output_final_state, checkpoint_level, cu_seqlens
    )
    if head_first:
