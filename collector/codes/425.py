...
def chunk_ttt_linear_fwd_h(
    ...
    assert (
        max(BK, BV) <= 128
    ), "current kernel does not support head dimension larger than 128."
    NK = triton.cdiv(K, BK)
    NV = triton.cdiv(V, BV)
    assert (
        NK == 1
    ...
    final_state_bias = (
        k.new_empty(N, H, 1, V, dtype=torch.float32) if output_final_state else None
    )
...
def chunk_ttt_linear_bwd_norm(
    ...
    dhbt: Optional[torch.Tensor],
    dv_new: Optional[torch.Tensor],
    do: torch.Tensor,
    scale: float,
    cu_seqlens: Optional[torch.LongTensor] = None,
    chunk_size: int = 16,
...
def chunk_ttt_linear_bwd_norm_ref(
    ...
    q, k, v, v_new, kh, y, h, eta, dv_new, do = [
        x.transpose(1, 2) for x in [q, k, v, v_new, kh, y, h, eta, dv_new, do]
    ]
    BT = chunk_size
    ...
    if pad_len > 0:
        q, k, v, v_new, kh, y, eta, dv_new, do = [
            F.pad(x, (0, 0, 0, pad_len))
            for x in [q, k, v, v_new, kh, y, eta, dv_new, do]
        ]
        eta[:, :, -1, :] = eta[:, :, -(pad_len + 1), :]

    q, k, v, v_new, kh, y, eta, dv_new, do = [
        x.reshape(B, H, NT, BT, -1).permute(2, 0, 1, 3, 4)
        for x in [q, k, v, v_new, kh, y, eta, dv_new, do]
    ]
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        _q, _k, _v, _v_new, _kh, _y, _h, _eta, _dv_new, _do = [
            x[i_t].to(torch.float32)
            for x in (q, k, v, v_new, kh, y, h, eta, dv_new, do)
        ]
        ...
        dy = (
            ...
            * (
                _dv_new * V
                - _dv_new.sum(dim=-1, keepdim=True)
                - x * (x * _dv_new).sum(dim=-1, keepdim=True)
            )
            / V
        )
        dx = (
            -rstd
            * (
                _dv_new * (x * _y).sum(dim=-1, keepdim=True)
                + _y * (x * _dv_new).sum(dim=-1, keepdim=True)
            )
            / V
        )
        ...
        dkh = (
            ...
            * (
                V * dx
                - dx.sum(dim=-1, keepdim=True)
                - x * (x * dx).sum(dim=-1, keepdim=True)
            )
            / V
        )
...
def chunk_ttt_linear_bwd(
    ...
    dv_new = chunk_ttt_linear_bwd_dv_local(
        q=q, k=k, eta=eta, do=do, scale=scale, cu_seqlens=cu_seqlens, chunk_size=BT
    )
...
def chunk_ttt_linear(
    ...
    if head_first:
        ...
        q, k, v, eta = map(
            lambda x: rearrange(x, "b h t ... -> b t h ..."), (q, k, v, eta)
        )
