...
@pytest.fixture
def create_fwd_scan_tensors(
    batch, seqlen, nheads, ngroups, chunk_size, headdim, dstate, delta_softplus, dtype
):
...
def fwd_compare(create_fwd_scan_tensors):
    ...
    (
        ...
        chunk_size,
        delta_softplus,
    ) = create_ref_tensors(*create_fwd_scan_tensors)

    out, out_x, dt_f, dA_cumsum_f, states_f, final_states_f = (
    ...
    out2, out_x2, dt_b, dA_cumsum_b, states_b, final_states_b = (
        _mamba_chunk_scan_combined_fwd(
            x=x.flip([1]),
            dt=dt.flip([1]),
            A=A,
    ...
    (
        ...
        final_states_f_ref,
        final_states_b_ref,
    ) = _mamba_chunk_scan_combined_fwd_bi(
        x=x_ref,
        dt=dt_ref,
        A=A_ref,
        B=B_ref,
