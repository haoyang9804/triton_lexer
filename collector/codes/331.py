...
@triton.autotune(
    configs=[
        triton.Config(
            {"BL": BL, "BK": BK, "BV": BV}, num_warps=num_warps, num_stages=num_stages
        )
        for BL in [32, 64, 128]
        for BK in [64]
        for BV in [64]
...
def fused_bwd_kernel_ds_dq(
    ...
    dKZ_block_ptr = (
        dKZ
        + (off_bs_head + B * H * start_l) * stride_dkz_bh
        + start_k * BK
        + tl.arange(0, BK)
    )
    tl.store(
        dKZ_block_ptr,
        dkz.to(dKZ.dtype.element_ty),
        mask=((start_k * BK + tl.arange(0, BK)) < DK),
    )
